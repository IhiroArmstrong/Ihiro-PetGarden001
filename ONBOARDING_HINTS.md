...
| `companion-mode` | A4 面板 | "Pick what fits this sitting." / 「选一个此刻合适的同坐方式。」 | 首次展开三选一 | 点选任一模式 | 是 |
...
| `ambient-soundscape` | A5 面板 | "Playing music gently brightens Yin's glow." / 「播放音乐时，阿寅的光会慢慢亮一点。」 | 首次展开曲目面板 | 选曲或关面板 | 是 |
...
4. 禁止教程类 UI（遮罩、高亮、箭头、分步导航）。
5. 文案用第一节已过观察式自检的中英稿，写入 locales。
6. 确认 Ambient 播放时 Rim 有可见缓亮（累计 presenceBoost + 正在播放 lift）；补单测。
7. 实验室 debug 面板增加「清空 hints-seen」；仅非 ?product=1。
8. 单元测试：首次显示、已读不再自动显示、补救始终可调、hintId 互不干扰、resolveScene。
9. 更新 TEST_TRACKER.md；更新 PRODUCT_MOMENTS.md 说明为何不做集中式引导。
10. 产品壳 ?product=1 仍显示「?」与即时提示（属于产品表面，不是实验室调试条）。
```
