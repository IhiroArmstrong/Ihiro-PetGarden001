...
1. Idle → 右上 **Support Yin** → 中间 Membership 卡报价行见 **About $8.99 · billed monthly**（与左右卡同一位置）；或 ⋯ 米色 Subscribe CTA → `#yin-membership-card` 见同一金额  
2. 点 **Subscribe**（须再点一次；开卡后勿误触立刻进 Stripe）  
3. Stripe **subscription** Checkout 金额应约为 **US$8.99/月** → 付完回跳（URL 带 `membership_session=cs_…`）  
4. **期望**：  
