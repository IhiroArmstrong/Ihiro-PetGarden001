...
> **状态（2026-08-18）**：政策已拍板。**L0 告一段落**（用户书面：hitch 无卡顿；8GB 数据缺失用「低配默认不出入口」兜底）。#336 探针 PR 仍开、不合也不挡品味层。不开 L1。  
...
L0 **告一段落**（实测无卡顿；缺 8GB 数据 → 低配不出入口）。数据补齐 / 生产入口仍须口令「开工桌面陪伴 L0」。本文件合入 **不等于** 已开工 L1。
