...
| **桌面 Focus Tiger Pro（已拍板 · Checkout 未接）** | L1 **入口正常开放后**：Stripe **US$12.99/月**（Price `price_1U6EB1FuIhgJPGLiuciuX1to`）。**包含** Base（B 轨）+ 合格 Electron 本地智能体。现货 Support 仍三卡。权威：`task-desktop-on-device-companion.md` |
...
**已拍板（2026-08-20）**：Stripe **Focus Tiger Pro** Price `price_1U6EB1FuIhgJPGLiuciuX1to` 已记文档。Pro **包含** Base（应用内 Yin Membership / B 轨）+ 本地智能体。**禁止**现在接 Checkout / 改三卡。L1 后付款可走 Safari；用模型仍 Electron。低配不得用购买覆盖隐藏入口。权威：`task-desktop-on-device-companion.md`。
