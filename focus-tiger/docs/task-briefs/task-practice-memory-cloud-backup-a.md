...
| tip-jar / entitlement 本地 cache | 另有支付 restore；勿与练习快照混桶 |
| Ambient 偏好、语言等设置 | 可另议；首版不做 |

实现 PR 若要增删字段，须在 PR 描述写清并同步本 Brief 表。

## 工程草图（实现时）

### 客户端

1. **本机权威**：现有 store 读写路径不变；快照 = 序列化上表 keys 的只读导出。  
2. **上传触发**（静默，不挡 UI）：  
   - 会话有意义写入后（例 Focus/Journey append）→ debounce（建议 ≥5–15 min 或「本会话 Idle 一次」）；  
   - 另可低频定时（例 24h 内最多 N 次）。  
   - Focus / Arrival / Reflection **进行中不抢网、不弹失败**。  
3. **恢复触发**：启动检测到「覆盖 keys 全空或明显被清」+ 已绑邮箱 → 拉最近快照合并/覆写（策略实现时单测锁：空库才自动恢复；非空本机优先，避免误覆盖）。  
...
- **主路径**：绑邮箱 + 同意 → 完成一场有 Journey 写入 → 静默上传（DevTools/日志可验；UI 无打断）→ 清相关 localStorage → 恢复后 Journey / 练习日仍在。  
- **回流**：未绑邮箱 → 角落引导可见、主路径可练；拒绝同意 → 不上传。  
- **保护**：Focus 中不弹失败；与 tip/Sanctuary restore **语义分立**（同一 OTP 基础设施，不同 purpose/KV）。

## 权威交叉

- `PROCESS.md` 最近拍板 + Backlog「练习记忆云端备份」  
- `FREE_PAID_MATRIX.md` A 行「练习记忆 · 云端快照备份 / 恢复」  
- `SHARED_RESOURCES.md`（journey-log / practice-days / milestone-glow）  
