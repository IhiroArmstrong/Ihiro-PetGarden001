...
- 新建可点击叠层：本 Brief 落地 O-04 完整契约面 SSOT（`overlayUiSurfaceContract.js`）。反馈列现为 `mutationFeedback.{pending,success,fail}`（定义见点击原则「持久化三态可见性」）。结论：存量行允许单键 `grandfather:true` gap；**新 occupancy 行三键禁止 gap**。
