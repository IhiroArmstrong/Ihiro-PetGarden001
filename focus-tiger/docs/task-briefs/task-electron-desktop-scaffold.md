...
> **状态（2026-08-18 步骤 B 已接线 · 待 Mac 人工测场景 AB）**：壳已拍板 Electron（#326）。规格 #329 / 两步 #331。步骤 A 窗口已在 `focus-tiger/desktop/`。**步骤 B**：托盘 + 关窗 hide + `AttentionSignals` **SB-18** 同一条改动线。Cloud Linux **不能**验窗口 / DMG / 公证 / 托盘。  
...
**进度（2026-08-18）**：上列三项已接线。待 Mac 人工：场景 AB 主路径 + 对照 B + 托盘退出。
