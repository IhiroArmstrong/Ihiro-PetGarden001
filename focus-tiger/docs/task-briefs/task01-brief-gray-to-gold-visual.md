...
> **⚠️ 历史注记（2026-07-15）**：本任务书按当时的"灰→金材质插值"方案编写并已交付验收，**原文保留不改写**。该视觉方案后被新原则取代：角色本体固有色恒定不变，金色由外围光环/环境光反射（Rim Light）表达，详见 `DESIGN.md`「视觉状态」章节与 `ARCHITECTURE.md` 文中 TODO。3D 侧按新原则的重构并入未来「奖励柜」任务。阅读本文时请以新原则为准。
