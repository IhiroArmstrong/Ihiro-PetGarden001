...
把乐五斋原诗纳入产品**隐形原创标志**：达标练习水平后，在完成仪式与 Reflection 之间以 Quiet Line 式卡片首次出现；之后可从 Idle 菜单重读。Case 1 = 《芥子须弥》；Case 2 = 七言歌行（山海奇云…英雄岂是池中物）。

## 契约

| 项 | 口径 |
|---|---|
| 门槛 | `score = practiceDayCount + floor(lifetimeMinutes/60) ≥ 21`（`computePracticeScore`）；两 case 同一门槛 |
| 首次时机 | 计时会话完成反馈播完 → `finishCompletedSession` → 印章卡（下一未揭示 case）→ Continue → Reflection |
| 文案 | Case 1：ZH 四句 + EN + `乐五斋诗稿 · Verses of Le Wu Zhai`。Case 2：ZH 四句 + EN + `乐五斋七言歌行 · Song Verse of Le Wu Zhai`。不随 locale 替换原诗 |
...
- 单元：`mustardSeedSeal.test.js`（含 badge src、两 case、旧档迁移）+ orchestration 解锁行
- 人工：见 `TEST_TRACKER`「芥子须弥纪念印」——须见方章（非 tip 圆金章）；Case 2 须见七言歌行署名
