...
> **状态（2026-08-14）**：Phase 1 内容层已接线（本支 `feature/quiet-line-insight-spark`）。生产洞察池 **10** 句（`INSIGHT_1`–`10`）。  
...
生产键连续 `INSIGHT_1`–`10`；候选 **#7 / #11 / #14 / #15** 留给小改或重写后再占新键（下一档从 `INSIGHT_11` 起），避免和现网 id 打架。
...
| 7 | Can looking look, without turning into a project? | 小改：去掉 ACT 行话 `project`，换成可感知意象 |
| 11 | If there is a watcher, does the watcher also come and go? | 重写：问句勿暗含「观察者亦无常 / 无我」结论 |
| 14 | Not every tightness needs a name to be felt. | 小改：避开与 #4 重复的 tightness |
| 15 | After the search for a self, sitting is still sitting. | 重写：保留「回到日常」，勿化用砍柴挑水句式 |
