...
> **状态（2026-08-11）**：**Phase 1–3 已合 develop（#233 / #234 / #238）**。圣诞文案桶人审 `ok` + `winter-quiet-wash` 氛围 UI + 总开关/`contentReady` 已翻真；仅 **B 轨 entitled** 用户在节日窗可见。无新 PNG 姿态序列（poses 空）。观感 TRACKER 行仍待人工。下一步 = Phase 4（其它节日配置/素材；情人节调性审），须另开任务。  
...
| **3** | 圣诞节素材+文案人审 ok；验收清单；翻开关小范围/全量 | 总开关 **开**；圣诞节 `contentReady: true`；**已合 develop（#238）** |
...
| Phase 3 | `feature/seasonal-theme-phase3` | **已合 develop（#238）**；TRACKER 圣诞观感行仍待人工 |
| Phase 4+ | 另开 `feature/seasonal-theme-*` | **须书面授权**（其它节日配置/素材；情人节调性审） |

QA：`?product=1&mockDate=2026-12-20`（需 B entitlement / preview）；未购应无 wash。开工口令示例：「按 `task-seasonal-theme-engine-v1` 开 Phase 4」。
