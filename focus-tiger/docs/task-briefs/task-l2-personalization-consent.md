...
> **状态（2026-08-26）**：产品会已拍 **H.3 V1 五键 + Pack 契约 + 异步不变量**（#454）。同日拍板 **关即删**、HINT+可展开 DETAIL、三语 LABEL/HINT/DETAIL **草稿**。  
> **本切片禁止**：写入 `src/locales/*.json`、Privacy 现网开关、Worker、ingest、改 `l0Config.js`、改 L1/桌面 generate runtime、改生产行为。  
> **权威**：`YIN_PERSONALIZATION_ENGINE.md` §H · §G · §I · 文首不变量。文案草稿以**本 Brief 附录**为准；locale 须口令「开工 L2 UI」。  
> **下一刀（仍文档）**：L2 **身份键**（删除主语）。再下一刀：Cloud algorithm contract。运行时须口令「开工 L2」。
...
点击反馈：本 Brief **不涉及可点击交互**（无 UI）。开工 L2 UI 后开关须 0–1s 看见选中/关闭。
...
- `PRIVACY_NOTICE.md` 仍是 v1.0.0 本地优先稿；**禁止**本切片把 YPE 开关句写进该页或 locale。漏斗现网可用「anonymous counts」口径——**仅漏斗**；YPE **禁止**抄 anonymous / 匿名。
- YPE 身份键 **未拍**（H.3：邮箱不作 YPE 默认主键）。删除主语本切片只锁产品句「与这次同意绑定的云端个人化记录」；跨设备 / 是否复用备份 OTP → **下一刀文档**。
...
V1 删除主语：**与这次同意绑定的那份云端个人化记录**。不暗示全账号、全设备，直到身份键拍板。
...
用户 DETAIL：可举例完成情况、反思频率、**你选择的陪伴方式**、练习日计数。若点名 intervention，须写成「与该陪伴档对应的干预偏好」，**禁止**写成系统另外推断的安静/温暖人格或 `derived quiet/warm preference`。

不上云（须在 HINT/DETAIL 点名否定）：Confide 原文、阿寅个人记忆摘要、你对阿寅说的正文。

### 禁止词（用户可见文案）

training / train the model / 教模型 / 云端大脑 / cloud brain / anonymous / 匿名 / 诊断句 / personality profile / behavioral score / ranking / scoring / intervention probability。
...
consent=false
  → no further ingest
  → delete cloud personalization data bound to this consent
...
身份键未拍前，实现不得用「anonymous id」当隐私文案；存储键可以是设备维，但用户句与架构文档须写 **limited structured signals bound to this consent**。跨设备删除是否复用练习备份 OTP → 下一刀。
...
- 在用户文案里承诺未拍板的身份范围（全账号 / 全设备）

---

## 附录 · 三语文案草稿（待产品过 · 非 locale）

> 过稿后仍须「开工 L2 UI」才接线。日文保持产品语气，不直译 technical pack。
...
- **What leaves this device?** Limited structured signals such as focus completion, reflection frequency, your chosen companion style (and the matching intervention preference derived from that same choice), and practice-day counts.  
...
- **可能离开本机的？** 少量结构化信号，例如专注完成情况、反思频率、你选择的陪伴方式（以及由同一档派生的对应干预偏好）、练习日计数。  
...
- **この端末から出るものは？** 集中の完了状況、振り返りの頻度、あなたが選んだ寄り添い方（同じ選択から派生する対応する介入の好み）、練習日の回数などの、限られた構造化シグナル。  
