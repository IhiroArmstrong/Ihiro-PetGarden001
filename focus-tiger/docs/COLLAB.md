...
4. **验收结论须带三元组**：每条测试/验收结论必须注明 **commit hash + worktree 路径 + 本地端口**。**关单默认** = 当时 `origin/develop` tip · `…-wt-qa-develop-tip` · **`:5173`**（相关 PR 已合后硬刷新即可；SSOT：`TEST_TRACKER.md`「关单默认路径」）。feature/fix **自检**才写旁支路径与其它端口。禁止只说「在 develop 上测到……」。
