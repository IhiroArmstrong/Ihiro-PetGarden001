...
**回归锁约定（2026-07-19；2026-07-20 增补；2026-07-21 N15）**：交互类修复不得只验 Happy Path；须含回流路径、禁止可点却静默失败、门闩失败用例或人工锁路径、同主题行步骤不互斥。**重写 / 改转场**：改前列已好清单、继承观感契约、序列衔接进高风险表。声称修好前先跑 **`npm run test:smoke` 与 `npm run test:e2e`**；确认修复的 bug 须留回归锚（不限门闩）。每次任务汇报末尾须独立 **「待你决定 / 待你知道」**（N14）。**Bug 修复一体包（N15）**：代码或修正措施 + **相关项目文档同批更新** + **立刻本地 commit**（不必再问）；缺文档或未 commit = 未完成。**push 仍须用户明确要求**。叙事全文见 **`DEV_WORKFLOW_QUALITY.md`**；摘要见 `PROCESS.md`「回归锁工作法」；门禁见 `.cursor/rules/focus-tiger-regression-lock.mdc`。
---
