...
| **17** | `src/ui/FocusAwarenessCardUI.js` | `#focus-awareness-card` Focusing 间隔拍觉察短句（可重复；**不**写 Whisper seen；气质近 Whisper） |
...
| **18** | `src/ui/ConfideToYinUI.js` | `#confide-to-yin-card` 向阿寅倾诉卡（⋯ / 抽屉；检索不生成；默认挂载关；QA `?confide=1`） |
