...
- **星光斗篷入睡 / 卸斗篷苏醒换装（2026-08-04）**：入库 `starlight-cloak-sleep` + `starlight-cloak-wake`（各 67 帧 @12fps≈5.6s）；`cloakSleep` / `sleeping` / `dormantWake` 全切新素材；旧 `cloak-sleep` 勿接。分支 `feature/starlight-cloak-sleep-2026-08-04`。拓展时段（深夜 Idle / 冷启动时段分流 / 长离苏醒）**未接线**，仅文档意向。
...
- **星光斗篷拓展场景（2026-08-04 · 待拍板是否接线）**：素材已换装进 DORMANT 入睡/Honesty 苏醒。你列的拓展——深夜 Idle 披斗篷、冷启动时段分流（晨苏醒 / 日 Idle / 夜入睡）、深夜 Rise 直接入睡、长离回前台苏醒——**尚未改状态机**；是否进 v1.0 须另开任务拍板。
