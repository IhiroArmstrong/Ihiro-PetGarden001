...
- **Ambient · 右上音符常驻小 label（2026-08-06）**：未读常驻 `HINT_AMBIENT_SOUNDSCAPE`；点击永久 markSeen；done 后悬停 `AMBIENT_NOTE_HOVER`、离开消失。分支 `feature/ambient-note-persistent-label`。
...
- **增长向内容包决策锁（2026-08-06）**：分析师+调查对齐后用户「合理则办」。顺序 ① YouTube ⋯平级 → ③ 签文 `COPY_POOLS`+存图 → ②A 电子书下载 → ②B 独立 `ebook-unlocked`（文案与 Glow streak-7 叙事刻意分开）。无 Settings/Culture Space；不绑壳/分享卖点。Brief `task-growth-content-pack-decision.md`。**① 已合 develop**（PR [#148](https://github.com/IhiroArmstrong/Ihiro-PetGarden001/pull/148)）；**③ 已合 develop**（PR [#153](https://github.com/IhiroArmstrong/Ihiro-PetGarden001/pull/153) · `fc6aa94`）；②A/②B 未开工。
...
**最近拍板（2026-08-06）**：增长向内容包（YouTube 菜单入口 → 每日签文存图 → 电子书 A 下载 → 电子书 B 独立解锁）口径已锁；**可延后增发**，不挤占壳选型/主路径债。禁止 Reflection 边缘入口；签文不复活 soft-schedule、不做「一键分享」核心承诺、仅 en+ja；电子书 B **禁止**碰 MilestoneGlow，文案禁「里程碑/成就/打卡」混 Glow。Brief：`task-briefs/task-growth-content-pack-decision.md`。**① Zen Cinema / ③ Quiet Line 已合 develop**（PR #148 / #153）；下一增长项按序为 ②A 电子书静态下载。
