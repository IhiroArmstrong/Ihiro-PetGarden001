...
- **远程参数化候选全项目审计（2026-09-12 · 纯文档 · 无运行时）**：`REMOTE_PARAM_CANDIDATES.md`。五条门槛扫前端+Worker。品味层 Emotion Weight = 云 API 仍读源码冻表（伪远程）；成长度量日封顶已试点不重复开工；莲花阶梯建议并入现有花园 KV；寅币日封顶与 MilestoneGlow 7/21/100 留产品判断。
