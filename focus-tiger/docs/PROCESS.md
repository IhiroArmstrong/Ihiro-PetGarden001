...
- **CI doc-contract 缺依赖红（2026-07-22）**：远端 `develop` 已推上后 workflow 跑失败——未 `npm ci`，`scenario-smoke` 间接 import `three` → `ERR_MODULE_NOT_FOUND`。已在 `.github/workflows/focus-tiger-doc-contract-check.yml` 补安装步骤；须再 push 后看 CI 是否转绿
