...
- **应用内提醒偏好 + 横幅候选判定（2026-07-22）**：`reminderPreference` 本地存 `{ hour, minute }`；`evaluateInAppReminderBanner` 在「已设置 + 已过提醒时分 + 今日未完成」时返回 `{ shouldShow, messageKey: 'reminder.gentle_waiting' }`；**未接 UI / 未接 visibility**；完成判定暂用 `DailyCompletionStore.hasCompletedToday()`（含 Honesty / 微仪式）
