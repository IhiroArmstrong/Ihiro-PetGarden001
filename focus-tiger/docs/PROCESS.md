...
- **星光斗篷入睡 / 卸斗篷苏醒（2026-08-04 · 入库后 Undo · v3 换帧）**：曾接线主线后用户书面 Undo 回 `cloak-sleep`。同日晚用 `Yin_Starlight_Cloak_Sleep_forward_v3` **替换** `starlight-cloak-sleep` / `-wake` 67 帧；产品仍**勿接**，待观感过关再议接线。拓展时段仍未接线。
...
- **星光斗篷拓展场景（2026-08-04 · 待拍板；主线已 Undo）**：星光素材已入库（**v3 帧**）但**勿接**。你列的拓展仍未改状态机；须观感过关后再议接线。
