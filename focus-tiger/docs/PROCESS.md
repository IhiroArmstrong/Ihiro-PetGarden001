...
3. 门闩类行为有失败用例单测，或 TEST_TRACKER 写明「人工锁路径」  
...
**最后更新时间**：2026-07-19 18:30（UTC+8）

**当前技术路线**：主线为 **2D PNG 序列帧动画**（素材来源：图生视频 + 抽帧，见 `ARCHITECTURE.md`）；既有 **3D 多姿态 GLB** 资产与 `PoseManager` / `DynamicMotion` 等代码**完整保留**，改用于未来「奖励系统」塑胶公仔展示，不再作为主界面情绪表现载体。

**近期落地（待人工测试）**：

- **Honesty 桥接 CTA**（2026-07-19 拍板）：补登 + thanks 后轻量「要不要现在也坐一会儿？」；Yes → 完整 Arrival（不跳过、不开 Ambient）；No → idle；当日一次。定稿 `HONESTY_BRIDGE_CTA.md` · TEST_TRACKER 已登记
