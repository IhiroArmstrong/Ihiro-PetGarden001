...
- **练习记忆云端策略拍板（2026-08-12 · 仅文档）**：A **免费**快照备份/恢复优先（防 Safari/ITP 等本机静默清库）；B **付费**多端无缝同步可后排；身份 **唯一复用邮箱 OTP**（一套身份、两种用途；不做 device id 跨端）。首版备份/恢复、非实时双向同步。未绑邮箱 = 无云端兜底 + 须温和提示。权威：`FREE_PAID_MATRIX` 两行 + 下文 Backlog「练习记忆云端备份」。无运行时。
...
权威表：`FREE_PAID_MATRIX`；详情 Backlog「练习记忆云端备份」。**仅文档；无运行时。**
...
> **拍板（2026-08-12）**：产品面已定；**仅文档**。实现须另开 Task / Brief。对账表：`FREE_PAID_MATRIX`「练习记忆 · 云端快照备份 / 恢复」与「练习记忆 · 多端无缝同步」。
