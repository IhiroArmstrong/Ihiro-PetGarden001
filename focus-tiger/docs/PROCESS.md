...
**最后更新时间**：2026-08-21（UTC+8） · 生产 Worker Redeploy Version `e7026512-664c-42c1-8cc7-954044eb8a85`（#378 layout ingest 现网已上；部署源 `1f8e184f`） · Companion 三句 hint 锁现稿（#381） · Companion 模式卡一句话 + 冷启动 30s/3min 验收脚本（#379 已合 tip `5352356`） · 意愿漏斗 layout=tea-first|sanctuary-first（#378 已合） · Support Modal 未练习请茶优先 · 精灵占用仲裁层（睡/欢迎/付款一处拍板）· 宣传站 Slice 0 已入库 `marketing-site/`（现网 DNS 未绑）· Welcome 后短切 tab 不得披毯 / Reflection 日签抬离三球（#341）· Reflect 开着不得 cloakSleep（#347）· 口令「开工桌面陪伴 L2」已接线（fallback 短生成；Checkout 未接）· #362 L1 已合 · 关单能聊仍待 Electron 人工 · 品味层四问筛选尺已拍板 · 生产 Worker 现网 schemaVersion 1 + layout 白名单 · overlay 开着 Arrival 叠化闪（`RB-20260820-L330`；`tasteLayer=0` 正常）· Quiet Line overlay 未开工 · Support 将来五卡 Price 已记、Checkout 未接
**当前技术路线**：主线为 **2D PNG 序列帧动画**（素材来源：图生视频 + 抽帧，见 `ARCHITECTURE.md`）；既有 **3D 多姿态 GLB** 资产与 `PoseManager` / `DynamicMotion` 等代码**完整保留**，改用于未来「奖励系统」塑胶公仔展示，不再作为主界面情绪表现载体。

**近期落地（待人工测试）**：

- **Companion 模式卡一句话 + 冷启动 30s/3min 验收（2026-08-20 · #379 已合 tip `5352356`；#381 锁现稿）**：三卡 hint 改结缘/静舍语感，不改标题、不复活 auto tip（SB-15）、不碰占用仲裁。30 秒/3 分钟专家稿落成 TRACKER **验收脚本**（3 分钟用 `?sessionMinutes=1`，不改默认 10 分档，不把 nod-bow 列为必测）。**同日用户书面**：三句先按现稿；人工测 How shall we sit? 时看语感；再改只动 locale，不必重开逻辑。Brief `task-companion-mode-copy-coldstart.md`。
- **意愿漏斗 layout 维（2026-08-20 · #378 已合 tip `8535da1`；2026-08-21 现网已上）**：Support 打开/CTA 记 `layout=tea-first|sanctuary-first`（及 `support_cta:tea:tea-first` 交叉键）。**上云只走 Privacy 漏斗 opt-in**，不走练习记忆联网备份（用户书面：两套同意继续分开，禁止备份成功后自动开漏斗）。**2026-08-21 本机 wrangler deploy 成功**：生产 **`focus-tiger-cloud`** Version **`e7026512-664c-42c1-8cc7-954044eb8a85`**（`origin/develop` tip `1f8e184f` / #380；163 / ihiro；未用 `--temporary`）。Privacy 开关打开后现网 KV **可以**留下 layout。Brief `task-support-funnel-layout.md`。
...
- **意愿漏斗 opt-in 回传（2026-08-12 · #262 已合 tip `582e79f`）**：Privacy 明示同意；默认关；匿名 counts → Worker；不挡支付。**≠** Journey Log 联网备份。**2026-08-20** 载荷含 Support `layout`（#378）。**2026-08-21** 现网 Version `e7026512-664c-42c1-8cc7-954044eb8a85` 已含该白名单。TRACKER 待人工。
...
- **开工前优先级（2026-08-21 更新）**：#358 / #348 / #336 / **#362 L1** / **#341** / **#343** / **#347** / **#365** / **#378** / **#379** / **#380** / **#381** 已合 `origin/develop`。**L2 口令已执行**（本旁支）。品味层 **#349 已合**；**2026-08-21 本机 wrangler deploy** 现网 Version **`e7026512-664c-42c1-8cc7-954044eb8a85`**（v1 冻结表 + #378 layout ingest）。品味层**下一刀** Quiet Line 句包 overlay：须先修 overlay 开着时 Arrival 叠化闪动（`RB-20260820-L330`），再口令开工。挥手点播 Play 在 #356（**≠** 品味层）。**下一步** 四页签珍藏壳。**仍不接**第四卡 Pro / 第五卡 Add-on Checkout（等关单级能聊）。Electron 步骤 B 已接线、待 Mac 场景 AB。
...
- **云端品味层（2026-08-18 · #349 已合）**：可选拉取 + 本地降级。Brief `task-cloud-taste-layer.md`。四问筛选尺 **2026-08-20 拍板**。生产 **已本机 wrangler deploy**（当前 Version `e7026512-664c-42c1-8cc7-954044eb8a85`，现网 v1；覆盖 `5b5b3451-…`）。下一刀 Quiet Line overlay **未开工**。
...
- **付费 · 意愿漏斗 opt-in 回传（#262 已合 tip `582e79f`；layout 维 #378 已合 tip `8535da1`）**：Privacy 明示同意；默认关；**≠** Journey Log 联网备份。**2026-08-21** 现网 Version `e7026512-664c-42c1-8cc7-954044eb8a85` 已含 layout 白名单。关单级人工见 TRACKER。
...
- **云端品味层**（旧称 v1.1 云端算法）：权重覆盖 + 日签/文案池；播放器永远本地；**窄冻结已拍板**；**四问筛选尺 2026-08-20 拍板**；**schemaVersion 1 可选接线已落地**（**2026-08-21 本机 deploy** 现网 Version `e7026512-664c-42c1-8cc7-954044eb8a85`，覆盖 `5b5b3451-…`；见 Backlog「云端品味层」）；下一刀 Quiet Line overlay 未开工；不进 v1.0.0 核心路径、不抢 SemVer `v1.1.0` 号
...
- **开工**：用户 2026-08-18 书面「L1/L0 可告一段落 → 安排下一步」视为口令「开工云端品味层」。**生产 Worker** 须单独「部署」才从旧 mock 换成 v1 表。**2026-08-20** 用户已书面同意部署；**同日本机 wrangler deploy 成功** Version `5b5b3451-4c35-4d9b-b27b-622b72ed673e`（现网 JSON 含 `schemaVersion: 1`）。**2026-08-21** 再 Redeploy Version **`e7026512-664c-42c1-8cc7-954044eb8a85`**（含 #378 layout ingest；v1 冻结表仍在）。
