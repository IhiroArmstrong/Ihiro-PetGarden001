...
- **Confide 离线语义冻表（2026-09-20 · Prompt 7）**：`npm run test:confide-semantic-acceptance` 用真 embedding GGUF 跑 A/B 各 50 + 3 锚点（leave-one-out）。不改现网回复。不进日常 smoke。Stage 2 切真路由仍等 Prompt 8 标注 CSV 后 PO 拍板。
...
- **全项目阻断式确认审计（2026-09-14 · 纯文档 · 无运行时）**：`MODAL_USAGE_AUDIT.md` 为 A 类 5 条 / C 类 10 条的唯一打勾处。PR #763 的 R/F/M 15 行已吸收（F7/M3/M4/R4 已判 B；新增 S03/S21）。勾选 G02/G06/G08 **不等于** PR #762 写代码（须另开对话「开工」）。NA4/NA5 批准只确认方向，改造仍跟 P4 排期。HIG round1 原稿同批入库。

- **官网 DMG 自动更新器 Brief（2026-09-12 · 纯文档 · 无运行时）**：不重开壳选型。第一份收费 DMG 前必须带检查更新。MVP：空闲左下芯片 → 后台下载 → 用户确认重启；失败有 Retry / Not now；Setapp/MAS 编译期关自建更新。验收分层（单测 / DEV 假通路 / 本地 feed），不靠两次正式发版。权威 `task-electron-desktop-updater.md`。运行时另口令。

- **远程参数化 §6 本批收口（2026-09-16 · 纯文档）**：① 品味权重+Honesty30 → `TASTE_LAYER_KV` **已合**（#722 · #724）；② 莲花阶梯 → `GROWTH_METRICS_KV` **已合**（#725）；③ 寅币日封顶 **Step D = A**（冻表、不远程）。§6 三条实现/评估轨均已闭环；①② 人工验收仍见 tracker 碎片（待人工测试）。下一刀（须另口令）：通用 overlay helper（§5）或新审计批。**这次不做**仍有效：Stretch / 好奇 / 1h 冷却。**暂不搬**：MilestoneGlow 7/21/100、徽章 `/3`、芥子 21。权威 `REMOTE_PARAM_CANDIDATES.md` §6。

- **远程参数化候选全项目审计（2026-09-12 · 纯文档 · 无运行时 · #717）**：`REMOTE_PARAM_CANDIDATES.md`。五条门槛扫前端+Worker。品味层权重 **#722 后已真 KV**（空=冻表）；成长度量日封顶与莲花阶梯均已试点。收口见上行 §6。

- **Yin Evolution 意义层方向锁（2026-09-10 · 纯文档 · 无运行时）**：`YIN_EVOLUTION.md` — 关系叙事叠在现有 Progression Engine 上；不改 `scoreFormula.v3`。看板：**重写** Epic #640（莲花池降为 Visible Growth 切片），#643 Journey 保持独立（P0 表面）。P0 / 莲花文案 PR 须另口令。
...
- **防剽窃层方向锁（2026-09-02 · 2026-09-06 补值得四测）**：品味云 / YPE 云 / 句包 overlay。准入四问 **与** 值得保护四测（复制/成熟/性价比/可维护）分开。SSOT `ANTI_PLAGIARISM_LAYER.md` §3 / §3.2。Quiet Line **#543**；YPE V2 **#545**（管道在、秘密薄）；Confide copy **#548**。
...
- **Gate 0.D 架构锁（2026-09-01 · 纯文档 · #524）**：三门禁（Model 暂过 / Architecture 未过 / 生产第一刀 #523）+ Hard/Soft/Pragmatic + 双命中合同。SSOT `LOCAL_AI_PHASE1_TASK_PLAN.md` §6.1。
- **E′ 规则预筛进发送层（2026-09-01 · `fix/confide-eprime-rule-prescreen`）**：把实验室已证优先级写成现网正则（软 BOUNDARY C13–C17、陪伴 sit next / breathe together、OTHER 窄门 consistently / mood trend / check-in）。**不**把 `companion:intent-diagnostic` 接到 Share。A14/A15/D7 无关键词句仍进 L3。TRACKER 待 Electron 人工。
- **Web 本地 AI 挂载 PO 拍板（2026-09-01 · 纯文档）**：真实 Web 壳内浏览器端本地推理 **暂不立项**（ROI 不足；须 PO 书面翻 flag 重评）。Electron / `?confide=1` **不算** Web ship。维持「网页结账、桌面用 AI」。SSOT `LOCAL_AI_WEB_MOUNT_PO_DECISION.md`；交叉 `task-desktop-on-device-companion.md`。

- **叠层占用三问接 registry（2026-08-31 · 本旁支 `fix/overlay-three-questions`）**：Confide / Privacy / Idle 玻璃卡 / Support 族摸头与进睡走 `derive*`；Privacy 点空白关 sheet 本身。**2026-09-01**：合入前补 Unlock Lifetime 失败不重开已关卡 + Support 淡出不抢点 + 冷启动额头一次提示；摸头回 Idle 显式 CapCut freeze；**Vite `/api` 代理**让 `:5174` 结账绕过 Worker 只允许 `:5173` 的 CORS。风险 5–10 仍未改。须人工复测，CI 绿 ≠ 可合入。
...
- **意愿漏斗 opt-in 回传（2026-08-12 · #262 已合 tip `582e79f`）**：Privacy 明示同意；默认关；匿名 counts → Worker；不挡支付。**≠** Journey Log 联网备份。**2026-08-20** 载荷含 Support `layout`（#378）；现网 Worker **尚未** Redeploy 该白名单。TRACKER 待人工。
...
- **验收基线 + 新鲜度门禁（2026-07-29；2026-09-01 扩触发）**：关单级人工验收 **只认 `origin/develop` tip**（`TEST_TRACKER` 文首 / `RULES_INDEX` → `qa-develop-tip`）；Agent 正式邀测、声称 develop 行为、或判断某功能是否已实现/已开工/已合入（含随口提问）前须跑 `npm run check:branch-freshness`（regression-lock「分支新鲜度」/ `branch-freshness`）。`Z_INDEX.md` 入 `RULES_INDEX`（`z-index-registry`）；PR 模板加 fixed 壳 / 375 邻接勾选。核实后删除空壳长命分支 `feature/wide-idle-more-menu`、`feature/onboarding-hints-followup`（ahead=0，已是 develop 祖先）。
...
- **本地 Cursor 能耗护栏（2026-07-26；2026-07-31 收紧；2026-08-11 补限时；2026-08-21 #383 扩 `.cursorignore`）**：根目录 `.cursorignore` + `.cursorindexingignore` 已合入 `develop`（PR #3；运行时媒体全挡见 PR #383）。Cloud 启用须提醒「独立会话」；起过 Vite/Playwright 须在收尾提醒确认已关（`focus-tiger-browser-energy.mdc`）。**2026-07-31**：取消窄屏/口头开 IDE Browser 特例；`deny-ide-browser-mcp` 硬禁 `cursor-ide-browser`（Safari 响应式 / Playwright 代窄屏）。**2026-08-11**：临时解禁路径的连续开放时长上限、续开不清零与精确时间戳汇报 → 以 [`.cursor/rules/focus-tiger-browser-energy.mdc`](../../.cursor/rules/focus-tiger-browser-energy.mdc) **当前生效条文**为准（本文档**不复述具体数值**）。**2026-09-02**：`Merged` 后必须新开会话 + 模型一行速查 → `agent-token-cost` + 下文「Merged → 必须新开会话」。非产品 UI，无需 TEST_TRACKER 人工项。
...
- **Cloudflare Workers 骨架（2026-07-22）**：`focus-tiger/cloud/` 独立包（`wrangler` + TS）；stub `POST /api/daily-message` / `POST /api/emotion-weight` + 字段校验 + 内存限流。**2026-08-18**：前端可选 overlay 已接线；生产须「部署」才从旧 mock 换成 `schemaVersion: 1` 冻结表。**2026-08-20**：口令已给；**同日本机 deploy** Version `5b5b3451-4c35-4d9b-b27b-622b72ed673e`，现网已是 v1 冻结表。本地 `cd cloud && npm run dev`（见 `cloud/README.md`）
...
- **陪伴 · 桌面端侧窄例外**：**L0 #336 / L1 #362 已合**。**L2 短生成已接线**（宽屏 fallback）。低配默认不出入口。**2026-08-24 选型拍板：1.7B Q4 unsloth**；**#419 已合**（Enter 发送 + 状态条型号 + 退役 0.6B）。Checkout 未接。测本地 AI：**Electron 窗口**。Personal Memory 架构已锁；**Slice 0 已排期、未开工**（`task-yin-memory-slice-0-practice-facts.md`）。
- **桌面 · Electron 步骤 B（2026-08-18）**：托盘 + SB-18 已接线，**待 Mac 人工场景 AB**。Safari 场景 B 不得回归。
- **云端品味层（2026-08-18 · #349 已合）**：可选拉取 + 本地降级。Brief `task-cloud-taste-layer.md`。四问见 `ANTI_PLAGIARISM_LAYER.md`。生产 **已本机 wrangler deploy**（Version `5b5b3451-4c35-4d9b-b27b-622b72ed673e`，现网 v1）。Quiet Line overlay **口令已给、运行时未接线**。
...
- **防剽窃层（2026-09-02 方向锁）**：品味云 ∪ YPE 云 ∪ 句包 overlay（Quiet Line #543 · Confide copy 本旁支）。SSOT `ANTI_PLAGIARISM_LAYER.md`。**≠** 支付云 / 备份云 / Confide 路由。
- **云端品味层**（旧称 v1.1 云端算法）：权重覆盖 + 日签/文案池；播放器永远本地；**窄冻结已拍板**；**四问筛选尺 2026-08-20 拍板**（与防剽窃层准入尺同一把）；**schemaVersion 1 可选接线已落地**（**2026-08-20 本机 deploy** 现网 v1 Version `5b5b3451-4c35-4d9b-b27b-622b72ed673e`；见 Backlog「云端品味层」）；Quiet Line overlay **口令已给、运行时下一 PR**；不进 v1.0.0 核心路径、不抢 SemVer `v1.1.0` 号
- **YPE / 云端个人化（2026-08-26 方向锁 · L0/L1 · L2 契约 · Consent 过稿 · 身份键 · 算法契约）**：编排见 `YIN_PERSONALIZATION_ENGINE.md`；**≠** 品味层（同属防剽窃层、不同路由）。L0/L1 全本地。L2 **契约**已合 #454。关即删文案过稿。V1 身份 = 本机随机 profile ID。V1 算法 = 五键→Pack 回声。Worker **ingest 已合源码**。V2 Brief `task-ype-v2-secret-transform.md`。禁止把核心路径绑云；禁止默认上传 Confide 原文
...
- **v1.0 纯本地 / 云端品味层（2026-07-30 已拍板 · 2026-08-18 收窄改名）**：**v1.0.0** 先发纯本地小发布——核心练习路径**不依赖**联网。旧称「v1.1 云端算法」已收窄为 Backlog「云端品味层」（权重 + 文案池；播放器永远本地）。**支付云 ≠ 品味云 ≠ YPE 云**（后两者属防剽窃层，见 `ANTI_PLAGIARISM_LAYER.md`）；功能堆多了 **不**自动打 SemVer `v1.1.0`。代码保留 `cloud/` 可扩展；**禁止**把核心门闩绑死在必须成功的云请求上。隐私仍遵守 `MVP_PRODUCT_DEFINITION`「未来云同步须明示同意」。YPE L2 见 `YIN_PERSONALIZATION_ENGINE.md`。非开放项，留此一行防重复开议题。
...
**最近跟进（2026-08-20 · 桌面陪伴 L1）**：口令「开工桌面陪伴 L1」已执行并 **#362 合入**。Electron 非低配宽屏 ⋯ 同一 Confide 行；下载进度可见；Focusing 卸载；拖窄关生成层。**壳行为不变；Share 路由见 L2 行（安全/情绪桶仍语料）。** **Checkout / 第四卡 Pro / 第五卡 Add-on 仍未接**。测须 `desktop:dev`，勿与 `dev:qa` 抢 5173。权威：`task-desktop-on-device-companion.md`。
...
**最近拍板（2026-09-12 · 远程参数化下一批 · 2026-09-16 §6 已收口）**：① #722 `TASTE_LAYER_KV` · ② #725 莲花阶梯并入花园 KV · ③ Step D = A 寅币冻表不远程。**暂不处理**：MilestoneGlow 7/21/100、徽章 `/3`、芥子 21。**这次不做**：Stretch / 好奇 / 一小时冷却。两案例已齐——通用 overlay helper 可另口令立项（§5）。权威：`REMOTE_PARAM_CANDIDATES.md` §6。
...
**最近拍板（2026-08-20 · 品味层四问 + 下一刀 Quiet Line）**：用户书面同意把「四问筛选尺」写入本 Backlog + Brief；同意现在就生产「部署」（**同日本机已 Redeploy**，现网 v1）；同意部署且三条观感人工过完后，下一刀品味层开 Quiet Line 句包 overlay；日签扩容与伸懒腰/好奇池后排。权威：下文 Backlog「云端品味层」。
...
**最近拍板（2026-09-01 · Web 本地 AI 挂载）**：**暂不立项。**「Web 挂载」= 真实 `?product=1` 壳内向普通用户交付浏览器端本地大模型推理（WebGPU / WebLLM / GGUF 等）；**须 PO 书面翻 flag** 方可重评。**不算** Web ship：Electron 宽屏 companion、`?confide=1` harness。结论：不是「浏览器跑不了」，而是**投入产出比现在还不划算**（缓存/生命周期/移动端/性能仍劣于原生 `node-llama-cpp`）。维持「网页结账、桌面用 AI」。SSOT `LOCAL_AI_WEB_MOUNT_PO_DECISION.md`；与 2026-08-18 桌面窄例外 **不矛盾**。

**最近拍板（2026-08-18）**：**禅意倾听者窄范围例外**——**不是**全面推翻 2026-08-10「检索不生成」。批复原话要点：**仅限桌面端受约束生成、其余场景仍然检索不生成。** 三问：① 修订范围仅 Electron + 最后一层自由倾诉；② 与 Confide **同一入口**；③ **仅用户主动、仅 Idle**，不主动开口。**同日补**：本地智能体 **仅宽屏 ⋯**；窄屏 / 手机没有该能力（`RESPONSIVE_LAYOUT` 原则 A 故意不对等）。禁止拿本次批准扩到 Web / PWA / Whisper / Recover / 提醒 / Arrival / 窄屏抽屉。L0 探针可合；当时 **仍不上 L1 入口**（**2026-08-20 口令后 L1 面板已接线；同日 L2 fallback 已接线**）；仍须内部多轮攒跑偏。权威：`PRODUCT_POSITIONING.md`「禅意倾听者」；Brief `task-desktop-on-device-companion.md`。
...
**最近拍板（2026-09-12）**：**官网 DMG 自动更新器**——挂 Epic #5，不重开 Electron vs Tauri。第一份收费/直销 DMG 出门前必须带检查更新。MVP 用户确认后才重启；失败 Retry / Not now；强制档只预留。Web 芯片（#263）仍只刷新。Brief：`task-briefs/task-electron-desktop-updater.md`。运行时另口令。

**最近拍板（2026-08-12）**：**Web 轻量版本更新提示**——先做「发现新版本 → 点一下刷新」；入口**仅在有新版本时出现**（不常驻）。**不是**补丁包下载；桌面壳真更新器见 `task-electron-desktop-updater.md`。非阻断（Focus/Arrival 不弹窗强刷）；与 Stripe 解耦；不做推送。Brief：`task-briefs/task-web-soft-update-prompt.md`。**实现已合** [#263](https://github.com/IhiroArmstrong/Ihiro-PetGarden001/pull/263)（tip `38edfe0`）；docs-only #261 已关（superseded）。
...
**最近拍板（2026-08-27）**：**From App to Culture** 方向锁入库——文化是探索假设，须用户自发认同后再产品化；**不**锁 The Yin Way / Awakeners 为运动名；Slack = 实验室（另口令接线）；Objects / Identity / Lanterns **先设计验证**。SSOT `FROM_APP_TO_CULTURE.md`。
...
- **防剽窃层**（2026-09-02 方向锁；`ANTI_PLAGIARISM_LAYER.md`）
- **云端品味层**（旧称 v1.1 云端算法；权重 + 文案；播放器永远本地；**#349 已合**；四问 2026-08-20 拍板；**2026-08-20 本机 deploy 现网 v1**；Quiet Line overlay 口令已给、运行时未接线；开机预取错峰未开工）
...
**PR 合入后禁止在同一会话接下一任务。** 旧会话每多一轮，会把历史工具返回与规则前缀全部重算——Usage 里单条 **500万–650万** token 多由此而来。

1. 看到 `Merged` / 「PR 合了」/ `#NNN` 已合 → **立刻新开 Chat**
2. 新会话首条消息末尾附 **模型一行**（见下表）
3. 验收只贴关键字段（如 `data-source`、失败用例名），**勿**粘贴整屏 console / 完整测试输出

| 任务 | 模型一行 |
|---|---|
| Git / PR / QA 同步 / 状态确认 | `Cursor Model: Composer 2.5` |
| 常规实现 / 多文件改代码 / 局部调试 | `Cursor Model: Composer 2.5 / Fast OFF` |
| 跨模块架构 / 疑难推理 / 安全边界 | `Cursor Model: Grok 4.6 / Fast OFF` |

**SSOT**：`focus-tiger-agent-token-cost.mdc` 全文（禁子 Agent、测试输出卫生等）。本节只指路。
...
| Cloud 旁支落本机 | 见 [`WORKFLOW.md`](../../WORKFLOW.md) 并行 worktree 节第 8 款（另开 `…-wt-…`；禁止主仓 Apply / migrated checkout） |
...
0b. **TEST_TRACKER 碎片拼装（强制 · 2026-09-01）**：若 `docs/tracker-entries/` 除 `readme.md` / `_*.md` 外仍有**任意**碎片 → **须**另开独立 `docs/*` PR 跑 `npm run tracker:assemble` 并提交 `TEST_TRACKER.md` 机器块（可与 step 0 同旁支；**禁止**在功能 PR 里拼装）。汇总须写明折入了多少条碎片。索引 `tracker-eod-sync`；SSOT `TEST_TRACKER.md`「拼装触发」P1。  
...
### Backlog:冷启动第一幕分散仲裁审计（11 入口 · 2026-09-06 立项）

- **状态**：approved · 待开工（纯文档审计 + 清单维护；fix 另 PR）。
- **Brief**：`task-briefs/task-cold-start-first-scene-audit.md`
- **清单 SSOT**：`cold-start-first-scene-audit-inventory.md`
- **阻塞**：`task-cold-start-goal-onboarding.md`（冷启动目标问答）解除 blocked 之前须本项收口。
- **权威根因**：`DEV_WORKFLOW_QUALITY.md` §6.17；场景 **AD** / **V**。

### Backlog:冷启动目标问答（2026-09-06 · blocked）

- **状态**：approved · blocked（方向采纳；现在不实现）。
- **Brief**：`task-briefs/task-cold-start-goal-onboarding.md`
- **依赖**：上行「冷启动第一幕审计」PO 拍板 gate SSOT。
- **设计锁**：1 问·4 选项·可跳过·session 级·不污染 Practice Identity。
...
**脚手架规格（#329 / #331 已合 · 步骤 A 窗口代码已提交 · 2026-08-17）**：薄主进程、`contextIsolation`、生产 `dist`、`dmg`（arm64 优先）、壳内不注册 PWA SW。**步骤 A 不带托盘**；**步骤 B（收费 DMG 前）必须有托盘**，并与走神修（场景 AB / SB-18）同一条改动线。步骤 A 的 P0：Stripe `openExternal`、CORS/origin 可见成败、精灵/音频从第一天 `extraResources`。签名/公证需 Apple Developer ID（用户侧，与写窗口并行）。真自动更新器口径：`task-electron-desktop-updater.md`（2026-09-12 已锁 Brief；运行时另口令；挡第一份收费 DMG）。Cloud Linux **不能**验 Mac 窗口 / DMG / 公证。

**适配面**：资源与用户数据路径、原生菜单与窗口生命周期、固定自定义协议 origin（禁止生产 `file://`）；云请求（Checkout / OTP / 备份）在壳内须可见成功或可见错误。

- **状态**：壳 **已拍板 Electron**（#326）；脚手架 Brief 已开（#329/#331）；**步骤 A 窗口代码已提交**。**步骤 B 托盘 + SB-18 已接线**（待 Mac 场景 AB 人工）。**自动更新器 Brief 已锁**（2026-09-12 · `task-electron-desktop-updater.md`；运行时待口令）。
...
### Backlog:防剽窃层（品味云 ∪ YPE 云 ∪ 句包 overlay）

> **拍板（2026-09-02）**：凡有「表/闭包在服务器、客户端只拿 overlay、没网用冻结表」作用的切片，统称防剽窃层。SSOT [`ANTI_PLAGIARISM_LAYER.md`](./ANTI_PLAGIARISM_LAYER.md)。  
...
- **开工**：用户 2026-08-18 书面「L1/L0 可告一段落 → 安排下一步」视为口令「开工云端品味层」。**生产 Worker** 须单独「部署」才从旧 mock 换成 v1 表。**2026-08-20** 用户已书面同意部署；**同日本机 wrangler deploy 成功** Version `5b5b3451-4c35-4d9b-b27b-622b72ed673e`（现网 JSON 含 `schemaVersion: 1`）。
- **四问筛选尺（2026-08-20 拍板）**：见 `ANTI_PLAGIARISM_LAYER.md` §3（与本条同一把尺）。**值得与否**另过 §3.2，禁止把「过了四问」写成「已经值得当秘密」。云端品味层 = 可远程调的 config / master data overlay。**禁止**推广成「只要不影响离线就把全部 master data 上云」。过四问的典型：日签 / 文学句池、Dispatcher 权重。永远本地：Idle / CapCut / 播放器、Sit·Rise 门闩、chrome 文案（语气契约）、权益与金钱。扩池改形须升 `schemaVersion` 并同步改本地兜底。
- **冻表 vs 现网分叉（2026-09-02 拍板）**：仓库冻结表 = **公开够用的离线兜底**（同键、未知 `schemaVersion` 可静默丢）。**允许**已部署 Worker 上的权重 / 日签 / Quiet Line 正文与 git 冻表数字不同。分叉后 **禁止**把现网灵魂数字再提交进 freeze 源文件。现状 schemaVersion 1 仍与冻表相同 → 拷仓库仍等于拷现网，直到下一次「部署」且不把新表写回 git。细则 `ANTI_PLAGIARISM_LAYER.md` §3.1。
- **下一刀（2026-09-03）**：Confide 句库 overlay **本旁支运行时**（生产 Redeploy 另须「部署」）。日签 14→N、伸懒腰 / 好奇池 overlay **后排**。**禁止**把挥手点播绑进品味层（#356）。  
- **后台网络（2026-08-22）**：开机 `prefetchTasteLayer` 错峰是**另一条**任务（`task-taste-layer-boot-prefetch-defer.md`）。Quiet Line / Confide copy 已并进品味层预取；生产是否已含新表以 Redeploy 为准。
