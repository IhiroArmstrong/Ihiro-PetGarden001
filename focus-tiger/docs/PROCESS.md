...
- **节日主题引擎 · Phase 2 已合（2026-08-11 · #234）**：catalog `theme.seasonal.access` + `src/core/seasonal/`；总开关关 / 圣诞节 `contentReady=false`；**无产品 UI**。Phase 1 Brief #233。下一步 Phase 3 须另授权。
...
- **付费 · 节日主题引擎（Seasonal Theme · 2026-08-11）**：**B 轨**；Phase 1+2 已合 develop（#233 / #234）；总开关仍关。下一步 **Phase 3**（圣诞素材人审 + UI 接线 + 翻开关）须另开。权威：`task-briefs/task-seasonal-theme-engine-v1.md`。
