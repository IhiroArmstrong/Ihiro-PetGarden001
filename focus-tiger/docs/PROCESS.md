...
**最后更新时间**：2026-08-20（UTC+8） · Support 将来五卡：第四卡 Pro + 第五卡 Add-on（Price 已记、Checkout 未接、须同批）· Lifetime AI 加购 SKU `companion.addon.lifetime`（方案 A）· #356 珍藏挥手点播 Play · 会话交接规则入库 · #358 清供 8 · #348 Breath 时长点 · #336 L0 已合 · 品味层 **#349 已合**、生产 Worker **未**部署 · 测本地 AI 须 Electron / `desktop:dev`；L1 口令已执行；Checkout 仍须口令
...
- **禅意倾听者 · 桌面陪伴 L1（2026-08-20 · 口令已执行）**：宽屏 Electron 非低配 ⋯ **同一 Confide 行**；点开玻璃卡 + 下载/加载进度；Focusing 卸载。**Share 仍检索**（不上 L2）。**Checkout / 第四卡 Pro / 第五卡 Add-on 仍未接**。低配 / 窄屏 / Web 无生成层。型号未锁。权威：`task-desktop-on-device-companion.md`。
...
- **开工前优先级（2026-08-20 更新）**：#358 / #348 / #336 已合 `origin/develop`。品味层 **#349 已合**；生产 overlay 另须「部署」。挥手点播 Play 在 #356。**下一步** 四页签珍藏壳。桌面陪伴 **L1 面板已接线**（口令「开工桌面陪伴 L1」）；**仍不接第四卡 Pro / 第五卡 Add-on Checkout**（Share 仍检索 = 假收费；接线须两卡同批）。Electron 步骤 B 已接线、待 Mac 场景 AB。
- **陪伴 · 寅币 / Yin's Collections（2026-08-20）**：L0–L3 已合。铁律进 PRINCIPLES。抽屉 = 清供 8。围着阿寅盖序列仍不跟。
- **陪伴 · 桌面端侧窄例外**：**L0 探针 #336 已合**。**L1** 宽屏面板 + 下载 IPC + Focusing 卸载已接线。低配默认不出入口。不锁 0.6B。**不上 L2 生成**。Stripe **Focus Tiger Pro**（将来第四卡）与 **AI Companion Add-on**（将来第五卡）Price 已记；**Checkout 未接**；接线须同批。Pro 含 Base（B 轨）+ 本地智能体，给**非 Lifetime**。Lifetime 用户走 **`companion.addon.lifetime`**（US$29.99 一次）。测本地 AI：**Electron 窗口**（`desktop:dev` / `desktop:companion-l0`），不能用 Safari 打开 QA 5173 代替。
...
**最近跟进（2026-08-20 · 桌面陪伴 L1）**：口令「开工桌面陪伴 L1」已执行。Electron 非低配宽屏 ⋯ 同一 Confide 行；下载进度可见；Focusing 卸载；拖窄关生成层。**Share 仍检索**。**Checkout / 第四卡 Pro / 第五卡 Add-on 仍未接**。测须 `desktop:dev`，勿与 `dev:qa` 抢 5173。权威：`task-desktop-on-device-companion.md`。
