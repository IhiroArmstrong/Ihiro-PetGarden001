...
- **Yin Personalization Engine Architecture V1（2026-08-26 · 方向锁 + L0/L1 + L2 契约 + Consent 过稿 + 身份键 + 算法契约 + V2 签发）**：编排层。核心原则：云端可以让阿寅越来越聪明，但不能让阿寅因为没有网络而消失。**L0/L1** 已开工。L2 **契约**已合 #454。Consent：关即删附录有条件通过。身份：本机随机 `ype_profile_id`（#456）。**V2**：五键闭包 + 白名单 `patternInsights` + KV `algorithmVersion: 2`（不下发 Pack）。现网 Worker 须绑定 `YPE_PERSONALIZATION_KV` 并部署后才签发 V2。属防剽窃层。SSOT `YIN_PERSONALIZATION_ENGINE.md`。
...
- **防剽窃层（2026-09-02 方向锁）**：品味云 ∪ YPE 云 ∪ 排队中的句包 overlay。SSOT `ANTI_PLAGIARISM_LAYER.md`。**≠** 支付云 / 备份云 / Confide 路由。
...
- **YPE / 云端个人化（2026-08-26 方向锁 · L0/L1 · L2 契约 · Consent 过稿 · 身份键 · V2 签发）**：编排见 `YIN_PERSONALIZATION_ENGINE.md`；**≠** 品味层（同属防剽窃层、不同路由）。L0/L1 全本地。L2 **契约**已合 #454。关即删文案过稿。V1 身份 = 本机随机 profile ID。**V2** = 五键闭包 + 白名单 insight + 服务器 `algorithmVersion`（Brief `task-ype-v2-secret-transform.md`）。禁止把核心路径绑云；禁止默认上传 Confide 原文；禁止用完成率改档
...
> **口令队列**：① Quiet Line 句包 overlay（Brief `task-quiet-line-copy-overlay.md` · 运行时下一 PR）→ ② YPE V2 + `algorithmVersion`（`task-ype-v2-secret-transform.md`）→ ③ Confide 句库 overlay（`task-confide-copy-overlay.md`）。日签 14→N、伸懒腰/好奇池 **后排**。
...
- **下一刀（2026-09-02）**：Quiet Line 句包 overlay **口令已给**；运行时 Brief `task-quiet-line-copy-overlay.md`（须答 `BACKGROUND_NETWORK` 三问；禁止与开机预取错峰混 PR）。日签 14→N、伸懒腰 / 好奇池 overlay **后排**。**禁止**把挥手点播绑进品味层（#356）。  
- **后台网络（2026-08-22）**：现网 Quiet Line **未**接线。已接线的开机 `prefetchTasteLayer` 错峰是**另一条**任务（`task-taste-layer-boot-prefetch-defer.md`）。
