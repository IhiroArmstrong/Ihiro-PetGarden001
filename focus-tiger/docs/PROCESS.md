...
- **Five Moments 用户可感表面（2026-08-09）**：用户拍板 **B Compass → A′ Whisper → D′ Journey Log**（Tea Log 模式本地留痕；**非** HealthKit）。不做常驻五点轴 / 教导 Banner。父决策 `task-briefs/task-five-moments-surface-plan.md`；叙事 `PRODUCT_MOMENTS` §5.6。**B 已合 #201**；**A′ 已合 #203**；**D′ 实现中**（`feature/journey-log`）。与 `feature/active-recover-*` 并存时勿抢 Hints/locales 大文件无同步。
