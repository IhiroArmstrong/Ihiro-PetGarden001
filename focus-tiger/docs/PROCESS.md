...
**最后更新时间**：2026-08-18（UTC+8） · 寅币时长 chip 静默 hint · 寅币对外名 + Breath 时长点 · Focus 最短档+默认 **10 分钟**（chip 10/15/25/45）· 同坐点 L2 内部兑换（`applyFocusCoinsRedeem`）· 云端品味层窄冻结已拍板 · 同坐点 L1 **#338 已合**（TRACKER 待人工）· 云端品味层政策锁（#337）· 长周期 QA 播种 `?qaSeedStreak=`（#328）· 禅意倾听者桌面窄例外拍板 · Electron 步骤 A 已提交（无托盘）· 莲花池 Slice A（#330）
...
- **寅币对外名 + Breath 时长点（2026-08-18）**：汉语对外只称 **寅币**（废止「同坐点」）。Breath 坐满按 Stay 档发时长点（5 分=1）并保留每日微仪式 +1；Leave 仍 0。无抽屉、无常驻「攒币」HUD。TRACKER 待人工。
...
- 对外名 **寅币**；只兑身份/情感锦上添花；**禁止**兑 B 轨（仪式 / Deep Ambient / Seasonal / 多端同步 / Enso / 付费章包）。
