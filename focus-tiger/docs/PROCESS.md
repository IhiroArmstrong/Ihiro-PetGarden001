...
**最后更新时间**：2026-08-19（UTC+8） · 寅币 L3 抽屉表面（8 SKU 全列、占位缩略图）· 云端品味层 **schemaVersion 1 可选接线** · Electron 步骤 B 待 Mac 人工 · Focus 默认 10 分 · 同坐点 L1 用户测达标/未达标 · 桌面陪伴 L0 告一段落

**当前技术路线**：主线为 **2D PNG 序列帧动画**（素材来源：图生视频 + 抽帧，见 `ARCHITECTURE.md`）；既有 **3D 多姿态 GLB** 资产与 `PoseManager` / `DynamicMotion` 等代码**完整保留**，改用于未来「奖励系统」塑胶公仔展示，不再作为主界面情绪表现载体。

**近期落地（待人工测试）**：

- **寅币 L3 抽屉表面（2026-08-19）**：宽屏 ⋯ / 窄屏抽屉在 Journey log 旁「寅币 / Yin Coin / 寅コイン」→ `#yin-coin-panel`（Journey 同族玻璃，不是 Support 三卡）。`FOCUS_COIN_CATALOG` **8 条全列可滚动**；占位 24px 圆点缩略图。兑换走既有 L2 `applyFocusCoinsRedeem`；不足显示具体缺口 + toast。`?focusCoins=0` 隐藏该行。无 HUD。TRACKER 待人工。
