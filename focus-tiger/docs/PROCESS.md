...
- **星光斗篷入睡 / 卸斗篷苏醒（2026-08-04 · 入库后 Undo）**：曾入库 `starlight-cloak-sleep` / `-wake` 并接线主线；**同日用户书面**——苏醒不如旧倒放、睡循环无呼吸感不如旧、个别帧抠图不对 → **产品接线 Undo 回 `cloak-sleep`**；星光素材保留**勿接**，待更好抠图再议。拓展时段仍未接线。
...
- **星光斗篷拓展场景（2026-08-04 · 待拍板；主线已 Undo）**：星光素材已入库但**勿接**（抠图/观感未过）。你列的拓展——深夜 Idle 披斗篷、冷启动时段分流、深夜 Rise 入睡、长离回前台苏醒——仍未改状态机；须先有更好抠图再议接线。
