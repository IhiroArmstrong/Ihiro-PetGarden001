...
- **窄屏主屏三主钮图标 v3（2026-07-26）**：用户拍板采用 kraft 米褐圆底 + 深棕图腾（火焰 / 禅圆 / 爱心双手）；`public/icons/icon-{quick-start,sit-with-yin,honesty-checkin}.png` 同名替换；缓存 `?v=4`；Prompt 见 `BUTTON_ICONS_PROMPT.md`。逻辑/门闩不变。
