...
- **YPE / 云端个人化（2026-08-26 方向锁）**：编排见 `YIN_PERSONALIZATION_ENGINE.md`；**≠** 品味层。L2 State Pack **未拍板**；禁止把核心路径绑云；禁止默认上传 Confide 原文
...
- **v1.0 纯本地 / 云端品味层（2026-07-30 已拍板 · 2026-08-18 收窄改名）**：**v1.0.0** 先发纯本地小发布——核心练习路径**不依赖**联网。旧称「v1.1 云端算法」已收窄为 Backlog「云端品味层」（权重 + 文案池；播放器永远本地）。**支付云 ≠ 品味云 ≠ YPE 云**；功能堆多了 **不**自动打 SemVer `v1.1.0`。代码保留 `cloud/` 可扩展；**禁止**把核心门闩绑死在必须成功的云请求上。隐私仍遵守 `MVP_PRODUCT_DEFINITION`「未来云同步须明示同意」。YPE L2 见 `YIN_PERSONALIZATION_ENGINE.md`（**未拍板**）。非开放项，留此一行防重复开议题。
