...
- **陪伴 · 桌面端侧窄例外**：**L0 #336 / L1 #362 已合**。**L2 短生成已接线**（宽屏 fallback）。低配默认不出入口。**2026-08-24 选型拍板：1.7B Q4 unsloth**；**spike 已绿 + M5 hitch 已书面 OK**；**`l0Config.js` 接线 1.7B 进行中**（`feature/companion-l0-config-17b`）。Checkout 未接。测本地 AI：**Electron 窗口**。Personal Memory 架构已锁、无运行时（`YIN_PERSONAL_MEMORY.md`）。
