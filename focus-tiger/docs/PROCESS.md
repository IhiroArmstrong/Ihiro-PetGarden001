...
- **Sanctuary 尊贵徽章素材入库（2026-08-09 · `docs/sanctuary-badges-ingest`）**：17 枚 → `public/ui/support/sanctuary-badges/`（kebab-case）；`ASSET_INVENTORY` + `YIN_SANCTUARY` 已登记。**未接线**授予/UI。Tea（`yin-badges/`）与 Sanctuary **两套视觉**。**下一正式任务**：统一练习徽章体系（免费起 1 / 付费起 3 / 练习上涨自动加枚 / Sanctuary 独立 `badgeIds`）→ Brief `task-briefs/task-unified-practice-badges.md`。
...
- **Five Moments 用户可感表面排期（2026-08-09）**：用户拍板安排 **B Compass → A′ Whisper → D′ Journey Log**（Tea Log 模式本地留痕；**非** HealthKit）。不做常驻五点轴 / 教导 Banner。父决策 `task-briefs/task-five-moments-surface-plan.md`；叙事 `PRODUCT_MOMENTS` §5.6。下一可开工实现 = **B**（`task-five-moments-compass-b.md`）。与 `feature/active-recover-*` 并存时勿抢 Hints/locales 大文件无同步。
