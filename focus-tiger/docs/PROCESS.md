...
- **远程参数化候选 PO 拍板（2026-09-12 · 纯文档 · 无运行时）**：同意审计排序。① 品味权重（含 Honesty 30）进 KV——优先填「伪远程」认知坑；② 莲花阶梯并入现有 `GROWTH_METRICS_KV`；③ 寅币日封顶单独经济 Brief。**这次不做**：Stretch / 好奇概率 / 1h 冷却（评估过，非遗漏）。**暂不搬**：MilestoneGlow 7/21/100、徽章 `/3`、芥子 21。通用 overlay helper 等两个真实案例后再抽。实现须另口令「开工」。权威 `REMOTE_PARAM_CANDIDATES.md` §6。

- **远程参数化候选全项目审计（2026-09-12 · 纯文档 · 无运行时 · #717）**：`REMOTE_PARAM_CANDIDATES.md`。五条门槛扫前端+Worker。品味层 Emotion Weight = 云 API 仍读源码冻表（伪远程）；成长度量日封顶已试点不重复开工。后续顺序见上行拍板。
...
**最近拍板（2026-09-12 · 远程参数化下一批）**：用户书面同意审计排序，并补理由。① 品味权重进 KV（消除「已经在云上」的错觉，改数字仍要发版）；Honesty 30 与权重同域。② 莲花阶梯并入现有花园远程表。③ 寅币日封顶单独经济评估（可兑换、影响稀缺，禁止与 ①② 顺手改）。**暂不处理**：MilestoneGlow 7/21/100、徽章 `/3`、芥子 21（没反复改的证据；芥子真要调再并入 `GROWTH_METRICS_KV`）。**这次不做**（须显式落档）：Stretch / 好奇概率 / 一小时冷却。overlay 小工具：先验证第二个案例（品味 KV）再抽象。权威：`REMOTE_PARAM_CANDIDATES.md` §6。实现须口令「开工」，本拍板不是开工令。
