...
- **YPE / 云端个人化（2026-08-26 方向锁 · L0/L1 · L2 契约 · Consent 过稿 · 身份键 · 算法契约）**：编排见 `YIN_PERSONALIZATION_ENGINE.md`；**≠** 品味层。L0/L1 全本地。L2 **契约**已合 #454。关即删文案过稿。V1 身份 = 本机随机 profile ID。V1 算法 = 五键→Pack 闭包（不按完成率改档）。Worker **未开工**；禁止把核心路径绑云；禁止默认上传 Confide 原文
...
- **v1.0 纯本地 / 云端品味层（2026-07-30 已拍板 · 2026-08-18 收窄改名）**：**v1.0.0** 先发纯本地小发布——核心练习路径**不依赖**联网。旧称「v1.1 云端算法」已收窄为 Backlog「云端品味层」（权重 + 文案池；播放器永远本地）。**支付云 ≠ 品味云 ≠ YPE 云**；功能堆多了 **不**自动打 SemVer `v1.1.0`。代码保留 `cloud/` 可扩展；**禁止**把核心门闩绑死在必须成功的云请求上。隐私仍遵守 `MVP_PRODUCT_DEFINITION`「未来云同步须明示同意」。YPE L2 见 `YIN_PERSONALIZATION_ENGINE.md`（**契约已拍、Consent 附录有条件通过、身份键已拍、算法契约已锁、Worker 未开工**）。非开放项，留此一行防重复开议题。
...
- **四问筛选尺（2026-08-20 拍板）**：云端品味层 = 可远程调的 config / master data overlay（改已接线表的权重或句子，不必等一次完整客户端发版）。**禁止**推广成「只要不影响离线就把全部 master data 上云」。一项要上云须**同时**过四问：
  1. **改错代价**：会不会让用户用不了、错扣/少给钱或点？会 → 不上云。
  2. **点击路径**：是否必须在用户点击当下用到（点击不能等网；产品门禁是 0–1 秒内看见接收反馈）？是 → 不上云。
