...
- **「一分钟呼吸」微仪式 · Idle 接入（2026-07-22）**：`#micro-ritual-idle-entry`（青绿立体 secondary，Sit 上方）→ 60s 吸/呼文案 + smiling@4fps + **光环/Yin 与文案同拍（5s 周期）** → `recordCompletion(1)` + `markToday(1)` + SessionComplete 摆尾 + toast；中途 Leave 安静退出；留存仅 `micro_ritual_complete`。e2e：`e2e/micro-ritual.spec.js`；**同日**：入口由下划线改立体钮、dock gap 16px；**同日晚**：Honesty 桥接时收起入口（勿叠 Yes/No）；**动画节奏待人工**
