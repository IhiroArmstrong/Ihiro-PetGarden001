...
- **状态变更三态可见性 Slice 1（2026-09-18 · 原则 + O-04 三键骨架）**：点击原则承载「持久化三态可见性」；O-04 / M-01 只引用。`failureFeedback` → `mutationFeedback.{pending,success,fail}`；`reminder-preference-saved` 改挂 success。**无**产品 UI。#839 关单 AND 未齐（Slice 2 审计 / Slice 3 中间件 / 跨模块回归仍缺）。
