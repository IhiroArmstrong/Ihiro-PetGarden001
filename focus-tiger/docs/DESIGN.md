...
唤醒起身 → Honesty Check-in 呼吸引导结束后的过渡动作
          （伸懒腰 + 对接既有金色 Rim Light / 光环系统；见下方唤醒仪式）
```
...
2. 选择后，进入约 **10 秒**的呼吸引导（简单视觉呼吸节奏即可；可复用 `DynamicMotion` 呼吸起伏思路，不强制新素材）；
3. 呼吸引导结束后，触发老虎「唤醒」反馈：
   - 伸懒腰动作（情绪键对齐 `WakeUp` / 对应 2D 素材就绪后接入）；
   - **金光 / Rim Light 激活**：对接既有视觉系统——老虎本体固有色不变，金色环境光反射随进度增强；**禁止**另起一套独立光效逻辑；
