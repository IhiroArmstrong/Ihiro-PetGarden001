...
**「本周陪伴」7 格热力图（2026-07-22；窄屏挂点 2026-07-24）**：仅 **Idle** 可见。**宽屏**：左下角（`#onboarding-hint-help` 上方）。**窄屏（≤479px）**：改挂 FocusHUD（Calm / Today's shared sitting）**下方**，避免盖住居中 dock。数据 `PracticeDaysStore.getLastNDays(7)`；亮格 = `totalMinutes === null \|\| totalMinutes > 0`（`--color-accent` 软透明）；暗格 = `--color-ink-faint` 浅洗，**无**红/警示/感叹号/文案/点击。非 Idle 隐藏。`#weekly-practice-heatmap` / `#weekly-practice-heatmap-cluster`。
