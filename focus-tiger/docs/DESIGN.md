...
- **计时提示音（免费 · 2026-08-12 / 间隔节奏 2026-08-13）**：Focus **开表**开始磬、**达标**结束铃（Soundscape「计时提示音」总开关，默认开）。**正念磬声间隔**独立三档：无（默认纯净陪伴）/ 每 3 分 / 每 5 分；第一声在 3:00 或 5:00（开表那一瞬不播间隔磬）。产品 Focus 最短 **15 分钟**，故正式档总会听到间隔（调试 `?sessionMinutes=` 短会话除外）；资产 `session-interval-bell.mp3`（短 one-shot，**不**接 Ambient Gate 长循环）；剩余 &lt; **30s** 跳过；氛围可闻时 duck→unduck。面板须**可见**写出各开关含义（勿只靠 `title` 悬停）。
