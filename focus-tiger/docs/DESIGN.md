...
状态四・打瞌睡(DORMANT；2026-07-21 起零完成开场不再自动进入；2026-07-26 冷启动亦不进睡)
  ...
  惰性进睡：距最近一次专注结束 ≥ `DORMANT_IDLE_HOURS`（默认 2h）时，在**回前台 / Rise 后 live sync** 进入 DORMANT 并播披毯（**2026-07-26 拍板**：回前台 ≥2h 继续披毯进睡，不改为 Idle）；调试面板仍可显式切入
...
- **计时提示音（免费 · 2026-08-12 / 间隔 2026-08-13）**：Focus **开表**一声开始磬、进行中每 **180s** 一声间隔磬（剩余 &lt; **30s** 跳过）、**达标**一声结束铃；资产 `public/audio/cues/`（非 ambient 曲库）；**不**走 Ambient entitlement。Soundscape 音量条下「计时提示音」总开关（默认开；`focus-tiger.session-cues.v1` 三字段同步）。开着氛围时开始/间隔铃：duck → 播 → ~1.5s unduck；chip/boot 预加载；
- **觉察观照卡（mid-session · 2026-08-13）**：与间隔磬同拍轻量短句（`FOCUS_AWARENESS_*`）；可每场多次；**不**写入 Moment Whisper 一生一次旗；总开关关则卡也不出；busy 叠层可压住卡、默认仍播磬；
...
| SLEEPING | 距最近一次专注结束 ≥ `DORMANT_IDLE_HOURS`（默认 2h，滚动窗口）且经 **live** `syncDormantState`（回前台 / Rise 后）进入 DORMANT；播 `cloak-sleep` 过渡。**冷启动 `onAppReady` 不进睡**（第一幕 Idle） | 叠加打呼噜 ZZZ 图标漂浮效果,rotation.x 微倾 |
