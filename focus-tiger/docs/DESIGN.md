...
**用户上传氛围乐（2026-07-31 正式重开 · 仅 Backlog，未立项实现）**：推翻此前「不做用户上传音乐」。产品意向——用户可上传多首自己喜欢的氛围乐；上传曲进入 Soundscape 清单且**排在内置曲之前**（用户曲之间可按上传顺序或最近优先，排期时拍板）；用户**仅可删除自己上传的曲**，不得删内置曲。存储与格式边界、容量上限等排期开 Task 时再定。权威排期入口：`PROCESS.md` Backlog「用户上传氛围乐」。
...
**「本周陪伴」7 格热力图（2026-07-22；窄屏壳 2026-07-24；主 CTA 上屏 2026-07-26）**：仅 **Idle** 可见。**宽屏**：左下角（`#onboarding-hint-help` 上方）。**窄屏（≤479px）**：`NarrowIdleShell` ActionBar + 主画布三 PNG 图腾圆球（顺序 **Quick Start · Sit with Yin · Honesty**；全宽 `space-evenly`；约 72px；Arrival 期仅留 Quick Start）+ 上滑抽屉（次要项）；7 格在抽屉内只读展示。**宽屏主 CTA（产品拍板 2026-07-31 · 实现待开）**：首页亦用同序三球，代替现行 Sit+⚡ 文案 pill；次要仍 ⋯——见 `task-briefs/task-wide-home-three-ball.md`（落地前运行时仍为 Sit+⚡+⋯）。数据 `PracticeDaysStore.getLastNDays(7)`；亮格 = `totalMinutes === null \|\| totalMinutes > 0`；暗格浅洗。非 Idle 隐藏。`#weekly-practice-heatmap` / `#ft-narrow-idle-shell` / `#ft-narrow-home-ctas`。
