...
**产品壳 FocusHUD（2026-07-21）**：左上角改为「香炉刻度」隐喻——碗身 + 轻烟高度/金环填充跟 `focusLevel`；时长默认半透明，专注中或悬停才加重；百分比仅悬停/键盘 focus 露出。禁止常驻 `Status: / Focus: N%` 计分牌文案。回归：`FocusHUD.test.js` + e2e `#hud-state` / `#hud-time`。
