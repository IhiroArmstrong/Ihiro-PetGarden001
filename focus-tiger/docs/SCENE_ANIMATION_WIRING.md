...
| 当日首次冷启动问候 | 加权池：`welcomeBack` ~60% · `nodGreeting` ~40% | ack | **已实现**（Dispatcher 同日 1 次） | 靠近自动仍 **勿接**。`welcomeBack`：`waveHelloWelcome` **正+倒一次** + ~1s CapCut Idle（禁 player pingpong；2026-08-02c 修 hide→跳过叠化）；`nodGreeting`：正放一次即可 |
