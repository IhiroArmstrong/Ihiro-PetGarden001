...
| 语言切换 → **English**（及日后其它 ready） | `teaDrinking`（单程喝茶、**无倒放**；末约 **1s CapCut** Idle） | ack | **Slice A · 已实现**（2026-08-02：告别 nod-bow；书硬切 OK 后改茶） | 同上限频；深夜池同素材亦用茶 |
...
> **A′ 已落地**：切语 ja → `palmsTogether`（与 Arrival Choose 的 `intentionSet`/nod 解耦）。**2026-08-02**：切语 en 曾试 `magicBookReading` 硬切（QA OK）→ 再改 `teaDrinking` 单程 + CapCut。
...
| 2026-08-02 | 切语 EN：曾 `magicBookReading` 硬切（QA OK）→ 改 `teaDrinking` 单程 + CapCut |
