...
| Day1 / 久别（≥3 日）吹花鼓励 | `conjureFlowersBlowAway` + 观察式气泡 | ack | **Phase 1 Lab 已入库**；产品**未接线** | **策略 C**（2026-08-05）：仅 Day1 / 久别强制吹花；普通隔天仍走上行欢迎池。**同日 XOR**：吹花与 `WELCOME_APP` 池（含 `magicBookReading`）二选一。DORMANT 优先跳过吹花。SSOT：`FLOWER_BLOW_WELCOME_DESIGN.md`。调试：`#emotion-debug-ui`「变花吹散(Lab·未接线)」 |
