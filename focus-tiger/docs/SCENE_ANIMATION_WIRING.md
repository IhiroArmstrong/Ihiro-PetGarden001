...
| 当日首次冷启动问候 | 加权池：`welcomeBack` ~60% · `nodGreeting` ~40% | ack | **已实现**（Dispatcher 同日 1 次） | 靠近自动仍 **勿接**。`welcomeBack`：wave **pingpong×1** 再叠 Idle（2026-08-01）；`nodGreeting`：正放一次即可（末帧坐姿≈Idle，差在睁闭眼） |
