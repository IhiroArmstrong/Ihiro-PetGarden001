...
| 当日首次冷启动问候 | 加权池：`welcomeBack` ~60% · `nodGreeting` ~40% | ack | **已实现**（Dispatcher 同日 1 次） | 靠近自动仍 **勿接**。`welcomeBack`：`waveHello` **正放至坐姿** + ~1s CapCut Idle（2026-08-02b；禁倒放/player pingpong——001=抬手）；`nodGreeting`：正放一次即可 |
