...
| **代码** | 情绪键 / 触发器 / 提醒限频池 **继续分开**；主动侧走 `triggerActiveRecover()`，**不占** Re-focus 提醒池；180s 冷却；冷却再点走 `acknowledgeActiveRecoverCooldownTap()`（微点头、无 toast） | 同左：独立键与触发，**不占** Re-focus 提醒池 |
