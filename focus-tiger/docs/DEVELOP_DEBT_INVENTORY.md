...
**人工走查步骤 SSOT**：[`KNOWN_RISKY_TEST_CHECKLIST.md`](./KNOWN_RISKY_TEST_CHECKLIST.md)（**2026-08-08 已刷新**：纯 tip 一批验收前置；#27 Support Modal；废 Support-only worktree 口径；§0.1 优先序）。同目录 `known-risky-test-checklist.csv` **不权威**（勿对照验收）。仓库根 KnownRisky `.numbers` 已于 2026-08-05 删除。
