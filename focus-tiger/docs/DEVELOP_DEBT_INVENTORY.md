...
| Honesty 桥接 CTA | known-risky | **2026-08-04 375 回归**：桥接 Yes/No 被窄屏三球挡住；Yin 过小。本地修（`bridgeVisible`→narrow suppress；zoom 1.28）。曾关单 tip `a76178f`。须 375 复测叠层/半透明。 | `HonestyBridgeCta*` · `idleChromeOrchestration` · e2e `375 bridge…` · `KNOWN_RISKY` #3 | 375 复测后可回 verified |
