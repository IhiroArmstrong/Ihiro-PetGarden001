...
| 「本周陪伴」热力图 UI | known-risky | **2026-08-04 走查**：步1–3 OK；步4 变亮 OK、**今日标记缺口**已开 `fix/weekly-heatmap-today-marker`；步5 weekly tip **不行**→ Hints 再设计。场景 O 曾大段 OK；e2e 锁壳与 seed。 | `WeeklyPracticeHeatmap*` · `PracticeDaysStore` · e2e `weekly-practice-heatmap` | 合入今日标记 → Hints 再设计（勿单点 tip） |
