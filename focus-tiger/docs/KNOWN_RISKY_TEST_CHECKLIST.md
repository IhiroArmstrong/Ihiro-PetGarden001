...
| 壳 = Electron（Mac DMG 已拍板） | **v1 默认纯 Web**；电脑版壳 **Electron**（2026-08-16）；**可选 PWA 基础层**已合 `develop`（#180：manifest + network-only SW + 品牌图标）；**安装验收排期 = PR #2→`main` + 稳定版后**（勿现在催测；#188）。Electron 脚手架未开，见 Brief `task-desktop-shell-electron.md` |
