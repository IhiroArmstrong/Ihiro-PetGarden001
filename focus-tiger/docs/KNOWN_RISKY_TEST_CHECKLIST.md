...
| 3 | Honesty 桥接 CTA | verified | 【前置】产品壳；先完成一次 Honesty 补登进入桥接。<br><br>【主路径】<br>1) 补登结束立刻出现桥接：顶行 thanks +「要不要现在也坐一会儿？」Yes/No。 【***测试OK】<br>2) 观感：气泡应为暖米半透明（约能看见阿寅下半身/蒲团），不得近乎不透明大白板挡角色。 【***测试OK】<br>3) 桥接可见时：Honesty Check-in /「一分钟呼吸」入口须隐藏，不得叠在 Yes/No 上。 【***测试OK】<br>4) 点 Yes → 完整 Arrival → Companion（不要直接开表）。 【***测试OK】<br>5) 另一次：点 No → 回 Idle，两入口恢复。 【***测试OK】<br><br>【回流 + 375】<br>6) 同日再补登应再出桥接。 【***测试OK】<br>7) 375×667：toast 与桥接同屏不互挡到不可读。 【***测试OK】<br><br>【通过标准】叠层/透明度/入口 suppress 都对；Yes/No 语义正确。 【***测试OK】<br><br>**2026-08-04 回归（用户图）**：375 Honesty 后桥接 Yes/No 被主画布三球挡住；半透明被盖住难辨；窄屏 Yin 过小。修：narrow bridge full-suppress + Yin zoom 1.28。**须 375 复测步 2–3、7**。<br>**2026-08-04 复测关单（375 回归修）**：用户书面 fix `:5189` · commit **`04b7de3`**（rebase 自 `759763e`）：步 2–3、三球/grabber suppress、半透明、No 回流、Idle Yin 放大 — **全部测试 OK**。合入 develop tip **`3ea79b9`**（PR #118）。<br>**2026-08-04 关单**：用户称「第2行」=本条步骤（完整记录）；tip **`a76178f`**（`:5176` · behind=0）。「时长→呼吸→toast→?」属清单 **#2**，非本行漏测。→ `TEST_TRACKER` 桥接行 **已通过**。 | TEST_TRACKER 桥接行记 375 OK（合 tip 后关「已通过」）；KnownRisky #3 → **verified**。 | 改桥接/叠层时复测 |
...
| 5 | 「本周陪伴」热力图 UI | known-risky | 【前置】产品壳 Idle。<br><br>【主路径】<br>1) Idle 见「本周陪伴」7 格热力图；**每格下方有星期缩写**；**今日格有软描边**（最右 = 今日）。【***测试OK】（2026-08-04 · Numbers 第7行 / 清单#5；今日标记合入后口径）<br>2) 宽屏：通常在左下「?」上方；窄屏：在抽屉/ActionBar 相关挂点，勿只剩看不见。【***测试OK】<br>3) 非 Idle（Focusing）：热力图应隐藏或按契约 park（不得挡 HUD）。【***测试OK】<br><br>【真数据】<br>4) 完成一场短计时（演示 1 分钟）或 Honesty 补登后回 Idle：今日格（描边那格）应变亮（或保持亮）。【变亮 OK】；今日标记修已合 `origin/develop` tip **`dc415d7`**（PR #120）。**2026-08-05 用户书面**：fix 分支已手工验收，**免 tip 复验**（不强制再测）。TRACKER 今日标记行仍「待人工测试」——仅因关单规矩认 tip hash，**非**再邀测。<br><br>【Hint】<br>5) 清空 hints 已读后，点「?」：weekly-heatmap tip 应立刻出现且尖角靠近热力图（不得等「More tips」才出、不得指空）。**【测试不行】**；用户书面：Hints 问题很多，**需要再设计**（清单 #7；交其他 Agent）。<br><br>【通过标准】能一眼认出今天；亮暗合理；Focusing 不挡；tip 几何待 Hints 再设计。 | **2026-08-04 走查**：步1–3 OK；步4 变亮 OK + 今日标记已合 PR #120（`dc415d7`）；步5 Hint **不行**→再设计。用户免 tip 复验。 | Hints 再设计批次（其他 Agent）；今日标记子项不再排复验 |
...
| 7 | Onboarding Hints · 脉冲悬停 + ? 简介（原 ? 补救） | known-risky | 【前置】产品壳；实验室可「清空引导提示已读」。<br>**2026-08-04 产品面收窄**：只验 (1) 脉冲点悬停 tip、移开即没；(2) 点/悬停「?」只出产品简介、无本页 tip 喷洒。下列旧「? 补救铺开」步骤作废。<br><br>【脉冲悬停】<br>1) 冷 Idle：薄荷绿 click 圆点（如音符 / ⋯ 行）悬停见 tip，移开即没；peek → 静止弱化 → 操作后 done。<br><br>【? 简介】<br>2) 点/悬停左下「?」：只出 `#onboarding-app-purpose`，**禁止**本页 tip 喷洒 / More tips。<br>3) （旧）点 tip 气泡本身 → 立刻消失——仅脉冲 tip 路径仍适用。<br>4) （旧）weekly tip 尖角几何——**2026-08-04 KnownRisky #5 步5**【测试不行】；产品方向再设计，勿单点硬修。<br><br>【回流】<br>5) Rise 后再 Focusing 点「?」仍只出简介。<br>6) Lit 薄荷绿观感可接受（护栏试点 ≠ 替代本条人工观感）。<br><br>【通过标准】脉冲悬停 + ? 简介不乱；文案可读。 | 有问题：历史 ? 补救尖角/park；**2026-08-04 产品方向**：整体再设计（运行时已收窄）。部分覆盖：HINTS_WIRING 簇 A；④ 视觉护栏试点（PR #93）；观察持有（PR #95）。 | **产品再设计批次**（暂缓单点 tip 几何）；扩护栏前先读 HINTS_WIRING || 8 | 冷启动「开场即睡」vs live DORMANT | known-risky | 【前置】需要本地有「≥2h 前」的 focus-session-end 戳。做法示例：先完成一场会话，用 DevTools/实验室改存储时间戳，或按 TEST_TRACKER「开场即睡」行说明准备。<br><br>【冷启动】<br>1) 硬刷新 / 重新打开 `?product=1`：阿寅须 Idle 闭目坐禅（有精神），**不得**立刻披斗篷睡着。<br>2) Honesty 小钮仍可出现（零完成时）。<br><br>【live DORMANT · 对照】<br>3) 保持 ≥2h 条件，把页签切到后台再回前台：应允许披毯进睡（live 路径 ≠ 冷启动）。<br><br>【通过标准】冷启动永不「开场即睡」；回前台 live 路径仍可睡。 | 有问题行仍开；历史「修好又失效」（DEV_WORKFLOW_QUALITY §6.7）。契约：onAppReady 禁进睡 / 回前台≥2h 仍披毯——双路径易回归。**2026-08-05**：清空本地后短暂切 tab **无误睡**＝基线污染对照，非新产品回归。 | 走查双路径 → 保持失败用例 |
...
| 10 | completionPending 时 Sit 静默 return | verified | 【说明】完成反馈窗口点 Sit 须不可点。<br><br>【步骤】<br>1) 注入或走完达标：`__sessionUiGate.setCompletionPending(true)` + `__resyncSessionChrome()`。<br>2) `#btn-focus` 须 **disabled** / `aria-disabled=true`。<br>3) 清 pending 后再可点。<br><br>【通过标准】无「点了没反应」。 | **2026-08-05 批 4 已修**：`shouldEnableFocusChromeButton` + resync；e2e `completionPending disables Sit`。 | 改完成反馈时复测 |
| 11 | Visibility 契约 gap-* 行 | verified | 【说明】原四条 gap 已收 locked（2026-08-05）。抽查宽窄 Focusing HUD / Honesty panel / heatmap / Companion 鞠躬即可。<br><br>【通过标准】`listVisibilityLockGaps()` 空；改 suppress 跑 visibility e2e。 | 批 4 收口；SHARED §6 机器块已 sync。 | 改壳时跑 `test:e2e:visibility` |
...
| 15 | Emotion / playEmotion 返回值常忽略 | assumed-ok | 【说明】批 4 已补 hold key SSOT + oneshot `!started` warn。非产品走查主项。<br><br>【若抽查】实验室连播强情绪/hold 勿被弱情绪打断。<br><br>【通过标准】可标「工程观察已部分收口」。 | `DEBUG_HOLD_POSE_EMOTION_KEYS`；EDGE #17–19 部分。 | 新情绪漏登记再升级 |
