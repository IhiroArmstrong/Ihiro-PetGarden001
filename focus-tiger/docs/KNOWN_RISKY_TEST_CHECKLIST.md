...
| **P1** | **25** Tip Jar **任务 5 部署** | 代码已合 ≠ 真收款；Stripe Price / KV / secrets / webhook / `VITE_CLOUD_API_BASE_URL` 整条未做 |
...
| 壳 = Electron/Tauri/PWA 三选一即将定 | **v1 默认纯 Web**；桌面壳**仍开放**；**可选 PWA 基础层已立项**（`feature/pwa-basics`：manifest + network-only SW；图标待补，不可验收完成）。桌面壳选型仍开放，见技术方向 Brief |
...
| 25 | Buy Yin a Tea / Tip Jar（A） | known-risky | 【UI】⋯/抽屉见 **Buy Yin a tea** → `#yin-tip-jar-card`；关卡回流；**不**解锁 Sanctuary/氛围。<br>【**任务 5 · 部署 · 未完成**】代码 alone ≠ 真实收款。按 `YIN_TIP_JAR.md` § 部署：Stripe Test Price → `STRIPE_PRICE_ID`；`wrangler kv namespace create TIP_KV` 换占位 id；`secret put STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET`；`wrangler deploy`；配 Webhook；前端 `VITE_CLOUD_API_BASE_URL`。<br>【部署后验收】Test 卡 Checkout → `?tip=1` 回跳；邮箱 verify-tip；零耦合抽查。 | PR #161 UI/路由已合；**部署清单仍开**（原 Founder Pack §6） | **排期任务 5**；UI 可先测；真收款另环境 |
...
| `task-briefs/task-tech-direction-v1-shell-monetization.md` | v1 纯 Web / 双轨付费 / **可选 PWA 基础层已立项**（图标待补） |
