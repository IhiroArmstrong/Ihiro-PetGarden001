...
| 25 | Buy Yin a Tea / Tip Jar（A） | known-risky | **逐步清单**：`docs/PAYMENT_MANUAL_TEST_CHECKLIST.md` **§A**。<br>【A1】⋯/Support → Tea → $9.99 → 回跳喝茶致谢。<br>【A2】**未测**：删 `tip-jar.v1` → 卡内邮箱 Restore。<br>【A3】**未测**：Tip 后 Sanctuary 仍未解锁；Rituals 仍锁。<br>Webhook tip 200（2026-08-11）≠ 整行关单。 | checklist §A · TRACKER Tip 行 | **优先测 A2+A3** |
| 26 | Yin's Sanctuary Lifetime（B） | known-risky | **逐步清单**：checklist **§B**。<br>【B1】Unlock → $89.99 → confirm + 鞠躬。<br>【B2】**未测**：删 `sanctuary-entitlement.v1` → Restore unlock。<br>【B3】**未测**：不与 Tip 互读。<br>Ambient 消费仍暂缓。 | checklist §B | **优先测 B2+B3** |
