...
| 4 | Ambient Soundscape + 右上音符静音 | known-risky | 【前置】产品壳；重置后默认应无音乐（opt-in）。<br><br>【宽屏】<br>1) 右上音符：关=可点开播；**桌面悬停**即可开清单（不静音）；有声且清单已开时再点=静音（音符+斜杠观感）。<br>2) 选曲后有声 → 点音符静音 → 再点音符：应续播同一曲（同一次手势续播口径）。<br>3) 面板显式 Off：再开不应自动续播。<br><br>【窄屏 375】<br>4) ActionBar ♪ 可开 Soundscape；抽屉打开时点 ♪ 应能开面板（勿被抽屉挡住）。<br><br>【Focusing / Rise】<br>5) Sit 开计时后仍可开 Soundscape **换曲且音乐不中断**（宽屏：悬停开清单；有声+面板关时点音符只开清单不静音）。<br>6) Rise 或达标结束 → 音乐须自动停；面板收起。<br><br>【回流】<br>7) 刷新后仍尊重静音/关偏好；再 Sit 后可按偏好再开。<br><br>【通过标准】停音/续播/Rise 停播符合上列；听感主观 OK 即可。 | 有问题多行（L311/L315/L414）：Rise 停音、续播手势、窄屏抽屉挡 ♪ 历史红。COVERAGE_GAP：听感/行为几乎无 DOM 锁。部分覆盖：已有 ambient-mute-resume-focusing e2e；**2026-08-04**：悬停开清单 + 换曲不中断（本地 `fix/soundscape-first-open-off-icon-size`）。停音/续播全矩阵与听感仍未关单——排期勿与「从零排查 Ambient」重复。 | 补测试（行为契约）+ 走查；对照已有 mute e2e |
