...
| 4 | Ambient Soundscape + 右上音符静音 | known-risky | 【前置】产品壳；重置后默认应无音乐（opt-in）。<br><br>【宽屏】<br>1) 右上音符：关=可点开播；**桌面悬停**即可开清单（不静音）；有声且清单已开时再点=静音（音符+斜杠观感）。 【***测试OK】（2026-08-04）<br>2) 选曲后有声 → 点音符静音 → 再点音符：应续播同一曲（同一次手势续播口径）。 【***测试OK】（2026-08-04）<br>3) 面板显式 Off：再开不应自动续播。 【***测试OK】（2026-08-04）<br><br>【窄屏 375】<br>4) ActionBar ♪：**鼠标悬停**即可开 Soundscape（无需点击）；**抽屉打开时悬停 ♪** 亦须能开面板（勿被抽屉挡住；悬停应先收抽屉再出清单）。<br><br>【Focusing / Rise】<br>5) Sit 开计时后：**悬停**音乐按钮即可开清单换曲（音乐不中断）。<br>6) Rise 或达标结束 → 音乐须自动停；面板收起。 【***测试OK】（2026-08-04）<br><br>【回流】<br>7) 刷新后仍尊重静音/关偏好；再 Sit 后可按偏好再开。 【***测试OK】（2026-08-04）<br><br>【通过标准】停音/续播/Rise 停播符合上列；听感主观 OK 即可。步 4/5 悬停契约须再验（本分支）。 | **2026-08-04 用户书面**（清单第 4 条 / 用户称第 6 行）：步 1–3、6–7 ***测试OK；步 4/5 要求改为「悬停开清单」且抽屉打开时悬停 ♪ 不被挡。本地 `fix/soundscape-first-open-off-icon-size`：悬停开清单 + 真鼠标绕过 375 设备模式 `(hover:none)`；抽屉悬停 e2e。未 push 前勿用 develop tip 关单。 | 复测步 4/5 悬停（本分支）→ 再议关单 |
