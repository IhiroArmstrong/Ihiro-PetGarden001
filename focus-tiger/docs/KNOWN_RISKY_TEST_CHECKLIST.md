...
| 18 | Arrival CapCut 抗闪（Notice→Breath / Choose→鞠躬） | known-risky · **P0** | 【主路径】Sit → Notice 任选 → 切入 Breath 微笑 **约 1s 叠化、无闪白** → Choose 任选 → 鞠躬切入与回落 **无闪白**。<br>【回流】Rise 后再走一遍 Arrival。<br>【分列】Notice 后眨眼；Choose→鞠躬；鞠躬→Idle/Companion —— 三条分开记。<br>【对照】`?tasteLayer=0` 与 overlay 开/关各走一遍。 | **release-blocker** recorded=2026-08-06；#150 + #386 已合 develop（`ef80d3cb`）；**禁止再开代码 PR** → develop tip **人工关单**。 | **优先人工复测** → TRACKER 关单 |
...
| 21 | 吹花欢迎冷启动（Phase 2b/2c） | known-risky | 【Day1】DEV：`__ftDebug.resetScenario('day1-flower-card')` → 硬刷新 → 吹花+头顶白玉气泡（含深夜——**压过** wellness 斗篷）。<br>【同日再刷】不得再吹花/书/点头欢迎。<br>【负例】`resetScenario('welcome-quota-blocks-flower')` → 不得再吹花。<br>【久别 ≥3 日】再吹花；文案尽量不连同句（2c）。<br>【375】气泡完整在 ActionBar **下方**。<br>【`?flowerWelcome=0`】永不吹花。 | Phase 1–2c 已合；多轮 feature tip OK；关单级 develop tip 分列仍开 | tip 分列关单 |
...
| 27 | Support Yin 统一入口（Modal） | known-risky · **P1** | 【主路径】Idle → 右上 **Support Yin** `#yin-support-fab` → `#yin-support-modal` **三卡始终可见可点**。<br>【未练习】清 `practice-days.v1` + `lotus-pond.v1`（勿 `?qaSeedStreak=` / `?qaLotusBlooms=`）→ 顺序 **Tea → Sanctuary → Membership**（375 最上请茶）；**Suggested** 在 Tea（`yin-support-suggested-badge` `data-host=tea`）。<br>【已练习】Focus 达标 / Honesty / Breath 任一完成 → **永久** Sanctuary → Membership → Tea；Suggested 回 Sanctuary。未达标 Rise **不得**切排序。<br>【文案】Maybe later；价 $89.99 / $6.99/月 / $4.99 不变；Join Membership 米色 CTA。<br>【回流】关后再开；Focusing 时 FAB 隐藏。<br>【对照】场景化请茶气泡不改；菜单付费三项仍走 Support FAB。<br>【375】三卡上下堆叠。<br>【通过标准】未练习请茶打头 + 已练习恢复现状；**多档 tip** **未做**。 | TRACKER Support 行 · `supportModalLead.test.js` | tip 走查 → TRACKER 关单 |
