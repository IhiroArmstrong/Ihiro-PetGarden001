...
| 5 | 「本周陪伴」热力图 UI | known-risky | 【前置】产品壳 Idle。<br><br>【主路径】<br>1) Idle 见「本周陪伴」7 格热力图；**每格下方有星期缩写**；**今日格有软描边**（最右 = 今日）。不得再是无提示裸格。【2026-08-04 用户书面：原先毫无今日提示 → 不对】<br>2) 宽屏：通常在左下「?」上方；窄屏：在抽屉/ActionBar 相关挂点，勿只剩看不见；抽屉内克隆条同样有星期+今日描边。<br>3) 非 Idle（Focusing）：热力图应隐藏或按契约 park（不得挡 HUD）。<br><br>【真数据】<br>4) 完成一场短计时（演示 1 分钟）或 Honesty 补登后回 Idle：**今日格**（描边那格）应变亮（或保持亮）。<br><br>【Hint】<br>5) 清空 hints 已读后，点「?」：weekly-heatmap tip 应立刻出现且尖角靠近热力图（不得等「More tips」才出、不得指空）；文案应提到描边格是今天。<br><br>【通过标准】能一眼认出今天；亮暗合理；Focusing 不挡；tip 几何可接受。 | 有问题：场景 O 曾大段 OK；2026-08-04 反馈「看不出哪格是今天」→ 已开 `fix/weekly-heatmap-today-marker`（星期+今日描边）；真练习变亮 + tip 尖角仍脆。 | 合入后走查今日标记观感 → tip 几何 |
