...
| 品味层 runtime | **已接线**（`schemaVersion: 1` 可选 overlay；失败用本地冻结表）。**2026-08-20 本机 wrangler deploy** 生产 Version **`5b5b3451-4c35-4d9b-b27b-622b72ed673e`**；现网 `/api/emotion-weight` 含 `schemaVersion: 1`。Safari `__tasteLayer.status()` 已见 weights/dailyWisdom true。现有 Playwright **不**需要品味层 `secrets.*` |
