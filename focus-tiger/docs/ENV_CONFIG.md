...
| Focus Tiger Pro Stripe Price（2026-08-20） | **已记、Checkout 未接。** Dashboard **Focus Tiger Pro** US$12.99/月 · Price **`price_1U6EB1FuIhgJPGLiuciuX1to`**（**已锁定**）。给**非 Lifetime** 用户（含 B 轨 + 本地智能体）。**禁止**写入任何 `VITE_*`。Worker **尚未**消费该 ID（无 Pro checkout 路由；**不要**把 ID 放进 `wrangler.jsonc` `vars` 当真接线）。模板注释：`cloud/.env.example`。权威：`task-desktop-on-device-companion.md` |
| Lifetime AI 加购 SKU（2026-08-20） | **Price 已记、Checkout 未接。** Dashboard **Focus Tiger: AI Companion Add-on** US$29.99 一次 · Price **`price_1U6GnXFuIhgJPGLiNlXs0IKe`**。SKU **`companion.addon.lifetime`**。**禁止**写入任何 `VITE_*`、**禁止**进 `FEATURE_CATALOG`。Worker **尚未**消费该 ID（无 checkout 路由；**不要**把 ID 放进 `wrangler.jsonc` `vars` 当真接线）。模板注释：`cloud/.env.example`。权威：`FREE_PAID_MATRIX` A6 |
