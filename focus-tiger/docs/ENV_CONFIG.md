...
| Focus Tiger Pro Stripe Price（2026-08-20） | **已记、Checkout 未接。** Dashboard **Focus Tiger Pro** US$12.99/月 · Price **`price_1U6EB1FuIhgJPGLiuciuX1to`**。Lifetime 用户要本地 AI → 另订该档（不必再买 Base）。**禁止**写入任何 `VITE_*`。Worker **尚未**消费该 ID（无 Pro checkout 路由；**不要**把 ID 放进 `wrangler.jsonc` `vars` 当真接线）。模板注释：`cloud/.env.example`。权威：`task-desktop-on-device-companion.md` |
