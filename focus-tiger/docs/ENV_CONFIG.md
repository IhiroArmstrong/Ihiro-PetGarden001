...
| Lifetime AI 加购 SKU（2026-08-20） | **政策已锁、Checkout 未接、Stripe Price 未建。** SKU **`companion.addon.lifetime`** · 政策价 US$29.99 一次。**禁止**写入任何 `VITE_*`、**禁止**进 `FEATURE_CATALOG`。模板只留空名 `STRIPE_COMPANION_ADDON_PRICE_ID`（无假 Price）。权威：`FREE_PAID_MATRIX` A6 |
