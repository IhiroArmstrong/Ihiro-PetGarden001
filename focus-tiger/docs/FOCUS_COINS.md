...
数字是否该动、要不要远程化：见经济评估 Brief（**Step D = A 已锁** · 评估 ≠ 已改账本）。`focus-coins-earn` 已链 5 条 persona 夹具（`focusCoinsPersonaRegression.js` · `npm run audit:growth-metrics`）。
...
店底须知（L3 文案）：「案头雅物皆由同坐日久所化，不可借金钱相求。」Lifetime 用户也靠坐禅攒寅币。
