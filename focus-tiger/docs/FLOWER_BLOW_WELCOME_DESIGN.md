...
```
1. Day1 首次冷启动 / ≥3 日久别吹花门闩  → 吹花 + 鼓励气泡（压过 wellness 斗篷/清晨苏醒；当日不再跑普通 WELCOME_APP）
2. 否则 wellness 深夜披斗篷 / 清晨苏醒   → 走休眠/苏醒既有逻辑；跳过普通欢迎池
3. 否则当日首次冷启动                    → 现有 WELCOME_APP 池（书/点头）
4. 欢迎已跳过 / 未播                     → 才可检深夜茶/哈欠（既有互斥不变）
5. 兜底                                  → Idle 坐禅
```

**原则（2026-08-06 纠正）**：**第一次看产品必须先吹花**——不得因 ≥23:00 斗篷或清晨苏醒仪式抢走 Day1 / 久别吹花。普通隔天欢迎仍可被 wellness 跳过。互斥时一招压一招；吹花播完回 Idle，不与斗篷同 tick 叠播。
...
| **优先级门闩** | §2.2；**Day1/久别吹花高于 wellness 斗篷/清晨**；吹花与欢迎池同日互斥 |
