...
- **Focusing 补救（2026-08-04 · §6.12）**：`isFocusing` 时点「?」**只画主条**（`rise-button`），HUD / ambient 等其余进「还有 N 条」芯片逐条展开——禁止窄屏多 tip 叠团。Idle 宽屏「可见锚立刻各出一条」不变（`resolveRemedyImmediateAndFolded`）。
