...
> **未做 / 勿当缺口报**：多档 tip。**场景化请茶** / **意愿漏斗**已接线（TRACKER 对应行）。Ambient 深库分层见 TRACKER「Ambient · 深度曲 entitlement」（免费 5 / 其余 B）。**Focus Tiger Pro US$12.99/月**：Stripe 已建、Checkout 未接；L1 后才允许第四卡。**当前验收仍只测三卡**。Pro 将含 Base（B 轨）；本地智能体仍只 Electron。**Lifetime 用户**将来走一次性加购 `companion.addon.lifetime`（US$29.99），**不是**现货第四卡。Safari 测的是 Web 壳/付款，不是 llama。
