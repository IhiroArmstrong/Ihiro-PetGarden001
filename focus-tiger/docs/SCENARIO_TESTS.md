...
| **P1** | **Z** Journey log 开卡 | 次优先（insight-spark 可顺手核对） | 本批已补 0–1s 句。**核对**：`origin/develop` **无** insight-spark 行标记；Daily Card 点一天存图仍是 Brief 未接线 |
...
> **仍须人工**：Skip Reflection 后 `reflect=false`；无 Arrival 路径降级；>30 裁旧；刷新仍在；开/关卡 0–1s。  
...
2. 宽屏 ⋯ / 窄屏抽屉 **Journey log** → **0–1 秒内**：菜单行按压（宽屏 `.ft-wide-more__btn:active` 缩放；窄屏抽屉同行按压）+ ⋯/抽屉收起 + `#journey-log` 开始淡入（~220ms `is-visible`）。随后见日期 + 分钟 + arrived & reflected（Skip Reflection 则 reflect 降级；缺 Arrival 则无 focus 降级文案）。空列表见 empty 文案，仍算「已开卡」，不要报成哑点击。
3. **回流**：Close / 点外侧 / Esc → **0–1 秒内**：关钮被点到（或外侧按下）+ 卡开始淡出；Idle Sit / ⋯ 或抽屉 grabber **仍可见**。刷新后条目仍在；再完成一场 → 新行在列表（上限约 30，裁旧）。
4. **Compass Reflect**（与场景 Y 交叉）：点 Reflect 芯片 → **0–1 秒内**：芯片被点到 + `#five-moments-compass` 收起 + 同一张 `#journey-log` 淡入。
5. **备份角**（开卡之后；与开卡反馈分开验）：角落备份链 → **0–1 秒内**：链被点到 + `#journey-log-backup-panel` 展开或收起。**Send code** → 立刻见 Sending…（`JOURNEY_LOG_BACKUP_STATUS_SENDING`）再变成发到邮箱的说明。**Enable** 成功须换一句可见状态（勿再用同一句 Backup enabled 让人以为没反应）。详测见 TRACKER 练习记忆备份行。
6. **列表行只读**：点某一天**不会**展开 Daily Card / Save image（Brief `task-journey-daily-card.md` 未接线）。当前 develop **无** insight-spark 行标记。这不是「开卡点了没反应」。
7. **375**：卡可关、不挡 Sit。
8. **对照**：Tip Jar `#yin-tip-jar-tea-log` **不得**因本场 Focus 自动多出一杯茶。
