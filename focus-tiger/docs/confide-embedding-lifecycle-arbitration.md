...
| 历史 `semantic_live_classify` 仅此 1 条（且未就绪） | 本机还没有「embedding 已就绪后再发同句」的成功对照；该实验仍须补做 |
...
#912 只止血（超时 + 已就绪跳过 ensure）。统一闸另开 Brief。渲染进程 `process.env` 崩溃是独立实现缺陷，按已锁 Stage 2 Brief 直接修，不另开行为 Brief。
