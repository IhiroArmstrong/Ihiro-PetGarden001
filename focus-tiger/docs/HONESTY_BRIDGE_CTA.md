...
- **视觉（2026-08-02）**：面板须为 **Arrival 式轻量半透明气泡**（暖米底约 0.62 透明度 + `backdrop-filter` 模糊），**禁止**近乎不透明厚重卡片挡住阿寅下半身与蒲团；Yes/No 按钮可略实一点以保证可读可点。实现：`HonestyBridgeCtaUI` + 共享 `glassPanelStyles.js`（同日拍板：项目内同类叠层厚卡片一律改此风格）。
