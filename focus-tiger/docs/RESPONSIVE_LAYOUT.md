...
**故意不对等（2026-08-18）**：Electron **本地小模型智能体**（`node-llama-cpp`）**不是**原则 A 的覆盖项。它只挂 **宽屏 ⋯**（`≥480px`）。窄屏抽屉、手机浏览器、未来 Capacitor **没有**该能力——设备跑不动，不是漏适配。Confide **检索不生成**仍可按 v1 在宽+窄挂载。QA **禁止**把「375 没有 AI 阿寅」记成响应式缺陷。权威：`PRODUCT_POSITIONING.md`「禅意倾听者」；Brief `task-desktop-on-device-companion.md`。
