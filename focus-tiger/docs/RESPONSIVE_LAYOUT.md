...
**故意不对等（2026-08-18）**：Electron **本地小模型智能体**（`node-llama-cpp`）**不是**原则 A 的覆盖项。它只挂 **宽屏 ⋯**（`≥480px`）。窄屏抽屉、手机浏览器、未来 Capacitor **没有**该能力——设备跑不动，不是漏适配。**低配（总内存 ≤8.5 GiB，Mac 与 Windows 同样）默认也不出入口。** Confide **检索不生成**仍可按 v1 在宽+窄挂载。QA **禁止**把「375 没有 AI 阿寅」记成响应式缺陷。权威：`PRODUCT_POSITIONING.md`「禅意倾听者」；Brief `task-desktop-on-device-companion.md`。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| 1.13 | 2026-08-19 | 原则 A：低配 ≤8.5 GiB（Mac 与 Windows）默认不出本地智能体入口 |
| 1.12 | 2026-08-18 | 原则 A：本地智能体故意不对等（仅 Electron 宽屏；375 无该能力不算漏适配） |
