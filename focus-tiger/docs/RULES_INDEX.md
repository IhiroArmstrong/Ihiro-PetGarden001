...
| `browser-energy` | 预览浏览器与能耗（默认 Safari；硬禁 IDE Browser MCP；临时解禁最长连续 10 分钟；进程收尾 / Cloud 独立会话提醒；用户侧 cd 路径口径） | `.cursor/rules/focus-tiger-browser-energy.mdc` | Focus Tiger · 预览浏览器与能耗 |
...
| `browser-energy` | 「预览浏览器 / 进程收尾 / Cloud 独立会话 / 用户侧 `cd` 路径口径见 `focus-tiger-browser-energy.mdc`」 | 复述完整条款；主张把内置 Browser 当默认预览 / 窄屏特例可开；绕过 IDE Browser 硬闸；临时解禁后挂着超过 **10 分钟**不关；起过 Vite/Playwright 却不在「待你知道」提醒收尾；给用户 `npm run dev` 却写省略号 / 占位 `cd` 路径 |
...
| [`.cursor/rules/focus-tiger-browser-energy.mdc`](../../.cursor/rules/focus-tiger-browser-energy.mdc) | **SSOT**：预览浏览器与能耗（默认 Safari；硬禁 IDE Browser MCP + hooks；临时解禁最长连续 10 分钟；Vite/Playwright 收尾；Cloud 独立会话；用户侧 `cd`/`npm run dev` 路径口径） |
...
| 2026-08-11 | 补回 `browser-energy`：临时解禁路径 **最长连续开放 10 分钟**（对齐 2026-07-25 原口径；控 token/能耗；不放宽默认硬禁） |
