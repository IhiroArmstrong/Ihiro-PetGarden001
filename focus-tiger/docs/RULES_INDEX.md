...
| `agent-tool-budget` | 工具调用预算分档（qa/impl/large + 口令「开工」「继续」「大任务」；hook 可执行） | `.cursor/rules/focus-tiger-agent-token-cost.mdc` | 工具调用预算分档（agent-tool-budget · 可执行） |
...
| `git-agent-commit` | 「见 regression-lock「Commit 汇报与分支门禁」」（含自动 commit + **任务完成后默认 push 旁支/开 PR** + Git 同步分级汇总 + 下班前口令补漏 + **SCENARIO_TESTS 增量核对** + **TEST_TRACKER 碎片拼装**） | 主张「先问再 commit」；主张**每次** push 须口头授权；完整抄门禁条文；**主张可直推 `develop`/`main`**；把 develop 与 feature/fix **并列**成同等可推目标；同步时只报「已 push」无 commit 列表 / 无高风险标注 / 无 PR；把「下班前 Git 同步」做成合并 main / 生产部署 / 推进无关 PR；**下班前同步却不更新 SCENARIO_TESTS**；**下班前同步却不拼装 TEST_TRACKER 碎片** |
...
| `visibility-ci-governance` | 「visibility CI 诊断/责任人/Required 顺序见 `WORKFLOW.md` visibility CI 治理节」；`PROCESS` Backlog 同条可一行引用 | 在 job 0% 绿时勾 Required；把「不等就合」当根因而跳过 E 类导航修；无周报仍声称 visibility 已治理 |
...
| `DESIGN.md` | 产品语义与玩法 |
| `SANCTUARY_UI_ART_DIRECTION.md` | 栖居主屏 + 次级表面**唯一美术 SSOT**（A 语义 / B `#id` / C 主屏补丁 / D Presence·Drawer·Collections）；**不**覆盖断点 / z-index / 三球 / 寅币账本；C.3 实施前玩法仍以 `DESIGN.md` 为准 |
...
| 2026-09-23 | 新增 `visibility-ci-governance`：visibility workflow 近 200 run 0 绿诊断（E/F/C 分类）、26/20 分层修复顺序、周报责任人、Required 前过渡期 PR 须贴 run；SSOT `WORKFLOW.md`；刷新 `PROCESS` Backlog 条 |
