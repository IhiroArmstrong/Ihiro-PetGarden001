...
| `focus-tiger.mustard-seed-seal.v1` | `mustardSeedSeal` / `MustardSeedSealCardUI` | 静思典藏 **第一枚纪念印**《芥子须弥》三 case（内容层 SSOT `CONTEMPLATIVE_ARCHIVE.md`）：`{ revealed, revealedAt, scoreAtReveal, revealedCaseIds, lastShownCaseId }`；门槛 = 统一练习 **score ≥ 21**；每首未揭示诗在完成仪式后出卡一次（Case 1 乐五斋诗稿 / Case 2 乐五斋七言歌行 / Case 3 乐五斋诗稿〇九〇二）；旧档仅 `revealed:true` 视为 Case 1 已见、仍可出 Case 2 再 Case 3；菜单轮换已揭示诗；**不**绑 tip/Sanctuary；章 = `public/ui/support/mustard-seed-seal/yin-badge-square-gold-on-silver-alt.png`（2026-08-12 入库；EN 译维持现稿） |
