...
| `focus-tiger.tip-jar.v1` | `tipJarGate` | Buy Yin a Tea 本地 tip 状态：`{ tipped, tipCount, lastTippedAt, email?, source?, badgeIds[] }`；`badgeIds` = 按练习水平授予的善意徽章（3–9，只增不减）；**不**解锁内容；与 Sanctuary **零耦合** |
