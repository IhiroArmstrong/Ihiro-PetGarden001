...
| `focus-tiger.focus-coins.v1` | `FocusCoinsStore` / `applyFocusCoinsGrant` / `applyFocusCoinsRedeem` | 寅币钱包：`{ balance, ownedIds, equippedTitle, lifetimeMarks, dateKey, day, session }`。L1 发点；L2 `__focusCoins.redeem(skuId)` 花点留下只增不减 `ownedIds`。叠层 SKU 新兑 `retired-overlay`（不点亮莲花/蒲团）。`?focusCoins=0` 关闸完全不写。**不**进练习备份 6 key；**不**满足 `isEntitled`；**不**写 Tea / Sanctuary `badgeIds`；**不**改莲花池自动开花 |
