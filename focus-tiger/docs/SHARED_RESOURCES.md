...
| `focus-tiger.ype-cloud-personalization-consent.v1` | `ypeCloudPersonalizationConsent` | L2 第四条同意 + 本机 `ype_profile_id`；默认关；ON 仅表同意（UI 切片不发送）；OFF 排队删云（Worker 未开工）；**不进**练习备份 |
