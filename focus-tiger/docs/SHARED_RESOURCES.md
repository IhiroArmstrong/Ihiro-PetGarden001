...
| **Companion 鞠躬后 stage** | Choose 点头鞠躬播完 → `companionModePicker.open()` 须带 **`ft-narrow-stage-companion`**，三选一在视口内（尚未 Focusing）；点选后才 Focusing + `#focus-hud`。**禁止**只断言 panel 非 `hidden`（park 下屏外仍假绿）。 | e2e `375 Choose bow: Companion staged in viewport…`（`toBeInViewport` + stage class） |
| **FocusHUD vs ActionBar** | Focusing 或约定叠层期：顶栏时间由谁负责、何时 suppress ActionBar、何时露出 `#focus-hud`——宽/窄须有书面一致结果；禁止一侧有计时、另一侧顶栏空白无约定。 | Choose→鞠躬→点选后 Focusing：375 见 HUD/约定顶栏；≥480 对照 |
