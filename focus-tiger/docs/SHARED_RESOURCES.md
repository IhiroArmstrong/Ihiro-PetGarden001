...
| `focus-tiger.milestone-glow.v1` | `MilestoneGlowStore` | 已播里程碑节点 id（如 `streak-7`）；只增不减；产品壳 `MilestoneGlow` 接线。**QA（仅 dev 或 `FT_QA_BOOT=1` 构建）**：带 `qaSeedStreak` 时默认清本 key（`qaKeepMilestones=1` 可保留）；亦可单独 `qaResetMilestones=1` |
