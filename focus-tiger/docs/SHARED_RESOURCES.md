...
| **`idleChromeOrchestration`**（双壳共享编排） | `listSecondaryChromeEntries` / `resolveRoleVisibility` / stage class 常量 | 窄抽屉与宽 ⋯ **同一业务列表**（四组标题）；宽屏呈现为右侧 sheet；禁止两壳各写漂移 if 树（Task 3 阶段 1） |
