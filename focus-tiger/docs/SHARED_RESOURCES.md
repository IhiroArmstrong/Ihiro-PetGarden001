...
| `focus-tiger.contemplative-archive-seals.v1` | `contemplativeArchiveSeal` / `MustardSeedSealCardUI`（`archiveEntryId`） | 静思典藏 **独立纪念印**（CA-01…CA-12；配置 `memorialSealCatalogCa.js`）：`{ revealedEntryIds }`；**现网 enabled = CA-01 古池（score ≥ 30）**；其余候选 `enabled:false` 待翻开关；仪式在芥子须弥队列之后、Reflection 之前；菜单按门槛解锁 |
