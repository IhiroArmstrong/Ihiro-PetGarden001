...
| **`IdleChromeFacade`**（统一入口） | `createIdleChromeFacade` → `main.js`；`setHandlers` 一次扇出；`applyShellProjection`；断点 `releaseInactivePresentation` | 禁止 main 分别 `setHandlers` 两套语义；resize 不得误关 Companion（Task 3 阶段 2） |
