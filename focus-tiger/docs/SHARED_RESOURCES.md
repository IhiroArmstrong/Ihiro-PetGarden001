...
| `focus-tiger.focus-coins.v1` | `FocusCoinsStore` / `applyFocusCoinsGrant` / `applyFocusCoinsRedeem` / `FocusCoinsPanelUI` | 同坐点/寅币钱包：`{ balance, ownedIds, equippedTitle, lifetimeMarks, dateKey, day, session }`。L1 发点；L2 `__focusCoins.redeem(skuId)` 花点留下只增不减 `ownedIds`。L3 `#yin-coin-panel` 抽屉目录（`FOCUS_COIN_CATALOG` **8 条全列**、可滚动；占位缩略图）。莲叶晨露只叠已有朵。`?focusCoins=0` 关闸完全不写、菜单行隐藏。**不**进练习备份 6 key；**不**满足 `isEntitled`；**不**写 Tea / Sanctuary `badgeIds`；**不**改莲花池自动开花 |
