...
| `focus-tiger.daily-wisdom.v1` | `DailyWisdomStore` / `resolveTodayWisdom` / `<daily-wisdom>` | Yin 每日一句：`{ dateKey, quoteId, recentIds[] }`；同日锁定；`recentIds` 滑动窗（默认 7）避近期重复；**免费**；**不**接 Sanctuary；与 Quiet Line / `dailyZenQuote` **分池分 key**（落点未定，组件可插拔） |
