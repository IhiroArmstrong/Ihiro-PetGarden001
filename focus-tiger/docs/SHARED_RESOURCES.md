...
| 品味层 overlay（内存，非 localStorage） | `tasteLayerOverlay` / `tasteLayerSync`；Dispatcher 池 + Honesty 分档阈值 + `dailyWisdom` 池 + **Quiet Line 混合句池**（`overlayQuietLineTextForKey`） | 未知 / 缺失 `schemaVersion` → 本地冻结表。**禁止**接 Sit 门闩。**禁止**改 `HonestyCheckInController` 来读 overlay。`?tasteLayer=0` 关拉取（含 Quiet Line）。`prefetchTasteLayer` 并行拉 `/api/quiet-line`；拉取不得与精灵预加载 / Arrival·Honesty CapCut 抢主线程；冻结表相同不另存副本（`RB-20260820-L330`）。扩池须过 `ANTI_PLAGIARISM_LAYER.md` 四问。**冻表 vs 现网分叉**见该文 §3.1（本行不锁生产数字） |
