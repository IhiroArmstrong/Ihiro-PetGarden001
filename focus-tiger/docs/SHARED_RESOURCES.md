...
| `focus-tiger.practice-backup.v1` | `practiceBackupOptIn` / Journey Log 角落引导 | 练习记忆云端备份 opt-in：`{ enabled, consentedAt, email, deviceToken, lastUpload* }`；整包 **6 key** 快照 → `PRACTICE_BACKUP_KV`；关闭须 OTP **删云端**。**不含** `monetization-funnel.v1`；打开备份不会上传意愿漏斗 |
