...
| `focus-tiger.mustard-seed-seal.v1` | `mustardSeedSeal` / `MustardSeedSealCardUI` | 纪念印《芥子须弥》：`{ revealed, revealedAt, scoreAtReveal }`；门槛 = 统一练习 **score ≥ 21**（`computePracticeScore`）；首次在计时完成仪式后自动出卡（ZH 原诗 + EN + 乐五斋诗稿 + 章）；之后 Idle ⋯/抽屉可重读；**不**绑 tip/Sanctuary；章视觉暂复用 tip `gold-mono` 路径 |
