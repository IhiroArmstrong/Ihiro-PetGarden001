...
| Wellness (not clinical) | **Cold start** one-shot `#onboarding-wellness-first` (Got it). Always-available lookup: same **?** purpose card, `.onboarding-app-purpose__wellness` (en + ja). Privacy sheet has a one-line cross-link back to that notice. |
