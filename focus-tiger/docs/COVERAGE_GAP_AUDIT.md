...
| **六语槽位** | 增加 `es` / `ja` / `de` / `fr` 文件与注册；**未审校完不进选择器**（或进选择器但标 Coming soon / 灰掉——实现时二选一，默认 **不展示未就绪**） | 仅完整语种 |
| **字典补齐** | 四语可先机翻草稿进仓 + `localeStatus: draft`；**母语/你书面审过**再 `ready` 并露出 | 随 ready 增加 |
| **对外声称** | 未 ready 的语言 **禁止**写进发版说明「已支持」 | — |

→ **结论**：六语**作为产品方向无工程硬阻挡**；**「v1 立刻齐声称六语」有内容/审校项目风险**。下一回合不挡开工；四语是否首发露出，待你对「机翻先上」vs「审完再露」拍板（见文末待你决定）。
