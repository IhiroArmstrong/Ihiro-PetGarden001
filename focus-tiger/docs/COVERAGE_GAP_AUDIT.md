...
> **纠正**：不得把「零自动化」默认解释成「推 post-v1」——**缺口应补覆盖**；缺的是测试与（可选）切语 UI，不是字典骨架。

### 9.1 现状（已有 vs 缺）

| 层 | 状态 | 说明 |
|---|---|---|
| 字典 | ✅ | `en.json` / `zh.json` **各 171 key，1:1 对齐**（抽样日核对：onlyEn/onlyZh = 0） |
...
| 默认语言 | ✅ | `en`（海外市场）；`zh` 为可切换备选 |
| **应用内切语 UI** | ❌ | 用户只能 DEV：`__i18n.setLocale`（场景 G / TEST_TRACKER i18n 行） |
| **locale 持久化** | ❌ | 无 `localStorage`；刷新回 `en` |
| **浏览器语言探测** | ❌ | 未接 `navigator.language` |
| **自动化** | ❌ | 无 `i18n*.test.js`；无切语 e2e；不进 `test:smoke` / PR 冒烟 |

### 9.2 可行性（结论：高）

多语言**运行时切换**在工程上已基本可行：字典齐、订阅面广、切换不必整页重载。  
相对「从零做 i18n」，当前是 **补齐产品入口 + 锁回归**，不是重写文案体系。

### 9.3 任务拆分 · 难度 / 风险 / 是否挡 v1

| 项 | 难度 | 风险 | 建议 | 挡 v1？ |
|---|---|---|---|---|
| **A. 补 unit\***：en/zh key 奇偶校验、`setLocale` 通知、`t` 缺键回退 | **低**（半天内） | 极低 | **立刻做**；并入 `test:smoke` | 若声称双语可用 → **应挡**「无奇偶锁」；成本低故建议直接做 |
| **B. 补 e2e（场景 G 瘦身）**：产品壳 `__i18n.setLocale('zh')` → 断言 Sit / Honesty / 桥接等若干可见文案变中文；再切回 `en` | **低–中**（1 日级） | 低（文案断言；非布局像素） | **立刻做**；可进全量 e2e；可选 1 条进 `test:e2e:smoke` | 声称双语 → **应有至少一条 DOM 锁** |
| **C. 应用内切语 UI**（⋯ / 抽屉一行 EN↔中）+ 写 `focus-tiger.locale.v1` | **中**（1–2 日 + 375 邻接） | 中（壳位、z-index、与 Sound/Reminder 抢位；窄屏文案变长） | **产品拍板**：要「用户可点切换」才进 v1；否则 v1 仍可「字典双语 + DEV/探测切」 | **仅当** v1 定义含「设置里可切语言」时挡 |
| **D. 冷启动语言策略**（记忆上次 / 跟随系统 / 固定 en） | **低–中** | 中（海外默认 en vs 系统 zh 用户预期） | 与 C 同批拍板 | 同 C |
| **E. 全表面人工排版**（375 中文是否裁切） | **人工** | 中（布局） | 进发布前人工 checklist；**永不**像素 e2e（对齐 §5） | 声称双语 → **人工必测**；自动化不替代 |

**不做 / 勿夸大**

- 勿为 i18n 新建第三层 `test:regression`（与 §7 同理：unit 亚秒）。  
- 勿把「Celebrating 动画中文」与切语文案锁混为一谈——动画观感仍 §5 人工。

### 9.4 对「零覆盖」的正确姿态

> 零自动化 + 零 PR 冒烟 → **补 A+B**，不是把整条多语言推 post-v1。  
> 推 post-v1 的只有：**可选的切语 UI（C）**——若你决定 v1 暂不提供用户可点入口。

### 9.5 待你拍板的产品问题（二选一即可推进）

1. **v1 用户可点切语（C+D）**：设置/⋯ 内 EN↔中 + 记忆偏好 → 自动化 A+B+（e2e 点 UI 而非仅 `__i18n`）+ 人工 E。  
2. **v1 字典双语、入口稍后**：运行时仍可 `__i18n` / 将来再加 UI；**仍做 A+B** 锁字典与刷新契约，避免「有 zh.json 却无人敢切」。

（旧稿「终端用户实质单语言 → 自动化 post-v1」**作废**，以本节为准。）
...
- Ambient 实际播放 e2e、Celebrating 动画 e2e、场景 E/F 真实墙钟 DOM  
- **i18n 应用内切语 UI（§9.3 C）**——**仅当**你拍板「v1 暂不提供用户可点入口」；**字典奇偶 unit + 切语 e2e（A+B）不进 post-v1，应补**  
- 若拍板 v1 含切语 UI → C+D 进阻塞面，与 A+B 同批
