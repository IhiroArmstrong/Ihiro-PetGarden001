...
| **MilestoneGlow / IncenseComplete / 莲花池** | ✅ 池 math/store | Glow e2e + `lotus-pond-product` + `lotus-pond-first-bloom` | ❌ | `MilestoneGlowStore` / `lotusPond*` | **池有 DOM；Glow 有 claim e2e；Incense 会话结束仍未接线** | 首朵槽位单测+e2e 已锁左侧不在蒲团中轴（§6.18）；满池螺旋观感仍人工 |
