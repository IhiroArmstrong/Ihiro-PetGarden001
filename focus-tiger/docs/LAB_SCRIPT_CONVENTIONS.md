...
| `FT_INTENT_PHASE` | 可选 · `1` / `2` / `2b` / `2b-hard5` / `2b-residual` | `1`=12 · `2`=设计师 20 · `2b`=v4（44）· `2b-hard5`=hard-5 子集（5）· `2b-residual`=C1 B9/B10 探针（2）；缺省 = Phase 1+2 共 32 |
...
- 单测：`node --test src/core/confide/confideRoundAcceptanceFixtures.test.js`
- 人工：Electron 宽屏按主表 `id` 字母序 **100/100**；**过关 = 本轮 Confide regex 肉测终止**（L3 质量 · Stage 2 语义真路由另门槛）
...
- 人工：Electron 宽屏按冻表 `id` 顺序逐句发，核对 `data-source` / 是否写 `read_hybrid_classify`；可分段调试，关单认主表 100 句
...
- 人工：Electron 宽屏按冻表 `id` 顺序逐句发，核对 `data-route` / 禁 Heard·点头；可分段调试，关单认主表 100 句
...
```bash
cd /Users/armstronghesapplelaptop/Downloads/Zen-tiger-Pet-garden001-wt-develop-qa/focus-tiger/desktop && npm run companion:intent-diagnostic
```
...
**C1 Phase 2B residual 探针（2026-09-06 · #587）**：
