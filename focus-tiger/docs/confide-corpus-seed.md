...
> **状态（2026-08-10）**：样板审稿一轮已落地 · `anxious-03` 待产品拍板 · 其余对齐桶仍须按样板再扫。  
...
| fallback-02 | 你说的，留在这里。 | What you said stays here. | あなたの言葉はここに置く。 | pending-reconfirm |
| fallback-03 | 坐一会儿。茶还热着。 | Sit a while. Tea is still warm. | 少し坐ろう。茶はまだ温かい。 | **ok** |

> fallback-02：已去掉「不必修好什么」（擦边建议）。请再确认是否标 `ok`。
...
| anxious-02 | 听见了。结，还在那儿。 | Heard. The knot is still there. | 聴いた。結び目は、まだそこにある。 | pending-reconfirm |
| anxious-03 | 寅在这儿。呼吸来，呼吸去。 | Yin is here. Breath comes; breath goes. | 寅はここにいる。息が来て、息が去る。 | **discuss** |

> anxious-02：已改为纯确认「结在」，不指示「要不要解开」。请再确认是否标 `ok`。  
> anxious-03：**待你拍板**（定位风险：呼吸引导式 vs 倾听者）。候选替换（未采用，等你选）：「寅在这儿。风来了，风走了。」——若选此，再改三语并标 `pending-reconfirm`。
...
- [x] 样板 A：fallback-01 / fallback-03 → `ok`（fallback-02 待 reconfirm）  
- [x] 样板 B：anxious-01 → `ok`（anxious-02 待 reconfirm；anxious-03 discuss）  
- [ ] 样板 A/B 六句全部 `ok`  
...
**MVP 门闩**：样板六句未全部 `ok` → **不开** `feature/confide-to-yin` 运行时。
