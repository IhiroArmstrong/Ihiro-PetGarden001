...
- **场景 Q** 现有三卡是 Tea / Membership **$6.99/月** / Sanctuary **$89.99**，**没有** Focus-tiger-Pro $12.99 SKU。把本地智能体写成新订阅档会和 Q 职责重叠，也容易让人以为 Membership 含该能力。
- **假收费**：L1 未开、低配默认隐藏入口时，再收一笔「只为本地智能体」的钱 = 付了看不见的能力。
- **场景 W** 的「?」适合放系统技术说明；阿寅不说这句。

拍板后的落地：说明文案进 Electron **?** + **Support Yin** 底部 + `desktop/README.md`；Windows 与 Mac 同样写 8GB；**不**开 $12.99 SKU；低配仍可买现有三卡（B 轨其它权益），**不能**自行打开被隐藏入口。
...
| 日期 | 说明 |
|---|---|
| 2026-08-19 | 正面案例补：低配 8GB 说明进 ? / Support（Electron-only）；禁止另开 $12.99 本地智能体 SKU（假收费 / 与场景 Q 三卡冲突） |
