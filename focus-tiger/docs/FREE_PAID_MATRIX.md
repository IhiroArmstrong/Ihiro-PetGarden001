...
**2026-08-19 补**：上表禁的是第三套 *内容* 层级。桌面本地智能体是 **运行时成本 SKU**（Focus-tiger-Pro），见 **A6**；现货 Support **仍只三卡**，本仓未接线。
...
### A6 · 桌面本地智能体运行时（Focus-tiger-Pro · 非 B 内容）

> **方向锁（2026-08-19 晚）**。这不是第三套进阶内容，也不是 C 轨寅币。权威：`task-desktop-on-device-companion.md`。**现货未接线。**

| 功能 / 资产 | 产品档位 | 付费方式备注 | Catalog / gate | 文档口径 | 代码落地 | 差距说明 |
|---|---|---|---|---|---|---|
| Focus-tiger-Pro 订阅（本地智能体） | 独立运行时 SKU | **US$12.99/月**（价格在 Brief，不升本表 SSOT）；仅 L1 入口正常开放后 | **无** FEATURE_CATALOG key（尚未立项接线） | 覆盖桌面推理成本；**不含** B 轨；Membership **不含**本能力 | **未接线** | Stripe Dashboard 可先建 Product。Support **现货仍三卡**。仅 Electron + 宽屏 + 非低配可买。禁止 L1 未开时售卖；禁止低配冒险覆盖隐藏入口 |
...
16. **Focus-tiger-Pro（桌面本地智能体）** — 政策已拍板；**未接线**。等 L1 入口正常开放 + 合格机型才露出；禁止现在改 Support 三卡。
