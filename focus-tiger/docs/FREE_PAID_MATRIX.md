...
| 兑换：空间变体 / 称号 / 稀有练习章 / 阿寅轻点缀 | 练习兑换 | **不可现金购买** | SKU `cosmetic.*` / `title.*` / `badge.rare.*` | 不拦截自动纪念物；≠ 换装柜 | **L3 抽屉已接线**（`#yin-coin-panel`；8 SKU 全列；占位缩略图） | 须弥坐 = 360 点 **且** `lifetimeMinutes ≥ 600`；不可现金 / 会员跳过 |
...
15. **同坐点 / 寅币（C · 练习货币）** — 方向锁 2026-08-18；L0 #335 · L1 #338 · L2 #339 已合（TRACKER 待人工）；**L3 抽屉本支**。**禁止**用点满足 `isEntitled`。
