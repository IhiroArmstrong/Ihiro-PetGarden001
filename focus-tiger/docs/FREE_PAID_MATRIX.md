...
| Stay in touch / Newsletter | `free` | 可选留资；非账号 | 无付费 gate | 增长；不挂钩解锁 | **已接线**（Worker `d0140328` await 欢迎信） | 本地只记 `submitted`；名单在 `NEWSLETTER_KV`；退订同批。无 Cloud 时 mock。群发未接线。2026-08-15 用户书面：欢迎信已收到（一封曾进垃圾箱；无 `welcomeSentAt` 再提交会重发） |
