...
| Sanctuary 尊贵徽章 | `lifetime∪subscription` | 付费/preview 起授 | Sanctuary `badgeIds`（非 FEATURE_CATALOG key） | B | **已接线** | Idle `#yin-tip-kindness-badges`：Lifetime **或** Membership 起授 ≥3（`idlePracticeBadges`）；**不**把 Sanctuary SKU `unlocked` 标真。**蒲团中央 Enso**：`#yin-sanctuary-enso-mark` 已接线（lifetime∪subscription；零 tip；Focusing 淡化；点按不开店） |
