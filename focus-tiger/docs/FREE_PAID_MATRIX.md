...
| 节日主题引擎（Seasonal Theme） | `lifetime∪subscription` | Lifetime 或 Membership；官方节日 `subscriberOnly: true` | 建议 key `theme.seasonal.access`（字面 subscription / **ongoing**） | B；时段氛围非纪念物 | **未接线** | Brief `task-seasonal-theme-engine-v1.md` Phase 1 文档已定；引擎/catalog/UI **未实现**；到期撤主题（ongoing），不写 persistent ownership |
...
7. **节日主题引擎 `theme.seasonal.access`** — 文档已定 B 轨 ongoing；catalog / 引擎 / 消费 **未接线**（见 `task-seasonal-theme-engine-v1.md`）。
