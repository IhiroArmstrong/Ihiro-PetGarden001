...
| 兑换：称号 / 稀有章 / 珍藏静物 / 闲笔挥手 | 练习兑换 | **不可现金购买** | SKU `title.*` / `badge.rare.*` / `collection.*` / `gesture.*` | 花园自动；珍藏结缘；禁止叠 PNG | **L2 可兑**；叠层 SKU `retired-overlay` | L3 = Yin's Collections 表面 |
...
15. **寅币（C · 练习货币）** — 方向锁 2026-08-19（花园 vs 珍藏）；L0 #335 · L1 #338 · L2 #339 已合。叠层 SKU 本支退役。L3 = Yin's Collections。**禁止**用点满足 `isEntitled`。
