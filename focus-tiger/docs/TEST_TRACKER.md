...
| 文档-代码结构对齐（DOC_CODE_CONTRACT） | 纯后端 | 仅单元测试覆盖 | `npm run docs:check`（hints / gate / **state-machine** 三块 registry ↔ md）；`state:doc-sync` / `gate:doc-sync` / `hints:doc-sync`；故意改 ARCHITECTURE §状态机或 SHARED_RESOURCES §4 机器块须 exit 1。pre-commit：根目录 husky → `test:smoke`。CI：`.github/workflows/focus-tiger-doc-contract-check.yml`（**须 `npm ci`**；否则 smoke→PoseManager 缺 `three` 红）。**基础设施红绿（2026-07-22）**：红 [`29916112037`](https://github.com/IhiroArmstrong/Ihiro-PetGarden001/actions/runs/29916112037)（`60c129a`，缺 `npm ci`）→ 修 `7b90283` → 绿 [`29919097318`](https://github.com/IhiroArmstrong/Ihiro-PetGarden001/actions/runs/29919097318)（**completed / success**）。详见 `DEV_WORKFLOW_QUALITY.md` §7.7。 | — | `docs/DOC_CODE_CONTRACT.md` · `StateManager.js` · `sessionUiGateContractRegistry.js` | 2026-07-22 |
