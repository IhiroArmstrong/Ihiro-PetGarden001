...
| 窄屏 · 主屏三主钮（Sit / Quick Start / Honesty） | UI可见 | 待人工测试 | **主路径（375×667 · `?product=1`）**：Idle 见 `#ft-narrow-home-ctas`——**Sit with Yin**（蒲团橙）/ **Quick Start** / **Honesty Check-in** 在老虎下方，填满原空白区；文案完整可读、触控 ≥44px。点 Sit → Arrival；点 Quick Start → 立刻 Focusing；点 Honesty → 时长三选一。**抽屉**：上滑后 **不得**再出现这三项；仅保留呼吸 / How shall we sit? / Sound / Reminder + 7 格。**回流**：Rise → 三主钮再出现；Arrival/Honesty 叠层期间主钮随壳 suppress 消失（不挡 Notice）。**宽屏 ≥480**：dock 不变（本行不改桌面）。**§8**：Sit→Notice 点 tip 只关 tip。**DOM**：e2e `weekly-practice-heatmap.spec.js`（主屏三钮 + 抽屉无三主钮）+ `scenario-a` 375 tip。 | **2026-07-26 用户书面**：窄屏首页底部太空，非极简；须放三个最常用按钮；拍板 Sit with Yin / Quick Start / Honesty Check-in，并从抽屉删除以便后续加项。 | DevTools 375 · `#ft-narrow-home-sit` · `#ft-narrow-home-quickstart` · `#ft-narrow-home-honesty` | 2026-07-26 |
