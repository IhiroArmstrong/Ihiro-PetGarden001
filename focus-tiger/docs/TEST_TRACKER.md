...
| 宽屏 · ⋯ Sound → Soundscape 选曲面板 | UI可见 | 待人工测试 | **主路径（≥900）**：Idle → ⋯ → Sound → **立刻**见曲目+音量面板；**禁止**只出红色 Sound FAB；**禁止**只出一行门闩 tip。**回流**：点外侧收起 → 再 ⋯→Sound 仍开面板。Focusing 时 ⋯ 隐藏、右下 Sound FAB **仍在**（设计如此，勿改掉）。 | — | `?product=1` · `#ft-wide-more-btn` · `.ambient-soundscape__panel` · e2e wide Sound | 2026-07-25 |
| 宽屏 · ⋯ Honesty 常列（DORMANT/入口 hidden 仍在） | UI可见 | 待人工测试 | **主路径**：Idle ⋯ 菜单含 Honesty。模拟/真实 DORMANT 或入口 `hidden` 后打开 ⋯ → **仍见** Honesty；点开进时长三选一。**回流**：补登/关面板后再开 ⋯ 仍在列。 | — | `#ft-wide-more-menu [data-proxy=honesty]` · e2e | 2026-07-25 |
| 宽屏 · 禁止开机自动播音乐（opt-in） | UI可见 | 待人工测试 | **主路径**：硬刷新 `?product=1` → **不得**自动出「Music is on」/可闻播放；须用户经 ⋯→Sound 或 Focusing mute/♪ 主动开。**回流**：Rise 后再进 Idle 仍默认关。 | — | DEV `__ambientSoundscape` · e2e no autoplay | 2026-07-25 |
| 宽屏 · Rise/达标后停播 ambient | UI可见 | 待人工测试 | **主路径**：⋯→Sound 选曲开播 → Quick Start → Rise → **音乐须停**。**回流**：再 Sit/⚡ 不得自动续播。 | — | Rise · `__ambientSoundscape.isAudiblePlaying` | 2026-07-25 |
| 宽屏 · 轻量面板点外侧关闭 | UI可见 | 待人工测试 | **主路径**：⋯→How / Sound / 提醒 打开后，点画布空白收起。**例外**：Honesty 桥接 Yes/No **不得**点外侧当 No。**回流**：收起后再经 ⋯ 打开。 | — | Companion / Soundscape / Reminder · e2e outside-click | 2026-07-25 |
| 宽屏 · Focusing HUD + mute 回流 | UI可见 | 待人工测试 | **已好清单复测**：⚡/Sit 开 Focusing → 左上 `#focus-hud` 正常；右下 Sound FAB 可见（设计如此）。**回流**：Rise → HUD 回 Calm；⋯ 再出；mute/音乐态不丢控制。一分钟呼吸进行中 HUD 亦可见。 | — | `#focus-hud` · `.ambient-soundscape__fab` · `#ft-wide-more-btn` | 2026-07-25 |
| 宽屏 · ? hint 锚点（park→⋯）完整回归 | UI可见 | 待人工测试 | **保护面**：宽屏 Idle park 后点左下 `?` 补救 → How / Honesty / Sound / Reminder / 呼吸相关 tip **须锚在 ⋯ 附近**（不得指到屏外 park 坐标）。**回流**：Focusing 时 Sound FAB 回来后 ambient tip 可回右上 mute/FAB；Rise 后再 Idle 仍 remap 到 ⋯。 | — | `#onboarding-hint-help` · `#ft-wide-more-btn` · e2e remedy near ⋯ | 2026-07-25 |
...
| Sleeping / DORMANT 睡态循环 | UI可见 | 待人工测试 | **自动**：距上次专注结束 ≥ `DORMANT_IDLE_HOURS`（默认 **2h**）→ `STATES.DORMANT`；披毯后睡姿为 **cloak-sleep 034→030 每帧两拍 pingpong** @ **1 fps**（不再用旧 `sleeping/` 侧卧 8 帧）。新用户无结束记录**不**触发。零完成开局仍为 Idle。**调试**：「睡着了」→ 同 pingpong 微动。**须分列验**：① 微动极缓安宁；② 与披毯末帧同姿连贯；③ Honesty 倒放唤醒仍可从睡姿接上。 | **2026-07-22**：用户书面——模拟 3h 前进睡后 HUD Asleep + sleeping 侧卧，再 Honesty 唤醒，**测试 OK**。**2026-07-25**：用户书面——披毯过渡与后续睡姿**完全不连贯**（见 cloak-sleep 行）→ 已改同源末帧 pingpong，请复测。 | `?product=1` · DEV 改 `focus-session-end` · `#emotion-debug-ui` | 2026-07-25 |
...
| 应用内提醒设置入口 + 横幅 UI | UI可见 | 待人工测试 | **设置入口**：Idle 左下热力图簇旁的小型时钟图标（`ReminderPreferenceUI`）。**宽屏 ≥480 Idle**：时钟常驻入口停靠，经底栏 **⋯** 菜单打开（`openPanel`）；热力图仍可见。**窄屏**：仍在抽屉/簇内。仅 Idle 可见；点开轻量面板 → 勾选「开启提醒」写入 `{ hour, minute }`；取消 → 清空存储。**横幅**：`#ui-overlay` 顶部居中非模态横幅（`InAppReminderBannerUI`），到点且今日未完成时出现文案 `reminder.gentle_waiting`（EN "Yin is right here when you're ready." / ZH「你准备好了，阿寅就在这儿。」；**禁** waiting/在等你紧逼感），可点 × 关闭。**主路径**：⋯→提醒（或窄屏点时钟）→开启并设时间→切后台再回前台→横幅出现。**回流**：关闭横幅后本页会话内不再出现；完整刷新 / 新开 App 后若条件仍满足，可再次出现。**忙碌期策略（已拍板）**：Arrival / Focusing / Celebrate / Reflection / 微仪式进行中 → **`suppress`**。DEV：`window.__inAppReminder.{sync,setNow,clearNow,controller,settings,banner}`。 | 2026-07-22：入口从右上改到热力图簇旁；**DOM** e2e `in-app-reminder.spec.js`。**2026-07-23 用户拍板**：忙碌策略用 **suppress**。 **2026-07-24 用户书面**：设置中途不弹横幅主/回流路径 **测试 OK**；横幅「Yin is waiting」紧逼感 → 已改 presence 措辞，**须复测**新文案。**2026-07-25**：宽屏改经 ⋯ 打开，**须复测**入口路径。 | Idle · `#ft-wide-more-btn` / `#reminder-preference-toggle` · `#in-app-reminder-banner` · e2e `in-app-reminder.spec.js` | 2026-07-22 |
...
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 待人工测试 | 1) 先点掉自动提示。2) 点 **?**。3) Idle 须同时见：Sit / How shall we sit? / **热力图** / **一分钟呼吸**（入口可见时）/ **Sound gated** / `help-remedy`（含 EN「Click a tip to dismiss… tap ? anytime…」）+ **用途简介卡**。4) Honesty 桥接打开时点 ? → 须见 **honesty-bridge** tip + 热力图/Sound（**不见** micro-ritual）。5) 点气泡关该条；再点 ? 仍全部出现。 | 2026-07-20：只有元文案。**2026-07-21**：加简介卡。**2026-07-22**：A 类测试 OK 后用户再书面缺 tip → **已补登记**四类 + 更新 help-remedy；请硬刷新复测。 | `#onboarding-hint-help` · `#onboarding-app-purpose` | 2026-07-22 |
