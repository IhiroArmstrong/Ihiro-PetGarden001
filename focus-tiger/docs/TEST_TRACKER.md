...
<<<<<<< HEAD
| 右上音符开/关声景（菜单已删 Sound） | UI可见 | 待人工测试 | **主路径（375）**：ActionBar ♪ → Soundscape 选曲面板（非 mute 翻转）；与抽屉 Sound 同效；FAB 不可见。**Focusing**：ActionBar 常显，点 ♪ 同样开面板（浮动 `.ambient-soundscape__mute` 已藏）。**主路径（≥480）**：右上 `.ambient-soundscape__mute` → 同面板；⋯ Sound 同效；右下 Sound FAB **不可见**（含 Focusing）。**回流**：面板内选曲 / Off；Rise 后停播口径不变。 | **2026-07-29**：承接上行用户书面。**同日**：rebase onto develop after PR#23（墙钟）合入，解 `NarrowIdleShell.js` 冲突。e2e：`375: ActionBar note opens…` · `wide Idle: top-right note opens…` · `wide Focusing: …FAB stays hidden`。 **2026-07-30 用户书面（图4）**：⋯/抽屉 **Sound 应删除**（与右上音符重复）；首次登录菜单项须薄荷绿脉冲；? more tips 勿乱指隐含菜单。音符：无薄荷绿；**有声时再点须关音乐**。 | `?product=1` · `#ft-narrow-mute-btn` · `.ambient-soundscape__mute` · `.ambient-soundscape__panel` | 2026-07-29 |
...
=======
| 右上音符开/关声景（菜单已删 Sound） | UI可见 | 待人工测试 | **主路径（375）**：ActionBar ♪ → Soundscape 选曲面板（非 mute 翻转）；与抽屉 Sound 同效；FAB 不可见。**Focusing**：ActionBar 常显，点 ♪ 同样开面板（浮动 `.ambient-soundscape__mute` 已藏）。**主路径（≥480）**：右上 `.ambient-soundscape__mute` → 同面板；⋯ Sound 同效；右下 Sound FAB **不可见**（含 Focusing）。**回流**：面板内选曲 / Off；Rise 后停播口径不变。 | **2026-07-29**：承接上行用户书面。**同日**：rebase onto develop after PR#23（墙钟）合入，解 `NarrowIdleShell.js` 冲突。e2e：`375: ActionBar note opens…` · `wide Idle: top-right note opens…` · `wide Focusing: …FAB stays hidden`。 | `?product=1` · `#ft-narrow-mute-btn` · `.ambient-soundscape__mute` · `.ambient-soundscape__panel` | 2026-07-29 |
| ⋯/抽屉菜单删除 Sound + 行内薄荷绿 | UI可见 | 待人工测试 | **主路径**：清空 hints → 开 ⋯/抽屉 → **无 Sound 行**；Honesty/呼吸/How/提醒旁见薄荷绿脉冲。音乐仅右上音符：未播→开面板；**可闻播放中再点→关音乐**。宽屏 ? more tips 折叠为 `wide-more-menu`。 | **2026-07-30 用户书面（图4）**拍板删重复 Sound；菜单项须薄荷绿；有声再点关乐。 | `#ft-wide-more-menu` · `.ft-secondary-menu-hint-dot` · `.ambient-soundscape__mute` | 2026-07-30 |
>>>>>>> bf2778e (fix(ux): drop menu Sound, mint menu dots, note mute toggle, Calm tip.)
...
| Onboarding hints · click 圆点 tier（peeked/static/done） | UI可见 | 待人工测试 | **对比反例（必测）**：① 清空已读 → How shall we sit? 旁见**脉冲**圆点（约 8–10px）。② 悬停或点圆点看 tip → 点外部关框 → 圆点应变**静止弱化**（约 5–6px、半透明、不闪）=`peeked`，**不得消失**。③ 再点 How shall we sit? 展开三选一（相关操作）→ 圆点**彻底消失**=`done`。④ `help-affordance`：点圆点见 tip+「了解此空间」仍脉冲；点 CTA 或「?」开简介卡后圆点消失。**回流**：实验室清空已读后脉冲复现。 | — | `?product=1` · `.onboarding-hint-badge[data-ack]` · `ONBOARDING_HINTS.md` §〇 | 2026-07-23 |
<<<<<<< HEAD
| Onboarding · 首次登录右上音符薄荷绿圆点 | UI可见 | 有问题 | **主路径**：实验室清空引导已读 → `?product=1` 冷启动 Idle → 右上音符旁见**薄荷绿脉冲圆点**；点圆点见 EN「If your environment allows…」/ ZH「若环境允许…」；点音符打开 Soundscape。**回流**：关 tip 后圆点 peeked 静止弱化；开过 Soundscape/选曲后圆点 done 消失。**不得**一堆 auto tip 挡操作。 | 2026-07-30：合入 click-trigger；文案按用户急需口径。 **2026-07-30 用户书面（图）**：宽屏 Idle/Asleep 右上音符无薄荷绿脉冲；? / ⚡ / ⋯ 有圆点。选曲面板可开但听不到声音（音符带斜杠）。 | `?product=1` · `.ambient-soundscape__mute` · `.onboarding-hint-badge` | 2026-07-30 |
| FocusHUD · Idle Calm 圆点可点（原钢蓝 discovery-dot） | UI可见 | 待人工测试 | **主路径**：清空 hints → Idle Calm 金环旁见**薄荷绿**脉冲圆点（非不可点钢蓝点）；悬停/点击展开 `focus-hud-ring` tip。**回流**：关 tip → peeked；相关操作后 done。 | **2026-07-30 用户书面（图5）**：Calm 左上角蓝点悬停/点击无反应 → 原 `ft-hint-discovery-dot` 为 `pointer-events:none`；已改为 Idle 亦调度 mint click badge，并清空非交互钢蓝宿主表。 | `#focus-hud .ft-hud__gauge` · `.onboarding-hint-badge[data-hint-id="focus-hud-ring"]` | 2026-07-30 |
=======
| Onboarding · 首次登录右上音符薄荷绿圆点 | UI可见 | 有问题 | **主路径**：实验室清空引导已读 → `?product=1` 冷启动 Idle → 右上音符旁见**薄荷绿脉冲圆点**；点圆点见 EN「If your environment allows…」/ ZH「若环境允许…」；点音符打开 Soundscape。**回流**：关 tip 后圆点 peeked 静止弱化；开过 Soundscape/选曲后圆点 done 消失。**不得**一堆 auto tip 挡操作。 | 2026-07-30：合入 click-trigger；文案按用户急需口径。 **2026-07-30 用户书面（图）**：宽屏 Idle/Asleep 右上音符无薄荷绿脉冲；? / ⚡ / ⋯ 有圆点。选曲面板可开但听不到声音（音符带斜杠）。 **同日再书面（图4）**：菜单删 Sound；音符有声再点须关乐；菜单项须薄荷绿。 | `?product=1` · `.ambient-soundscape__mute` · `.onboarding-hint-badge` | 2026-07-30 |
| FocusHUD · Idle Calm 圆点可点（原钢蓝 discovery-dot） | UI可见 | 待人工测试 | **主路径**：清空 hints → Idle Calm 金环旁见**薄荷绿**脉冲圆点；悬停/点击展开 tip。**回流**：关 tip → peeked。 | **2026-07-30 用户书面（图5）**：Calm 左上角蓝点悬停/点击无反应 → 原 `pointer-events:none` 钢蓝点；已改 Idle 调度 mint badge 并清空非交互宿主表。 | `#focus-hud .ft-hud__gauge` · `.onboarding-hint-badge[data-hint-id="focus-hud-ring"]` | 2026-07-30 |
>>>>>>> bf2778e (fix(ux): drop menu Sound, mint menu dots, note mute toggle, Calm tip.)
...
| | 人工 · 点 ? 补救（主条 +「更多提示」一次性抽屉说明） | 有问题 | 待人工测试 | **主路径（375）**：先点掉自动提示 → 点 **?** → 见 **情境主条** tip（如 Sit）+ **「More tips — tap here」芯片**（无 3/2 倒计时）+ **用途简介卡**。点芯片 → **只多出 1 条** `narrow-drawer-menu`（上滑拉开底部抽屉：呼吸 / How shall we sit? / Sound / Reminder / 近日同坐格），芯片消失。**禁止**抽屉未开时热力图 tip 尖角指 Sit 球。**宽屏 / 抽屉已开**：仍可逐条展开（同时最多主条+1）。抬升须 **lift→separate** 堆叠，禁止压到同一 Y。**回流**：关气泡后再点 ?；Rise 后再点 ?。 **2026-07-30 用户书面（图）**：宽屏仍见「5 more tips」；担心 tip 指向 ⋯ 隐含菜单内控件（窄屏 one-shot 已修，宽屏未对等）。 | **2026-07-20**…**2026-07-27 前**：逐条展开 + tip stacking 曾验收。**2026-07-27 用户书面（图2）**：「4 more tips」在抽屉关闭时乱指 Sit → 已改 one-shot `narrow-drawer-menu`（cherry-pick 入 develop）。**须复测 375**。 | `#onboarding-hint-help` · `#ft-hint-catalog-chip` · `.ft-narrow-grabber` · e2e `375 park: ? remedy` | 2026-07-27 |
...
| Hints · 背景音乐 opt-in 提示锚右上 mute（非右下 Sound） | UI可见 | 待人工测试 | **主路径**：`?product=1` 重置 hints → Idle 或开计时后见 `ambient-soundscape` 文案（EN「…tap the note to open Soundscape」/ ZH「点音符打开声景面板…」）→ 气泡**尖角须指右上音符钮** `.ambient-soundscape__mute`（375 remap `#ft-narrow-mute-btn`），**不得**指右下 Sound FAB。**回流**：点「?」补救；点音符应开面板（与 Sound 同效）。 | **2026-07-22**：用户书面——旧「Music is on…」Hint 错指右下 Sound。**2026-07-25**：文案改为 opt-in；锚点仍 mute。**2026-07-29**：音符改为开面板；文案同步。 | `?product=1` · `.ambient-soundscape__mute` · `#ft-narrow-mute-btn` | 2026-07-29 |
