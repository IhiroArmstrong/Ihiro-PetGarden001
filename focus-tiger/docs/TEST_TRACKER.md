...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 待人工测试 | **主路径**：① hint → 任选模式 → Arrival → **Skip — begin 或 Choose/鞠躬后立刻 Focusing + 计时**，**无需**再点 Sit。② Sit → Arrival → 三选一 **点选即计时**（含 Offline）。**回流**：Rise 后再 hint → 任意模式 → Arrival 结束仍自动开表。 | **2026-07-21**：Flow/Reading 鞠躬开表已修。**2026-07-21 晚**：用户书面 Offline 亦禁止二次 Sit；三模式统一开表。 | 底部 Sit 旁 dock · e2e A/K | 2026-07-21 |
...
| cloak-sleep / 披毯入睡（进 DORMANT） | UI可见 | 待人工测试 | **主路径**：非 DORMANT→DORMANT 转换 → `cloakSleep` 正放 @6fps≈5.7s → cross-fade → `sleeping`。**回流**：已 DORMANT 持续睡（含跨午夜 sync）→ **不**重复披毯；唤醒后再进 DORMANT → 再播一次。**调试**：入库素材仍可单条试播。 | — | 改 `focus-session-end` 时间戳模拟 · `#emotion-debug-ui` | 2026-07-21 |
...
| Sleeping / DORMANT 睡态循环 | UI可见 | 待人工测试 | **自动**：距上次专注结束 ≥ `DORMANT_IDLE_HOURS`（默认 **2h**）→ `STATES.DORMANT` + `sleeping` 1 fps；新用户无结束记录**不**触发。零完成开局仍为 Idle。**调试**：「睡着了」仍可直接试播。 | — | `?product=1` · DEV 改 `focus-session-end` · `#emotion-debug-ui` | 2026-07-21 |
| DORMANT 2h 滚动触发 + sleep→wake 串联 | 仅单元测试 | 仅单元测试覆盖 | **自动化**：`dormantIdle` chain + smoke `D sleep→wake`（2h→cloakSleep→dormantWake→离 DORMANT→桥接）。**人工**：改 `focus-session-end` 时间戳 → 刷新/回前台见披毯入睡 → Honesty 选时长见倒放睡醒 → 桥接。 | — | `npm run test:smoke` · `focus-tiger.focus-session-end.v1` | 2026-07-22 |
...
| dormantWake / Honesty 睡醒序列 | UI可见 | 待人工测试 | **主路径**：调试「睡着了」→ 再「Honesty唤醒」选时长 → **`cloak-sleep` 34 帧倒放**（**6 fps** ≈5.7s）→ 定格末帧（合掌坐姿）。**回流**：定格后桥接 Yes/No。**对比**：「唤醒(伸懒腰)」仍走 stretch-reminder，视觉须不同。**不**淡入闭眼呼吸、**不**自动接光环金光。 | **2026-07-21**：用户希望用「斗篷覆盖→趴下睡觉」动画倒放替换原 `dormant-wake` 正放；已改 manifest `frameIndices` 倒序。请硬刷新复测睡→醒衔接与末帧定格。 | Honesty / `#emotion-debug-ui` · `playEmotion('dormantWake')` | 2026-07-21 |
