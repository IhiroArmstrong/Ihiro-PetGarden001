...
| 场景动画 · Slice A（切语合十/鞠躬 + Honesty Idle 点头） | UI可见 | 待人工测试 | **实现未开工**（Brief 已立项）。落地后：**主路径** Language→日本語→合十；→English→鞠躬；同日同语不重复。Honesty Idle 选时长→呼吸结束→toast **且**短点头→桥接。**回流** Focusing 中切语跳过。**禁止** Celebrating。微仪式完成仍 sessionComplete。 | — | Brief `task-scene-animation-wiring-v1-slice-a.md` · `?product=1` | 2026-07-31 |
