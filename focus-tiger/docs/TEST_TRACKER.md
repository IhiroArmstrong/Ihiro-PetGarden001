...
| Onboarding · 首次登录右上音符薄荷绿圆点 | UI可见 | 待人工测试 | **主路径**：实验室清空引导已读 → `?product=1` 冷启动 Idle → 右上音符旁见**薄荷绿脉冲圆点**；点圆点见 EN「If your environment allows…」/ ZH「若环境允许…」；点音符打开 Soundscape。**回流**：关 tip 后圆点 peeked 静止弱化；开过 Soundscape/选曲后圆点 done 消失。**不得**一堆 auto tip 挡操作。 | 2026-07-30：合入 click-trigger；文案按用户急需口径。 | `?product=1` · `.ambient-soundscape__mute` · `.onboarding-hint-badge` | 2026-07-30 |
...
| | 人工 · 点 ? 补救（主条 +「更多提示」一次性抽屉说明） | UI可见 | 待人工测试 | **主路径（375）**：先点掉自动提示 → 点 **?** → 见 **情境主条** tip（如 Sit）+ **「More tips — tap here」芯片**（无 3/2 倒计时）+ **用途简介卡**。点芯片 → **只多出 1 条** `narrow-drawer-menu`（上滑拉开底部抽屉：呼吸 / How shall we sit? / Sound / Reminder / 近日同坐格），芯片消失。**禁止**抽屉未开时热力图 tip 尖角指 Sit 球。**宽屏 / 抽屉已开**：仍可逐条展开（同时最多主条+1）。抬升须 **lift→separate** 堆叠，禁止压到同一 Y。**回流**：关气泡后再点 ?；Rise 后再点 ?。 | **2026-07-20**…**2026-07-27 前**：逐条展开 + tip stacking 曾验收。**2026-07-27 用户书面（图2）**：「4 more tips」在抽屉关闭时乱指 Sit → 已改 one-shot `narrow-drawer-menu`（cherry-pick 入 develop）。**须复测 375**。 | `#onboarding-hint-help` · `#ft-hint-catalog-chip` · `.ft-narrow-grabber` · e2e `375 park: ? remedy` | 2026-07-27 |
