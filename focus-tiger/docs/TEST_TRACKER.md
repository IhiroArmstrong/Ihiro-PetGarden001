...
**§B 未单列、但在场景 checklist 里测的项**（见 **L178–L185**）：**[L185](#L185)** A1 Idle 开局（**待人工测试**；旧睡着口径作废） · **[L183](#L183)** Celebrating / 同日 SessionComplete 观感（**已通过**） · **[L184](#L184)** Honesty 桥接完整 Arrival（**已通过**） · DEV 一键重置（**L-logic / 仅单元测试**）。
...
| Ambient Soundscape / 静音图标 + Sound | UI可见 | 待人工测试 | **静音**：右上米色圆形 **音符钮**（在播=音符+斜杠，点一下即静音）。**Sound**：右下蒲团橙 **Sound** **始终可见**；**Sit 开计时后**可展开选曲/音量；未专注点 Sound 会提示先开始专注。**主路径**：默认 Mer-Ka-Ba；音符静音/恢复；专注后 Sound 换曲；**Yin idle 闭目坐禅可见**。**回流**：关→刷新仍关；Rise 后 Sound 仍可见但不可开面板。 | **2026-07-21**：用户书面——整页只剩 HUD/Sit/调试条，Yin 与左右按钮全不见。根因：`AmbientSoundscapeController` 在 `_volume` 未初始化时设 `audio.volume` 抛错 → `init()` 中断。已修。请硬刷新。 | 右上 `.ambient-soundscape__mute` · 右下 `.ambient-soundscape__fab` | 2026-07-21 |
...
| 人工 · A1 Idle 开局（非 Sleeping） | UI可见 | 待人工测试 | 1) 实验室「重置全部本地状态」。2) 开 `?product=1`。3) 确认阿寅是 **Idle 闭目坐禅**，**不是**睡着。4) Honesty 可忽略提示可见。5) 右下角见 **关闭音乐**（默认有声）。 | **2026-07-21**：用户书面——第一幕不能睡觉；须 Idle + 默认音乐。旧「A1 睡着已通过」口径作废。 | `?product=1` · 重置按钮在 `/` | 2026-07-21 |
...
| 人工 · 静音图标 + Sound | UI可见 | 待人工测试 | 1) `?product=1` 重置后进入。2) **右上**音符钮静音/恢复。3) **右下 Sound 始终可见**（未专注略淡）。4) **Sit 开计时** → 点 Sound 展开选 Mer-Ka-Ba / Meditation Impromptu + 音量。5) Rise 后面板应收起、Sound 仍可见。 | **2026-07-21**：用户书面 Sound 不见；已恢复始终可见。 | `?product=1` | 2026-07-21 |
