...
| 场景 A · 应用内提醒 × 鹦鹉信使 | UI可见 | 待人工测试 | **主路径**：设提醒 → 回前台 / 到期（含同页静候，约每 60s 再评）→ 横幅 + 鹦鹉。**回流**：横幅从隐藏再变为可见（含 suppress 后再显、刷新后再到期）应**再播**信使；同一次可见期间 re-sync 不重播。× 关闭后本页横幅不复现（既有 dismiss）。冷启动：Welcome 动画优先，信使等欢迎结束。**可选邻接（非必测）**：横幅已出时点 ⚡/Sit 进 Focusing → 横幅因忙碌 **自动藏**（不是点 ×）；Rise 回 Idle 后横幅再出并可再播鹦鹉。 | **2026-08-03**：接线。**同日用户书面**：点 Reminder 首次见鹦鹉可接受；**后续到期不再播**——已改「每次 hidden→visible 可播」+ 60s 静候再评。欢迎抢播见上行；**工作流根因** §6.10。 **同日晚用户书面**：设未来 1～2 分钟等到期（或切后台再回）→ 横幅+鹦鹉 — **测试 OK**；问 Focusing 隐藏含义——属上列可选邻接，主路径已够。 | — | — | `?product=1` · `e2e/in-app-reminder.spec.js` | 2026-08-03 |
| 场景 B · 鹦鹉稀有彩蛋（轻完成池） | UI可见 | 待人工测试 | **主路径**：微仪式完成或非首日计时完成 → 偶发（约 7%）播鹦鹉。**streak-7**：清 `focus-tiger.milestone-glow.v1` 后连练到第 7 天，约半数见蝴蝶、半数见鹦鹉（可 DEV 多刷）。**实验室捷径**：调试钮强制播鹦鹉 / 里程碑金辉。不必干等真实 7 天。 | **2026-08-03**：池 weight 8 + streak-7 50/50。**同日用户问**：不易测？→ 用清节点 + 调试钮，勿干等。 | — | — | 调试面板 · `__milestoneGlowStore` · Dispatcher 单测 | 2026-08-03 |
