...
| starlight-cloak-sleep / 披斗篷入睡（进 DORMANT） | UI可见 | 待人工测试 | **主路径**：非 DORMANT→DORMANT → `cloakSleep`（`starlight-cloak-sleep` 67 帧 @**12fps**≈5.6s，末帧 067）→ cross-fade → **睡姿**（同源 067→063 双拍 pingpong）。**须验**：① 星光燕麦斗篷飞入披肩再趴下；② 末帧与睡姿循环**姿态连贯、无硬切**。**回流**：已 DORMANT 不重复披斗篷；唤醒后再进 DORMANT → 再播一次。**调试**：实验室可单条试播。**冷启动不播本段**。 | **2026-07-22～25**：旧 cloak-sleep 观感曾书面通过。**2026-08-04**：换装 starlight 素材，须复测观感（旧通过不自动继承）。 | — | — | 改 `focus-session-end` · `#emotion-debug-ui` · `?product=1` | 2026-08-04 |
...
| Sleeping / DORMANT 睡态循环 | UI可见 | 待人工测试 | **自动（live）**：≥2h 回前台 / Rise 后进 DORMANT；披斗篷后睡姿为 **starlight-cloak-sleep 067→063 每帧两拍 pingpong** @ **2 fps**。**冷启动不进睡**。新用户无结束记录不触发。**调试**：「睡着了」→ 同 pingpong。**须分列验**：① 节奏约 2 fps；② 与披斗篷末帧同姿；③ Honesty `starlight-cloak-wake` 可从睡姿接上。 | **2026-07-22～26**：旧 cloak 睡姿曾书面通过。**2026-08-04**：换装 starlight 尾帧，须复测。 | — | — | `?product=1` · DEV 改 `focus-session-end` · `#emotion-debug-ui` | 2026-08-04 |
| DORMANT 2h 滚动触发 + sleep→wake 串联 | UI可见 | 待人工测试 | **单元/控制器集成**：`dormantIdle` chain + smoke `D sleep→wake`（状态机序；**非**观感 DOM）。**人工主路径**：改 `focus-session-end` → live 进睡见星光披斗篷→sleeping → Honesty 选时长 → **卸斗篷苏醒** + 10s 呼吸 → 离 DORMANT / 桥接。 | **2026-07-22**：旧 cloak 串联曾书面通过。**2026-08-04**：换装 starlight 正放/苏醒，须复测串联观感。 | — | — | `npm run test:smoke` · `#emotion-debug-ui` | 2026-08-04 |
...
| dormantWake / Honesty 睡醒序列 | UI可见 | 待人工测试 | **主路径**：进 DORMANT 后「Honesty唤醒」选时长 → **`starlight-cloak-wake` 67 帧正放**（sleep 物理倒序；**12 fps** ≈5.6s）→ 定格末帧 + **约 10s 呼吸**并行。**回流**：定格后桥接 Yes/No。**对比**：「唤醒(伸懒腰)」仍 stretch-reminder。**不**淡入闭眼呼吸、**不**自动接 halo。 | **2026-07-21～22**：旧 cloak 倒放曾书面通过。**2026-08-04**：独立入库苏醒序列，须复测。 | — | — | Honesty / `#emotion-debug-ui` · `playEmotion('dormantWake')` | 2026-08-04 |
