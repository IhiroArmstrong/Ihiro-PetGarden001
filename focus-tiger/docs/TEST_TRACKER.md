...
| | 窄屏 · 主屏三主钮（Sit / Quick Start / Honesty） | UI可见 | 待人工测试 | **主路径（375×667 · `?product=1`）**：Idle 见 `#ft-narrow-home-ctas`——顺序 **Quick Start · Sit with Yin · Honesty**；三个 **PNG 图腾圆球**（`public/icons/`），约 **72×72**；三球 **全宽均匀**（`space-evenly`）。**观感（分列）**：① 三球底色均为**蒲团橙/陶色纹理**（与宽屏 Sit/Rise 主 CTA 同系），**禁止**米白底；② 图腾为**金色**（火焰 / Enso / 捧心），对比可读；③ 硬刷新后 `?v=3` 生效。**Arrival 开着时**仅留 Quick Start 球（Sit/Honesty 藏）。点球 → 同桌面语义。**首次 hints**：`quick-start` / `sit-button` / `honesty-optional`。**抽屉**：不得再出现这三项。**回流**：Rise → 三球再出现且色系仍统一。**宽屏 ≥480**：dock 文案 pill 不变。**DOM**：e2e 锁顺序 + aria-label + Honesty 可点 + 抽屉无三主钮。 | **2026-07-26**：三主钮上屏 → 圆球 SVG → PNG 图腾。**同日晚书面**：Sit 须居中；Honesty 虚/不可点 → 已改；顺序/满色 OK；**三球太近** → `space-evenly` 全宽均分；**Arrival 丢 Quick Start** → keepQuickStart + 375 e2e；**Breath 仍藏 Sit** → `arrival-breath-sit-still-hidden`（宽+375）。 **2026-07-26 用户书面（5176 · 375）**：三主钮顺序/72px/均分/Honesty 满色 — **测试 OK**；仅换图案不重开验收。Arrival 仅留 ⚡ 后点 ⚡ **没反应** → 已修（代理勿因 dock hidden return）。 **2026-07-26 用户书面**：右图三球（侧球米白底）无法使用，须改成与 Rise 蒲团色统一风格的颜色与式样搭配 → 已重着色 Quick/Honesty 为陶色底+金色图腾；**须复测观感①–③**。 | DevTools 375 · `/icons/icon-*.png?v=3` · `#ft-narrow-home-*` | 2026-07-26 |
