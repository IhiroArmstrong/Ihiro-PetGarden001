...
| CI 定时全量 e2e + 环境/密钥隔离基线 | 纯后端 | 仅单元测试覆盖 | **2026-07-31 核实**：PR `test:pr-smoke` 已在 CI 解放本地 Agent（无 Secret）。全量 `focus-tiger-e2e-full`：`schedule`+dispatch 已有；`develop` job=120m，但 **schedule 读 main YAML（仍 60）** → nightly cancelled。当前套件**不**需 Actions API Key。隔离：`ENV_CONFIG.md` + `.env.example`；客户端禁硬编码 Secret；尚无已提交的 `.env.development` / `.env.production`（正确：gitignore）。下一步：同步 workflow 到 `main`（须授权）。 | — | `ENV_CONFIG.md` · `.github/workflows/focus-tiger-e2e-full.yml` · `pr-smoke.yml` | 2026-07-31 |
