...
| 行 | 功能 | 为何在 P0 |
|---|---|---|
| **[L174](#L174)** | Arrival · 轻量气泡 + ⚡ Quick Start | 07-24 修 Sit/Notice 重叠，**须复测** |

| 宽屏 Idle 底栏清场 · Sit + ⚡ + ⋯ Popover | UI可见 | 已通过 | **量级**：跨模块交互 + 用户可感知体验（**非**纯 UI 微调；合并前须走场景人工验收）。**适用范围**：≥480；≤479 仍走窄屏抽屉。**主路径（≥900 或笔记本宽）**：Idle 底栏只见 **Sit · ⚡ · ⋯**；Honesty / 一分钟呼吸 / How / Sound FAB / 提醒时钟**不**常驻。点 ⋯ → 向上菜单 → 各项可打开对应面板/流程。**Arrival**：仅 ⚡；Sit 与 ⋯ 隐藏。**Focusing**：⋯ 隐藏；Sound FAB 回到右下。**回流**：Rise → 再出 Sit/⚡/⋯；菜单内 Sound（Idle 仍 gated tip）/ 提醒开面板 → 点空白关闭 → 再开 ⋯。左下 ? / 热力图不变。**窄屏对照（强制）**：DevTools **375×667**（或 `innerWidth≤479`）须变 ActionBar + 上滑抽屉，**禁止**仍见 Sit·⚡·⋯；**禁止**只靠拖窄桌面窗口当窄屏验收（Chrome/Safari 最窄常仍 ≥480）。**DOM**：e2e `wide-idle-more-menu.spec.js` + 既有 A/I 经 `openCompanionHint`。 | **2026-07-25**：用户书面（Chrome `5174` 拖到最窄附图）——底端仍是 Sit·⚡·⋯，**未见**窄屏抽屉式菜单；问「是不是应该变窄屏」。口径：是，但须 **CSS 宽 ≤479**；图中仍是 ≥480 宽屏清场（⋯ = More，不是坏掉的抽屉）。**同日 §9 窄屏对照人工（Cursor 内置 Browser）**：① **375×667** → ActionBar +「Swipe up for options」、**无**桌面 ⋯ → **测试 OK**；② 拉回 **≥480**（520×720）→ 窄壳消失、Sit·⚡·⋯ 恢复，点 ⋯ 菜单含 Honesty/呼吸/How/Sound/提醒 → **测试 OK**。用户确认 §9 两条记通过。 | `?product=1` · `#ft-wide-more-btn` · `#ft-wide-more-menu` · 宽 ≥900 + **DevTools 375 窄屏对照** | 2026-07-25 |
...
| 宽屏 · ? hint 锚点（park→⋯）完整回归 | UI可见 | 待人工测试 | **保护面**：宽屏 Idle park 后点左下 `?` → How / Honesty / Sound / Reminder / 呼吸 tip **锚在 ⋯ 附近**；另须见 ⚡ / HUD tip + 蓝点；用途卡不挡热力图 tip。**回流**：Focusing 后再点 ?；Rise 后再 Idle。 | **2026-07-25**：缺失 Hints。已补 tip/蓝点/用途卡避让，**须复测**。 | `#onboarding-hint-help` · `#ft-wide-more-btn` · `.ft-hint-discovery-dot` | 2026-07-25 |
| Focusing HUD · ? 补救 / 蓝点提示（缺口） | UI可见 | 待人工测试 | **主路径**：清空 hints → Idle/Focusing 见 HUD 三控件**软钢蓝点**；点 ? → 金环 / 今日同坐条 / 7 点环 tip（说明一日一点非按次）。**回流**：点 tip 关掉 → 蓝点消失；DEV 清空 hints 再出。 | **2026-07-25**：用户要求首次必有蓝点+Hints。已实现，**须复测**。 | `#focus-hud` · `.ft-hint-discovery-dot` | 2026-07-25 |
| **[L227](#L227)** | 「本周陪伴」7 格热力图（场景 **O**） | 07-25 Sound→选曲面板，**须复测** |
| **[L237](#L237)** | 应用内提醒设置 + 横幅（场景 **P**） | 流程 OK；**新文案**须复测 |
...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 待人工测试 | **主路径**：① hint → **Here & Now / Flow** → Arrival → Choose/鞠躬后立刻 Focusing。② hint → **Offline Space** → **立刻 Focusing，不出 Arrival Notice/Choose**。③ Sit → Arrival → 三选一点选即计时。**回流**：Rise 后再 hint。**点外侧**：三选一面板打开时，点框外空白须收起。 | **2026-07-21**：Flow/Reading 鞠躬开表已修。**2026-07-22**：Safari Companion **测试 OK**。**2026-07-25**：外侧关闭已补；**同日书面**——点 Offline 不应出 Arrival（离开语境无 Choose）→ 已改 Offline 跳过 Arrival，请复测。 | 底部 Sit 旁 dock · **DOM** e2e A/I/K · Safari 人工 | 2026-07-25 |
...
| FocusHUD streak-meter 近日同坐 | UI可见 | 已通过 | **主路径**：HUD 右侧小 7 点环；空日全暗；达标/Honesty 记账后多亮一点（非 Day N 计分牌）。悬停见「Recent days… / 近日同坐的日子」。满 7 点短金息。**回流**：重置本地后环回空。禁止断签焦虑文案。**语义**：亮点 = **有练习的自然日**（同日多次仍只亮 1 点），**不是**当日次数。 | **2026-07-22**：A 类测试 OK。**2026-07-25**：用户误以为同日两次应两红点；已用 tip/蓝点说明「一日一点」。 | `#focus-hud streak-meter` · `/?product=1` | 2026-07-25 |
...
| 应用内提醒设置入口 + 横幅 UI | UI可见 | 待人工测试 | **设置入口**：Idle 左下热力图簇旁时钟；**宽屏**经 ⋯ 打开。勾选开启写入 `{ hour, minute }`。**面板内说明（2026-07-25）**：开启后须见 `#reminder-preference-help`（EN「When this time arrives…」/ ZH「到点且今天还没练习时…」）。**横幅**：到点且今日未完成 → 顶部 `reminder.gentle_waiting`；忙碌 **suppress**。**主路径**：开启并设时→见面板说明→（今日未完成）切后台再回→横幅。 | **2026-07-25 宽屏人工（图1）**：设时后无文字框。已补面板内说明，**须复测**。 | `#reminder-preference-help` · `#in-app-reminder-banner` | 2026-07-25 |
...
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 待人工测试 | 1) 清空已读。2) 点 **?**。3) Idle 须见：Sit / How / **热力图全文** / 一分钟呼吸 / **⚡** / Sound gated / **HUD 三 tip** / help-remedy + 用途卡。4) **用途卡不得挡住**热力图 tip。5) 未读 ⚡/HUD 须有**软钢蓝点**；点 tip 后消失。 | **2026-07-24/25**：用途卡压 tip。已加避让 + Quick Start/HUD tips + 蓝点，**须复测**。 | `#onboarding-app-purpose` · `.ft-hint-discovery-dot` | 2026-07-25 |
