...
| FocusHUD 香炉隐喻 / 数字弱化 | UI可见 | 有问题 | **主路径**：开局左上角见小香炉+淡金环，**无**常驻「Focus: N%」计分牌；时长半透明。Sit 开计时后烟/环随进度渐亮，时长加重。**悬停/Tab focus** HUD → 才露出 Focus %。**回流**：Rise 后环/烟回淡、时长回 `00:00`。勿与角色抢视觉重量。 | 2026-07-21：按 UI 分层意见落地香炉隐喻。**同日书面**：① 左上角还有其它参数没办理？② 进度圈中间像葡萄酒、不动，干啥的；若表专注进度建议换 icon，征求高见。待拍板是否换隐喻。**自动化**：`focusHudIncense.test.js` + e2e `#hud-state`/`#hud-time`。 | `#focus-hud` · `/?product=1` | 2026-07-21 |
