...
|---|---|---|---|---|---|---|
| Arrival Practice / Notice「What is present…」点选后 | UI可见 | 待人工测试 | Honesty 桥接 Yes 或 Sit → Notice：点任一状态后，**整屏图标区应收起**，只短暂显示 Yin 观察式短句（约 0.9s），再进呼吸。不应再停在「What is present right now?」+ 六图标不动。 | 2026-07-20：用户反馈点选后框仍停一段时间（多次）。已改：回应期只留短句。请复测。 | `http://localhost:5173/` · Honesty Yes / Sit | 2026-07-20 |
| Arrival Practice / 抵达练习（Welcome → Notice → Breath → Choose） | UI可见 | 待人工测试 | 1) Breath：阿寅推近并保持。2) Choose 点意图：合十期间**不要**缓缓缩小。3) 合十→idle 淡入应对齐重叠；**淡入完成后再**缓慢拉回视距。 | 2026-07-20：用户反馈 idle 缓缓缩放导致跳动 → Dolly 改在合十淡入后再拉回；displayFit 改用布局尺寸。请复测 Choose→idle。 | `http://localhost:5173/` · Sit · DEV：`__arrivalPractice` | 2026-07-20 |
...
| FocusSession + Focus HUD（Sit with Yin / Rise） | UI可见 | 有问题 | Arrival 后选 Here & Now / Flow State → HUD 走动；主按钮应变 **Rise**。演示 **1 分钟**达标→庆祝/摆尾→Reflection。 | 2026-07-19：自动开计时时曾漏切 Rise 文案→已补 `beginFocusing`，请复测。**同日用户反馈**：点 Rise 后页面动画「没有任何变化」。**再反馈**：确认有 Reflection（what did you notice today?），但**必须有角色动画变化**；请从未用上的、开头同为闭目坐禅无背景素材里挑一个接 Rise。 | `#focus-hud` · 演示 1 分钟 | 2026-07-19 |
...
| 候选陪伴手势 · 逐条试播 | UI可见 | 待人工测试 | 调试「入库素材」点：`tea-drinking` / `yawn-stretch` / `ear-wiggle` / `blink-breathe` / gaze-p* —— **应直接播该条并定格末帧**；**不应**先闪一下闭目坐禅。张望组合用「组合试播」张望A/B。 | 2026-07-20：用户反馈「Idle 变体链强制」会在闭目帧一闪，误以为抠图问题→根因是 `restart idle` 后再播变体；已改为离开 Idle 后直播、定格、不回呼吸。 | `#emotion-debug-ui` · 入库素材 / 组合试播 | 2026-07-20 |
| blink-breathe 眨眼深呼吸 | UI可见 | 待人工测试 | 入库素材点「blink-breathe 眨眼深呼吸」→ 13 帧 → 定格。Rise 候选，**尚未接线**。 | 2026-07-20：入库并纳入调试面板。 | `#emotion-debug-ui` | 2026-07-20 |
| MilestoneGlow 备选 breath-halo-hq | UI可见 | 待人工测试 | 调试「金环呼吸(HQ备选)」→ 16 帧闭目+金环扩展 → 回 idle。确认不再出现旧 `breath-halo-expand`（17 帧）路径。主候选「里程碑金辉」仍为含蝴蝶版。 | 2026-07-20：HQ 替换 expand；旧帧归档 `art-reference/sprites-archived/`。 | `#emotion-debug-ui` · manifest `breathHaloHq` | 2026-07-20 |
