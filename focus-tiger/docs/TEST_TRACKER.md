...
| FocusSession + Focus HUD（Sit with Yin / Rise） | UI可见 | 待人工测试 | **主路径**：Sit → **Skip — begin** → 立刻计时且按钮变 **Rise**（记忆 Companion 模式）。Choose 完整走完 → 右侧 Companion → 选模式开计时。**回流**：Rise → Reflection → 再 Sit。 | 2026-07-20：Skip begin 半卡 Sit→已改为直接开始；请复测主路径+回流。**DOM**：e2e A 锁 Arrival→Here & Now 开表；e2e **A2/A3** 锁预选+Skip — begin 开表（**无需**再点 Sit）。 | `#focus-hud` · `#btn-focus` | 2026-07-20 |
...
| 「?」朱砂 notification-badge | UI可见 | 已通过 | **（2026-07-23 交互改版）** `help-affordance` 改为 **click** 薄荷绿脉冲圆点（`tone="hint"`），不再挂「?」内朱红角标。**主路径**：清空已读 →「?」旁见薄荷绿圆点脉冲 → 点圆点展开 tip；或点「?」进补救并记已读。 | 2026-07-21：稀缺强调色。**2026-07-22**：用户书面保留朱砂表示未读。**2026-07-23**：onboarding click 线索改薄荷绿（避免与错误/警告朱红混淆）；朱红仍留给真正通知。 | `#onboarding-hint-help` · `.onboarding-hint-badge` · `/?product=1` | 2026-07-23 |
| Onboarding hints · click 圆点触发 | UI可见 | 待人工测试 | **点击前**：清空已读 → Idle 应见 Sit **主动气泡** + How shall we sit? / Sound fab / 热力图 / 微仪式 /「?」旁 **薄荷绿脉冲圆点**（非朱红、非主动气泡）。**点击后**：点任一圆点 → 展开原薄荷绿 tip；再点圆点或点外部 → 收起且圆点仍在；点气泡本身 → 记已读、圆点消失。**回流**：FOCUSING 时 Rise 为圆点、`ambient-soundscape` 仍**主动**弹出「默认有音乐」。实验室「清空引导提示已读」后圆点复现。 | — | `?product=1` · `.onboarding-hint-badge` · `ONBOARDING_HINTS.md` §〇 | 2026-07-23 |
| Hints · Registry triggerMode + ambient anchorGroup | 仅单元测试覆盖 | 仅单元测试覆盖 | `onboardingHintRegistry.test.js`：`triggerMode` 派生 1:1；click/auto 定稿清单；**`anchorGroup: ambient` selector 仍互异**（`ambient-gated`=click / `ambient-soundscape`=auto，锚点未改）。`npm run hints:doc-check` 机器块含 `triggerMode` 列。 | **2026-07-23**：圆点交互；ambient 回归锁未改 selector。 | `onboardingHintRegistry.js` · `hints:doc-check` | 2026-07-23 |
...
| 应用内提醒设置入口 + 横幅 UI | UI可见 | 待人工测试 | **设置入口**：Idle 左下热力图簇旁的小型时钟图标（`ReminderPreferenceUI`）。仅 Idle 可见；点开轻量面板 → 勾选「开启提醒」写入 `{ hour, minute }`；取消 → 清空存储。**横幅**：`#ui-overlay` 顶部居中非模态横幅（`InAppReminderBannerUI`），到点且今日未完成时出现文案 `reminder.gentle_waiting`（EN "Yin is waiting" / ZH「阿寅在等你」），可点 × 关闭。**主路径**：点热力图旁时钟→开启提醒并设时间→切后台再回前台→横幅出现。**回流**：关闭横幅后本页会话内不再出现；完整刷新 / 新开 App 后若条件仍满足，可再次出现。**忙碌期策略（已拍板）**：Arrival / Focusing / Celebrate / Reflection / 微仪式进行中 → **`suppress`**（隐藏、不排队；**不做** `defer` 延迟弹出）。`main.js` 固定 `busyPolicy: 'suppress'`；`defer` 代码/单测可保留作对照，产品路径不得启用。DEV：`window.__inAppReminder.{sync,setNow,clearNow,controller,settings,banner}`。 | 2026-07-22：入口从右上改到热力图簇旁；**DOM** e2e `in-app-reminder.spec.js`（入口面板 + 设时→回前台→横幅→关闭不重复 + Focusing suppress 隐藏；**非**刷新后再出现、**非**负例、**非** defer）。**2026-07-23 用户拍板**：忙碌策略用 **suppress**（专注等忙碌期隐藏横幅，不做 defer）。 | Idle 左下 `#weekly-practice-heatmap-cluster` · `#reminder-preference-toggle` · `#in-app-reminder-banner` · e2e `in-app-reminder.spec.js` · DEV `__inAppReminder` | 2026-07-22 |
...
| 人工 · help-affordance 尖角对准 ? | UI可见 | 待人工测试 | 1) 清空引导提示已读。2) 点「?」旁**薄荷绿脉冲圆点**展开 `help-affordance`（或点 ? 进补救）。3) 气泡在 **? 右侧**，尖角对准 ?，薄荷绿。4) 回流：缩放后再看。 | 2026-07-20：尖角未对准。**2026-07-21**：恢复绿色式样。**2026-07-22**：与 Hints 薄荷绿批次一并 **测试 OK**。**2026-07-23**：改为 click 圆点后再展开，请复测尖角。 | `?product=1` 或 `/` · `#onboarding-hint-help` · `.onboarding-hint-badge` | 2026-07-23 |
