...
| 窄屏 · 主屏三主钮（Sit / Quick Start / Honesty） | UI可见 | 待人工测试 | **主路径（375×667 · `?product=1`）**：Idle 见 `#ft-narrow-home-ctas`——三个 **PNG 图腾圆球**（`public/icons/icon-sit-with-yin.png` / `icon-quick-start.png` / `icon-honesty-checkin.png`），约 64×64；点球 → 同桌面语义（逻辑未改）。**首次 hints**：`sit-button` / `quick-start` / `honesty-optional` 可点气泡消除。**抽屉**：不得再出现这三项。**回流**：Rise → 三球再出现。**边距**：资产约 410 正方形、圆球自带留白；64px 下观感若偏空/偏挤请书面反馈。**宽屏 ≥480**：dock 文案 pill 不变。**DOM**：e2e 锁 aria-label + 抽屉无三主钮。 | **2026-07-26**：三主钮上屏 → 圆球 SVG → **同日晚**入库正式 PNG 图腾（弃内联 SVG）。 | DevTools 375 · `/icons/icon-*.png` · `#ft-narrow-home-*` | 2026-07-26 |
