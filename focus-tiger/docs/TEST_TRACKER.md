...
| FocusSession + Focus HUD（Sit with Yin / Rise） | UI可见 | 待人工测试 | Choose Reading → Companion → Rise；Rise 后角色动画 + Reflection。 | 2026-07-20：用户将另挑更合适的 Rise 动画（当前仍为 blink-breathe）。 | `#focus-hud` | 2026-07-20 |
...
| IdleOrchestrator / 坐禅闭眼 | UI可见 | 待人工测试 | 点「坐禅闭眼」→ **仅**呼吸×5→眨眼往复；**眨眼切入一刻不应闪一下**（溶解期定格首帧）。加速：`__idleOrchestrator.setTiming({breathCyclesBeforeBlink:1})`。 | 2026-07-20：用户反馈默认 Idle 眨眼切换又闪——根因是 breath×5→blink 重写时加了 crossfade 却**未** freezeUntilCrossFadeEnds（溶解期新序列跑帧）；另 play() 先把 opacity 拉回 1。已修。 | `#emotion-debug-ui` · `__idleOrchestrator` | 2026-07-20 |
