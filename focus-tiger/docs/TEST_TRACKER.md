...
| Idle 突然东张西望 | 已通过 | 已关随机池；无自动调度单测 | **[L119](#L119)** `调试面板 · 全入库素材` · **[L137](#L137)** `idle / 坐禅闭眼呼吸基底` |
| 靠近自动点头 | 已通过 | 行为已拆；无 e2e | **[L136](#L136)** `PointerInteraction · 靠近点头 nodGreeting` |
...
**§B 未单列、但在场景 checklist 里测的项**（见 **L178–L185**）：**[L185](#L185)** A1 Idle 开局（**已通过**） · **[L183](#L183)** Celebrating / 同日 SessionComplete 观感（**已通过**） · **[L184](#L184)** Honesty 桥接完整 Arrival（**已通过**） · DEV 一键重置（**L-logic / 仅单元测试**）。
...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 待人工测试 | **主路径**：① hint → 选 Here & Now / Flow → Arrival → **Skip — begin 或一步步 Choose/鞠躬后应立刻 Focusing + 计时**，**无需**再点 Sit。② Sit → Arrival → Companion → 选 Here & Now / Flow 点选即计时；Offline 再 Sit。**回流**：Rise 后再 hint → 选 Here & Now / Flow → Arrival 结束仍应自动开表。 | **2026-07-21**：用户书面「点 Flow / Reading / Yin 鞠躬后仍不开 timer…」→ 已修 `pendingAutoStartMode`。**2026-07-21 用户书面**：hint→Flow/Here&Now→Arrival→Skip begin 或一步步鞠躬后立刻 Focusing+计时，无需再点 Sit，**测试 OK**。**自动化**：e2e A/A2/A3/I/I2/K。 | 底部 Sit 旁 dock · DEV：`__companionModePicker` | 2026-07-21 |
| Honesty Check-in / Mindful Check-in | UI可见 | 待人工测试 | **主路径**：零完成开局 → **Idle 闭目坐禅**（不是睡着）+ 可忽略提示（EN 含 `sitting with you` / `Quiet time elsewhere`；ZH 含「闭目同坐」「别处的静心」）。点提示 → 选时长 → **呼吸引导**（不播 dormant-wake）→ 桥接。**回流**：同日再补登走空闲 Mindful Check-in → 呼吸 → 再出桥接。调试「睡着了」仍可试 Sleeping→dormant-wake。 | **2026-07-21**：用户书面——登录后第一幕不能是睡觉模样（not uplifting），须 Idle 闭目坐禅。已改零完成默认 Idle。请硬刷新 `?product=1` 复测。 | 零完成自动 · `#honesty-idle-entry` · DEV：`__honestyCheckIn` | 2026-07-21 |
...
| Ambient Soundscape / 静音图标 + Sound | UI可见 | 已通过 | **静音**：右上米色圆形 **音符钮**（在播=音符+斜杠，点一下即静音）。**Sound**：右下蒲团橙 **Sound** **始终可见**；**Sit 开计时后**可展开选曲/音量；未专注点 Sound 会提示先开始专注。**主路径**：默认 Mer-Ka-Ba；音符静音/恢复；专注后 Sound 换曲；**Yin idle 闭目坐禅可见**。**回流**：关→刷新仍关；Rise 后 Sound 仍可见但不可开面板。 | **2026-07-20**：原「Ambient Sound 入口」书面测试 OK。**2026-07-21**：整页空白已修（`_volume` 初始化）。**同日晚**：用户确认原 8 条独立行批次全部关闭（含 Ambient 观感验收）。 | 右上 `.ambient-soundscape__mute` · 右下 `.ambient-soundscape__fab` | 2026-07-21 |
...
| FocusHUD 金环+呼吸光 / 数字弱化 | UI可见 | 待人工测试 | **主路径**：左上角约 **2×** 大卡：金环（琥珀金、够显眼）+ 中心光点**持续一张一缩**；Sit 后环随进度走。**悬停**露 Focus %。**回流**：Rise 后环回淡。 | 2026-07-21：香炉误读→改金环+光点。**同日书面**：①圈/点看不清 ②呼吸点应不停一张一缩 ③整体太小、老花怕看不清、建议放大两倍 → 已加对比、加强呼吸动画、约 2×。请硬刷新 Focusing 复测。**自动化**：`focusHudHalo.test.js` + e2e `#hud-state`/`#hud-time`。 | `#focus-hud` · `/?product=1` | 2026-07-21 |
| FocusHUD 今日同坐 progress-bar | UI可见 | 待人工测试 | **主路径**：HUD 下方见蒲团橙软条 + 文案「Today's shared sitting / 今日同坐」；空日接近空；Sit 计时中条渐长且有**轻脉冲**。**回流**：Rise 后脉冲停；若本场未达标完成则条回落至已完成分钟。勿与 Companion 三选一抢「怎么坐」。**非** daily-quest 清单。 | 2026-07-21：UI Kit progress-bar 纳入产品壳。**自动化**：`sharedSittingProgress.test.js`；观感须人工。 | `#focus-hud progress-bar` · `/?product=1` | 2026-07-21 |
| FocusHUD streak-meter 近日同坐 | UI可见 | 待人工测试 | **主路径**：HUD 右侧小 7 点环；空日全暗；达标/Honesty 记账后多亮一点（非 Day N 计分牌）。悬停见「Recent days… / 近日同坐的日子」。满 7 点短金息。**回流**：重置本地后环回空。禁止断签焦虑文案。 | 2026-07-21：UI Kit streak-meter 纳入。**自动化**：`PracticeDaysStore.test.js`。 | `#focus-hud streak-meter` · `/?product=1` | 2026-07-21 |
| 「?」朱砂 notification-badge | UI可见 | 待人工测试 | **主路径**：新用户左下「?」角有朱砂小红点；点一次「?」后红点消失且不再常驻。**回流**：DEV 重置 hints 后再见红点。勿做成常驻角标噪音。 | 2026-07-21：稀缺强调色。 | `#onboarding-hint-help` · `/?product=1` | 2026-07-21 |
| How shall we sit? secondary 米色立体钮 | UI可见 | 待人工测试 | Sit 仍为蒲团橙主 CTA；旁「How shall we sit?」为**米色立体次要钮**（渐变暖米底＋凸起阴影），不透明，不抢 Sit。Honesty Bridge Yes/No 仍同级（勿改成主次）。 | 2026-07-21：用户书面——「How shall we sit?」背景透明不对，应改米色立体按钮；已改 CompanionModePicker.js。 | `.session-start-dock__hint` · `#btn-focus` | 2026-07-21 |
...
| Sleeping / DORMANT 睡态循环 | UI可见 | 待人工测试 | **仅调试**「睡着了」→ `sleeping` 8 帧循环（**1 fps**）。零完成开局**不应**再自动睡觉。 | **2026-07-21**：用户书面——开场睡觉 not uplifting → 已取消零完成自动 Sleeping。节奏 1 fps 仍可在调试复测。 | `#emotion-debug-ui` 「睡着了」 | 2026-07-21 |
...
| Lit 试点 · OnboardingHintsUI（步 4） | UI可见 | 待人工测试 | **主路径**：实验室「清空引导提示已读」→ 见 `help-affordance`（Lit **薄荷绿**气泡）。点 **?** → 本页全部锚点提示 + **App 用途简介卡**（标题/正文/知道了）。**回流**：关气泡后再点 ?；Rise 后 FOCUSING 再点 ?。 | 2026-07-21：Lit 试点。**同日书面**：① 恢复原先绿色式样（已从奶油色改回薄荷绿）② 点 ? 除 hints 外应跳出简介「能帮你做什么」→ 已加 `#onboarding-app-purpose`。请硬刷新复测。 | `#onboarding-hint-help` · `#onboarding-app-purpose` · Brief | 2026-07-21 |
| 分散式即时提示 + 「?」补救（ONBOARDING_HINTS v3） | UI可见 | 待人工测试 | **主路径**：清空已读 → 左下 **?** 旁 `help-affordance`（**薄荷绿**）。点 ? → 本页 hints + 用途简介卡。**回流**：关卡后再点 ?。 | 2026-07-20：尖角/补救反馈。**2026-07-21**：恢复薄荷绿 + 用途简介卡；请复测。 | `#onboarding-hint-help` · 实验室「清空引导提示已读」 | 2026-07-21 |
| 人工 · help-affordance 尖角对准 ? | UI可见 | 待人工测试 | 1) 清空引导提示已读。2) `help-affordance` 在 **? 右侧**，尖角对准 ?，气泡为**薄荷绿**（非奶油米黄）。3) 回流：缩放后再看。 | 2026-07-20：尖角未对准。**2026-07-21**：用户要恢复绿色式样 → 已恢复。 | `?product=1` 或 `/` · `#onboarding-hint-help` | 2026-07-21 |
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 待人工测试 | 1) 先点掉自动提示。2) 点 **?**。3) 须同时见：本页控件 hints + `help-remedy` + **用途简介卡**（EN 含 gamified mindfulness companion / regular practice, at your own pace；ZH 对应）。4) 点「Got it / 知道了」关简介卡；点气泡关该条。5) 回流：再点 ? 仍全部出现。 | 2026-07-20：只有元文案。**2026-07-21**：加简介卡；同日用户定稿定位句（略改 daily→regular + at your own pace）。请硬刷新复测。 | `#onboarding-hint-help` · `#onboarding-app-purpose` | 2026-07-21 |
...
| 浏览器 e2e 场景 A/I/K Companion DOM（Playwright） | 纯后端 | 仅单元测试覆盖 | `npm run test:e2e`：**I** hint 开面板；**I2** 预选开 Arrival；**A** Arrival→Here&Now 开表；**A2/A3** 预选 Flow/Here&Now → Skip begin **无需 Sit** 开表；**K** Offline 须再 Sit。 | 2026-07-21：补 A2/A3 锁「预选后 Arrival 结束自动开表」。 | `e2e/scenario-a.companion.spec.js` | 2026-07-21 |
| 人工 · A1 Idle 开局（非 Sleeping） | UI可见 | 已通过 | 1) 实验室「重置全部本地状态」。2) 开 `?product=1`。3) 确认阿寅是 **Idle 闭目坐禅**，**不是**睡着。4) Honesty 可忽略提示可见。5) 右下角见 **关闭音乐**（默认有声）。 | **2026-07-21**：用户书面——第一幕不能睡觉；须 Idle + 默认音乐。旧「A1 睡着已通过」口径作废。**同日晚**：用户确认原 8 条独立行批次全部关闭（含本行 Idle 开局）。 | `?product=1` · 重置按钮在 `/` | 2026-07-21 |
...
| 人工 · 静音图标 + Sound | UI可见 | 已通过 | 1) `?product=1` 重置后进入。2) **右上**音符钮静音/恢复。3) **右下 Sound 始终可见**（未专注略淡）。4) **Sit 开计时** → 点 Sound 展开选 Mer-Ka-Ba / Meditation Impromptu + 音量。5) Rise 后面板应收起、Sound 仍可见。 | **2026-07-20**：原「Ambient Sound 入口」书面测试 OK。**2026-07-21**：Sound 不见已修。**同日晚**：用户确认原 8 条独立行批次全部关闭（本行承接原 Ambient 入口验收）。 | `?product=1` | 2026-07-21 |
