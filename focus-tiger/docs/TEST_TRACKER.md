...
| Arrival · 轻量气泡 + ⚡ Quick Start（去 Skip 双钮） | UI可见 | 待人工测试 | **对照** `ARRIVE_MOMENT_DESIGN.md` v2：Notice/Choose 为点图标+观察式短句，**非**重型模态。**本次**：去掉 Arrival 内 Skip / Skip — begin；UI 改为透明气泡/字幕+轻图标格；Sit 路径仍为 Welcome→Notice→Breath→Choose；快速开表改 **⚡ `#quick-start-focus`**（记忆 Companion 模式立刻 Focusing）。**主路径**：Sit → 见轻气泡（非大卡片）→ Notice 点选 → 短句 → Breath → Choose → Focusing。**快速路径**：点 ⚡ → 立刻 Rise/Focusing（Arrival 开着或 Idle 皆可）。**回流**：Rise 后再 Sit 仍走完整 Notice/Breath；⚡ 再开表。**非** Reflection 的 Skip/Skip all（反思三问仍保留）。 | **2026-07-24 用户书面**：① Notice/Breath 本属 Sit 路径（勿写成「默认跳过」）；② 测试感若成「弹窗1→2→3+Skip」= 实现偏离设计，须改回轻量气泡并移除 Skip 双钮，以 ⚡ 替代快速开始。 | `?product=1` · `#arrival-practice.arrival-practice--bubble` · `#quick-start-focus` · e2e A/A2/A3/K | 2026-07-24 |
