...
| **[L228](#L228)** | 「本周陪伴」7 格热力图（场景 **O**） | 07-25 375：**有问题**（图1–5）；用户书面 **正在修理** |
| **[L238](#L238)** | 应用内提醒设置 + 横幅（场景 **P**） | 07-25 reminder 新文案 **测试 OK** |
| **[L257](#L257)** | 用户场景剧本 SCENARIO_TESTS | 本轮 **C / P 测试 OK**；**O 有问题**（见 L228） |
| **[L183](#L183)** | Honesty 桥接 CTA 叠层 | 07-25 **测试 OK** |
| **[L250](#L250)** | 门闩一体包 · Companion 点选→开表 | 07-25 主路径 + Rise 回流 **测试 OK** |
...
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 有问题 | 1) 先点掉自动提示。2) 点 **?**。3) Idle 须同时见：Sit / How shall we sit? / **热力图** / **应用内提醒时钟**（`in-app-reminder`）/ **一分钟呼吸**（入口可见时）/ **Sound gated** / `help-remedy`（含 EN「Click a tip to dismiss… tap ? anytime…」）+ **用途简介卡**。4) Honesty 桥接打开时点 ? → 须见 **honesty-bridge** tip + 热力图/提醒/Sound（**不见** micro-ritual）。5) 点气泡关该条；再点 ? 仍全部出现。 | 2026-07-20：只有元文案。**2026-07-21**：加简介卡。**2026-07-22**：A 类测试 OK 后用户再书面缺 tip → **已补登记**四类 + 更新 help-remedy；请硬刷新复测。**2026-07-25**：补 `in-app-reminder`（每日时分），**同日用户书面（5174 · 场景 O）**：点 ? 一大群 hints 乱指不存在的按钮。 | `#onboarding-hint-help` · `#onboarding-app-purpose` · `#reminder-preference-toggle` | 2026-07-25 |
...
| 用户场景剧本 SCENARIO_TESTS（A–H + I–P） | UI可见 | 待人工测试 | 权威：`focus-tiger/docs/SCENARIO_TESTS.md`。用 **`?product=1`** 走完整故事串。**新增 O**（7 格热力图）· **P**（应用内提醒 + suppress/defer 说明）。**单元/控制器** `npm run test:smoke`；**DOM 用户链路** `npm run test:e2e`（覆盖层见文首 §A 与 SCENARIO 各场景标题——**全绿 ≠ 故事走完**）。**观感子项已拆成下方独立行，勿只勾本行。** | 2026-07-22：新增正式场景 O/P。**2026-07-25 用户书面（5174 · P0）**：场景 **C**（中途 Rise）**测试 OK**；场景 **P** **测试 OK**；场景 **O** **有问题**（见 L228 热力图行）。 | `SCENARIO_TESTS.md` · `?product=1` | 2026-07-25 |
