...
| Soft-schedule 路径 B（本地 SSOT + CloudConfigClient） | 纯后端 | 仅单元测试覆盖 | `npm run test:smoke`（`softScheduleConfig.test.js` / `CloudConfigClient.test.js`）：Celebrating 50/50、全 0→回退、按日稳定 messageKey、local 不 fetch、remote 失败回退。无 UI。契约：`CLOUD_CONFIG_V1.md`。 | — | — | 2026-07-29 |
| Cloudflare Workers API 骨架（cloud/ stub · 路径 B 未接线） | 纯后端 | 待人工测试 | **可选**：`cd focus-tiger/cloud && npm ci && npm run typecheck`；`npm run dev` → curl（形状见 `cloud/README.md`：`messageKey` + `variants[]`）。**产品主路径不测**（未 remote）。限流 429 仍可人工看。 | 2026-07-29：路径 B——不挡产品；CI typecheck 防 stub 过期。 | `http://127.0.0.1:8787` · `CLOUD_CONFIG_V1.md` | 2026-07-29 |
