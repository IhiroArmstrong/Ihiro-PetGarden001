...
| Lit 试点 · OnboardingHintsUI（步 4） | UI可见 | 待人工测试 | **主路径**：实验室「清空引导提示已读」→ 见 `help-affordance`（Lit 气泡）。点 **?** → `help-remedy` + 本页全部锚点提示。**回流**：关气泡后再点 ?；Rise 后 FOCUSING 再点 ?（Rise+Sound）。尖角对准见下行。实现：`ft-onboarding-hint-bubble` + `lit`。 | 2026-07-21：试点目标 OnboardingHints；代码已落地，请复测。 | `#onboarding-hint-help` · `ft-onboarding-hint-bubble` · Brief | 2026-07-21 |
