...
| Hints 接线表 SSOT（`HINTS_WIRING`） | 纯文档 | 仅单元测试覆盖 | **无运行时产品行为变更**（本轮校文档 + 契约单测）。`HINTS_WIRING.md` + `HINT_WIRING_BATCH_CLUSTER` 库存硬闸 + PR 批次钉。**③ 已完成（2026-08-03）**：簇 A 全流程（改接线表 → cluster → registry 注释 → 宽窄路径指既有 e2e → TRACKER）→ **可宣称接线 SSOT 格式已真实编辑检验**。单元：`hintsWiringClusterA.test.js` + `hintsWiringDocCheck`。宽窄/补救仍靠既有 e2e（`wide-idle-more-menu` / `onboarding-remedy-contract` / `micro-ritual`）。**仍 Backlog**：④ 视觉快照；⑤ viewport-context 解耦。 | **2026-08-03** 用户书面：合理则办——单独立项 HINTS_WIRING。 **同日分析师**：骨架≠生效；须簇 A 跑通才可宣称格式检验。 **同日再书面**：同意用簇 A 走完全流程。 | — | — | `docs/HINTS_WIRING.md` · `hintsWiringClusterA.test.js` · `scripts/hints-doc-check.js` | 2026-08-03 |
