...
| 窄屏 · 主屏三主钮（Sit / Quick Start / Honesty） | UI可见 | 待人工测试 | **主路径（375×667 · `?product=1`）**：Idle 见 `#ft-narrow-home-ctas`——顺序 **Quick Start · Sit with Yin · Honesty**；三个 **PNG 图腾圆球**（`public/icons/`），约 **72×72**（PNG 已收紧内边距）。点球 → 同桌面语义。**Honesty 不得因 `#honesty-idle-entry` 尚未创建/attribute-hidden 而 `disabled`+淡化**（壳 `display:inline-flex` 曾盖过 UA `[hidden]`，会留下虚影）。**首次 hints**：`quick-start` / `sit-button` / `honesty-optional`。**抽屉**：不得再出现这三项。**回流**：Rise → 三球再出现。**宽屏 ≥480**：dock 文案 pill 不变。**DOM**：e2e 锁顺序 + aria-label + Honesty 可点 + 抽屉无三主钮。 | **2026-07-26**：三主钮上屏 → 圆球 SVG → PNG 图腾。**同日晚书面**：Sit 须居中；Honesty 虚/不可点（实为门闩同步误伤 + `[hidden]` 被盖）→ 已改顺序 / 常亮可点 / 72px+收紧导出。 | DevTools 375 · `/icons/icon-*.png` · `#ft-narrow-home-*` | 2026-07-26 |
