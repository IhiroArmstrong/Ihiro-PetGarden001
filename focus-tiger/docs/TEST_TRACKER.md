...
| 浏览器 e2e harness · storage wipe + preview webServer | 纯后端 | 仅单元测试覆盖 | **2026-07-26 根因**：`openFreshProductShell` 曾在**每次**导航 wipe `focus-tiger.*`，heatmap seed→`reload()` 后种子被清 → `expected lit for YYYY-MM-DD`（日期来自 `shift(-6)` 相对日，**非**硬编码产品日、**非**产品 lit 逻辑错）。已改为 **每 browser context 只 wipe 一次**。全量套件用 **production `vite preview`**（避 `vite dev` 挂死）。e2e hook 须生产构建暴露：`__honestyBridge` / `__inAppReminder` / `__dailyCompletionStore` / `__companionModePicker`。实验室 `#dev-reset-all-local-state` 仅 vite serve 有；preview 下该用例 skip。窄屏 catalog tip 抬升：`_liftBubblesAboveNarrowHomeCtas` + e2e poll。覆盖层：**DOM e2e** harness；**不**锁序列观感。 | — | `e2e/helpers/product-shell.js` · `playwright.config.js` · `src/main.js` · `OnboardingHintsUI.js` | 2026-07-26 |
