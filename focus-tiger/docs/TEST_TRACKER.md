...
| Honesty 桥接 CTA（补登后邀请再坐） | UI可见 | 待人工测试 | **主路径**：走完 Honesty（选时长 → 呼吸+坐起 → thanks）→ 约 3.2s 后出现「要不要现在也坐一会儿？」Yes/No 同级。点 **Yes** → 完整 Arrival（Welcome→Notice→Breath→Choose）→ Companion，**不得**直接开 timer/Ambient。点 **No** → idle，无二次挽留。**回流**：同日再走 Honesty（或清完成后再补登）→ **不应**再出桥接；Sit / hint 可取消尚未出现的邀请。 | 2026-07-19：拍板见 `HONESTY_BRIDGE_CTA.md`（加桥接、非合并）。 | `http://127.0.0.1:5173/` · DEV：`__honestyBridge` / `__honestyBridgeStore` | 2026-07-19 |
...
| 分散式即时提示 + 「?」补救（ONBOARDING_HINTS v3） | UI可见 | 待人工测试 | 1) `?product=1` 硬刷新（或实验室点「清空引导提示已读」）。2) DORMANT：应见休眠提示气泡。3) Sit→Arrival 各步（Notice/呼吸/Choose）各有一句。4) Companion 展开有提示。5) 计时中见 Rise 提示。6) Reflection 有提示。7) 左下「?」在任意步可复述当前场景句。8) 再次走同一路径：自动提示不再出现，「?」仍可用。 | — | `ONBOARDING_HINTS.md` · `/?product=1` · 实验室「清空引导提示已读」 | 2026-07-19 |
