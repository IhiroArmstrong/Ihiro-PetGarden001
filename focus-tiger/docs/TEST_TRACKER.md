...
| Ambient Soundscape / 静音图标 + Sound | UI可见 | 待人工测试 | **静音**：右上米色圆形 **音符钮**（在播=音符+斜杠，点一下即静音）。**Sound**：右下蒲团橙 **Sound** **始终可见**；**Sit 开计时后**可展开选曲/音量；未专注点 Sound 会提示先开始专注。**主路径**：默认 Mer-Ka-Ba；音符静音/恢复；专注后 Sound 换曲。**回流**：关→刷新仍关；**Rise / 达标结束 → 自动停播**（不抹存储偏好，下次 Sit 可再按偏好开）；Sound 仍可见但 Idle 不可开面板。 | **2026-07-20**…**2026-07-21 晚**：开关 OK。**2026-07-25**：用户拍板 Rise 后**自动停播**；**须复测**。 | 右上 `.ambient-soundscape__mute` · 右下 `.ambient-soundscape__fab` · `AmbientSoundscapeController.test.js` | 2026-07-25 |
...
| 「本周陪伴」7 格热力图 UI（Idle 常驻） | UI可见 | 待人工测试 | **主路径**：`?product=1` Idle → 见 7 格。**宽屏 ≥480**：左下 `?` 上方；底栏 Sit · ⚡ · ⋯（次要经 ⋯）。**窄屏 ≤479 / 375**：ActionBar（? · Calm/time · ♪）+ Yin + **上滑抽屉**——Sit / Quick Start / **Honesty 必须在列** / 呼吸 / How / **Sound（未专注→门闩提示；后续应对齐选曲面板）** / **提醒（开设置面板）**；抽屉钮紧凑半高。**Focusing**：左上 `#focus-hud`；Sound FAB 藏、右上 mute 可关。**回流**：Rise → grabber；♪ 静音。**DOM**：e2e `weekly-practice-heatmap.spec.js`（Honesty 常列 + Sound/Reminder）。 | **2026-07-24**：375 书面①–⑦。**2026-07-25**：Honesty/提醒路径对齐 b2342c0；宽屏合入后须分列复测。 | `?product=1` · DevTools 375×667 · `#ft-narrow-idle-shell` · 宽屏 `#ft-wide-more-btn` | 2026-07-25 |
...
| Hints · 默认音乐提示锚右上 mute（非右下 Sound） | UI可见 | 待人工测试 | **主路径**：`?product=1` 重置 hints → Idle 或开计时后见 `ambient-soundscape` 文案（EN「Music is on by default…」/ ZH「默认有音乐…」）→ 气泡**尖角须指右上音符钮** `.ambient-soundscape__mute`，**不得**指右下 Sound。**回流**：点「?」补救同页两条：music→右上 mute；gated→右下 Sound（Idle 未专注时点 Sound 才见 gated 句）。 | **2026-07-22**：用户书面——「Music is on…」Hint 错指右下 Sound；原本 Sound 应有 gated 句。根因：两 hint 共用 `.ambient-soundscape__fab` 自 2026-07-20。已改 mute 锚点 + `onboardingHintAnchors.test.js`。 | `?product=1` · `.ambient-soundscape__mute` · `.ambient-soundscape__fab` | 2026-07-22 |
