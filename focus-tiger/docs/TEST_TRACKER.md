...
| Ambient Soundscape / 静音图标 + 专注 Sound | UI可见 | 待人工测试 | **静音**：右上米色圆形 **音符钮**（在播=音符+斜杠，点一下即静音）。**Sound**：**仅 FOCUSING** 时右下蒲团橙 **Sound** → 曲目/音量（恢复旧版）。**主路径**：默认 Mer-Ka-Ba；音符静音/恢复。**回流**：关→刷新仍关；Sit 后见 Sound。 | **2026-07-21**：用户书面——Turn music off 仍关不掉；要恢复专注 Sound；要米色图标换位置（非文字钮）。已修 DOM audio + `toggleFromUi` + UI 拆分。请硬刷新。 | 右上 `.ambient-soundscape__mute` · 专注 `.ambient-soundscape__fab` | 2026-07-21 |
...
| 人工 · 静音图标 + 专注 Sound | UI可见 | 待人工测试 | 1) `?product=1` 重置后进入。2) **右上**米色音符钮：在播时有斜杠，点一下立刻静音。3) 再点恢复。4) **Sit 开计时** → 右下出现 **Sound**，点开可选曲目/音量。5) 未专注时 Sound **不应**出现。 | **2026-07-21**：旧右下角文字钮作废；改右上静音 + 专注 Sound。 | `?product=1` | 2026-07-21 |
