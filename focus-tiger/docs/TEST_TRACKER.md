...
| MilestoneGlow · 琉璃星石变体（meditation-star-reward） | UI可见 | 待人工测试 | **主路径**：连续练习至第 **21**（或 **100**）天 → 播 `meditation-star-reward`（闭目坐禅 + 空中发光琉璃星石），**非** Celebrating 舞、**非** 金辉蝴蝶。同节点只播一次。调试：「入库素材」点 `meditation-star-reward` 可单播定格。**回流**：节点已播后再达标 → 正常 Celebrating/SessionComplete。单元：`pickMilestoneGlowVariant` + manifest `milestoneGlowStar`。 | **2026-08-03**：用户书面同意 MilestoneGlow 变体池 + 入库根目录 `Yin_Meditation_Star_Reward_transparent` → `meditation-star-reward`（63 帧 @6fps）。 | — | — | `?product=1` · `#emotion-debug-ui` · `public/sprites/.../meditation-star-reward/` | 2026-08-03 |
