...
| Honesty Check-in / Mindful Check-in · DORMANT 唤醒 | UI可见 | 待人工测试 | **主路径**：零完成开局 → 睡着 + 可忽略提示文案须为邀请式（EN 含 `Quiet time elsewhere can live here too`；ZH 含「别处的静心，也可以记在这里」），**不得**再出现 `Practiced elsewhere today?` / 「今天在别处修行了吗？」。点提示 → 选时长 → dormant-wake → 桥接。**回流**：同日再补登走空闲 **Mindful Check-in**（不睡、不播 dormant-wake）→ 呼吸 → 再出桥接。 | **2026-07-21**：用户书面——「Practiced elsewhere today?」像给离开一段时间回来的人用，却每次游戏刚开始就出现；建议改措辞。说明：触发是「当日零完成」（含首访），不是「离开很久」；已改邀请式文案。请硬刷新 `?product=1` 复测。**此前**：2026-07-19 拍板 B；2026-07-20 气泡锚 Sit 右侧。 | 零完成自动 · `#honesty-idle-entry` · DEV：`__honestyCheckIn` | 2026-07-21 |
