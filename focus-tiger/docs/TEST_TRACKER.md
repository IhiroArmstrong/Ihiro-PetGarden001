...
| Arrival Practice / Choose 合十→idle 衔接 | UI可见 | 待人工测试 | Choose 点意图 → 合十正倒放 → **约 1s 叠代溶解**（合十末帧与 idle 首帧交叉淡化，期间两帧定格）→ 再呼吸 + 缓慢拉远。不应硬切跳动。 | 2026-07-20：用户复测仍跳动；要求 CapCut 式 1s 前后帧叠代/渐隐渐显。已改为 1000ms 溶解并定格。请复测。 | `http://localhost:5173/` · Sit → Choose | 2026-07-20 |
...
| FocusSession + Focus HUD（Sit with Yin / Rise） | UI可见 | 待人工测试 | **主路径**：Arrival 后选 Here & Now → 计时中点 **Rise** → 角色应立刻切到 **blink-breathe pingpong**（正放+倒放完整呼吸，可循环），并出现 Reflection。**回流**：Reflection 跳过/答完后 → Idle 呼吸（若当日零完成则 Sleeping）；再 Sit → Arrival → 再选模式开计时。 | 2026-07-19：点 Rise 后无角色动画变化。**2026-07-20**：已接 `blinkBreathe` pingpong；请复测主路径+回流。 | `#focus-hud` · 演示 1 分钟 | 2026-07-20 |
...
| blink-breathe 眨眼深呼吸 | UI可见 | 待人工测试 | **调试**：情绪入口「眨眼深呼吸(Rise)」或入库素材 → **pingpong 循环**（13 帧正放+倒放）。**主路径**：专注中点 Rise → 同动画。**回流**：Reflection 结束后回 Idle/Sleeping。 | 2026-07-20：改 pingpong 并接入 Rise。 | Rise / `#emotion-debug-ui` | 2026-07-20 |
| MilestoneGlow 备选 breath-halo-hq | UI可见 | 待人工测试 | 调试入库「breath-halo-hq 备选」→ **16 帧 pingpong 循环**（金环扩展+倒放收回）。确认无旧 `breath-halo-expand`。主候选「里程碑金辉」仍为含蝴蝶版。 | 2026-07-20：HQ 改 pingpong（用户反馈正放不完整）。 | `#emotion-debug-ui` · manifest `breathHaloHq` | 2026-07-20 |
