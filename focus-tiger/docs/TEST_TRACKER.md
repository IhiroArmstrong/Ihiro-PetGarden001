...
| cloak Expand A/B + 长离 2B | UI可见 | 待人工测试 | **Expand A**：本地 ≥23 Idle → 披斗篷进 DORMANT（替代旧 yawn/tea）；或 Idle **≥15min 无操作** → forceDormant。**Expand B**：≥23 中途 Rise 或自然达标 → `cloakSleep` holdPose → Reflection（白天仍 Rise 池 / 庆祝）。**2B**：Focusing 时切走 tab **≥30min** 再回 → `dormantWake`→Idle 呼吸（**仍 Focusing**）。**与 2h→DORMANT**：非 Focusing 且距上次结束≥2h 回前台仍进睡（**不删**）。**回流**：关 Reflection 后披斗篷 hold 回 Idle。 | **2026-08-04**：用户同意接线；2B 与 2h DORMANT 互补整合。**同日用户书面**：长时触发条件不易人工测；要求以廉价自动化确信门闩后再开 PR。**自动化锁**：`companionRestPolicy`（2B/15min/深夜 Rise）+ `LATE_NIGHT→forceDormant` + `forceDormant` dormantIdle；**不锁**真实时钟观感 DOM。 | — | — | worktree Vite · `?product=1` · 调系统时钟 / 藏 tab | 2026-08-04 |
...
| 冷启动首屏呼吸感（Yin 留白 + HUD/球区收纳） | UI可见 | 待人工测试 | **主路径（宽屏 ≥900）**：`?product=1` 硬刷新 → ①阿寅整体略小、上下留白更空灵，**不**贴满中下；②底部 Quick/Sit/Honesty/⋯ 与蒲团有缝，Sit 球略大（~1.1×）；③「Tap to sit…」气泡字号/留白更舒适；④左下热力+时钟+? 同玻璃微簇；⑤右上 ♪ 未播放时更幽灵（opacity≈0.58）；⑥左上 Calm 卡为柔毛玻璃、色更淡。**回流**：Sit→Rise 后再回 Idle，留白/球区/HUD 仍对。**375**：三球仍可读可点；热力簇在 HUD 下；ActionBar ?/♪ 仍可用。**须人工锁观感**（无像素 e2e）。 | **2026-08-04**：用户书面冷启动第一眼压迫/碎点 → 舞台 inset + dock 下移 + Sit 强调 + 簇玻璃 + mute ghost + HUD 毛玻璃。 | — | — | worktree `feature/cold-start-first-screen-breathing` · `?product=1` · 375×667 | 2026-08-04 |
