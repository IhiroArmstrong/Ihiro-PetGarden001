...
| 右上音符 = 菜单 Sound（开 Soundscape；宽屏藏 FAB） | UI可见 | 待人工测试 | **主路径（375）**：ActionBar ♪ → 出现 Soundscape 选曲面板（非直接 mute 翻转）；与抽屉 Sound 同效；FAB 不可见。**主路径（≥480）**：右上 `.ambient-soundscape__mute` → 同面板；⋯ Sound 同效；右下 Sound FAB **不可见**（含 Focusing）。**回流**：面板内选曲 / Off；Rise 后停播口径不变；再点音符再开面板。 | **2026-07-29**：承接上行用户书面。e2e：`375: ActionBar note opens…` · `wide Idle: top-right note opens…` · `wide Focusing: …FAB stays hidden`。 | `?product=1` · `#ft-narrow-mute-btn` · `.ambient-soundscape__mute` · `.ambient-soundscape__panel` | 2026-07-29 |
