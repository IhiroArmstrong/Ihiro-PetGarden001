...
| parrot-ear-visit-feather / 鹦鹉耳边造访入库 | UI可见 | 待人工测试 | **调试**：无 `?product=1` → `#emotion-debug-ui` →「鹦鹉耳边造访(信使)」或入库素材。**须见**：鹦鹉飞来耳边低语 → 留彩羽飞走；约 **8 fps**；末约 **1s CapCut** 回闭目呼吸。**回流**：播完回 Idle。**勿**与 Welcome 抢播（冷启动欢迎结束前不播信使）。 | **2026-08-03**：入库。**同日用户书面**：调试试播 **测试 OK**；但首次进产品页见鹦鹉误以为 Welcome——已改：欢迎 hold 期间只出横幅、欢迎结束后再播信使。 | — | — | `#emotion-debug-ui`（**勿** `?product=1`） | 2026-08-03 |
| 场景 A · 应用内提醒 × 鹦鹉信使 | UI可见 | 待人工测试 | **主路径**：设提醒 → 回前台 / 到期（含同页静候，约每 60s 再评）→ 横幅 + 鹦鹉。**回流**：横幅从隐藏再变为可见（含 suppress 后再显、刷新后再到期）应**再播**信使；同一次可见期间 re-sync 不重播。× 关闭后本页横幅不复现（既有 dismiss）。冷启动：Welcome 动画优先，信使等欢迎结束。 | **2026-08-03**：接线。**同日用户书面**：点 Reminder 首次见鹦鹉可接受；**后续到期不再播**——已改「每次 hidden→visible 可播」+ 60s 静候再评（不再本页一次封死）。欢迎抢播见上行。 | — | — | `?product=1` · `e2e/in-app-reminder.spec.js` | 2026-08-03 |
