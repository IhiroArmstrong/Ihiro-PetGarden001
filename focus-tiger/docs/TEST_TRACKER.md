...
| FocusHUD 香炉隐喻 / 数字弱化 | UI可见 | 待人工测试 | **主路径**：开局左上角见小香炉+淡金环，**无**常驻「Focus: N%」计分牌；时长半透明。Sit 开计时后烟/环随进度渐亮，时长加重。**悬停/Tab focus** HUD → 才露出 Focus %。**回流**：Rise 后环/烟回淡、时长回 `00:00`。勿与角色抢视觉重量。 | 2026-07-21：按 UI 分层意见落地香炉隐喻。**自动化**：`focusHudIncense.test.js`（映射）+ e2e `#hud-state`/`#hud-time`；观感须人工。 | `#focus-hud` · `/?product=1` | 2026-07-21 |
