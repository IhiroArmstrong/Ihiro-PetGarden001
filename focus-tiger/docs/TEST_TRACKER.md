...
| Hints 接线表 SSOT（`HINTS_WIRING`） | 纯文档 | 仅单元测试覆盖 | **无运行时**。新建 `HINTS_WIRING.md`：场景→hint 门闩/互斥/宽窄批次（对标动画接线）；文案仍 `ONBOARDING_HINTS`；机器真源仍 registry。交叉链 `RULES_INDEX` / `focus-tiger-docs` / registry `@see`。 | **2026-08-03** 用户书面：合理则办——单独立项 HINTS_WIRING（或扩 ONBOARDING_HINTS）当 SSOT；采纳独立文件。 | — | — | `docs/HINTS_WIRING.md` · `ONBOARDING_HINTS.md` | 2026-08-03 |
