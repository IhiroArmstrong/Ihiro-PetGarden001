...
**命令**：`cd focus-tiger && npm run test:smoke`（scenario + 重置 L-logic + **SessionUiGate** + HUD 映射等）· `npm run test:e2e`（约 **31** 条：产品壳 2 + Companion A/I/K 等 + 意图回显 2 + 热力图 7 + 提醒 4 + 微仪式/桥接 4 等）。Agent 环境若缺浏览器：本机先 `npm run test:e2e:install`，或 config 已默认 `channel: 'chrome'` 用系统 Chrome。
