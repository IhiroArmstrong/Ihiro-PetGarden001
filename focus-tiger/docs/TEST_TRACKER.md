...
| 留存漏斗骨架 RetentionTelemetry | 纯后端 | 仅单元测试覆盖 | **无 UI、无第三方**（正式工具暂不选型）。`RETENTION_FUNNEL.md`；`console.log('[RetentionTelemetry]', …)`。事件：`app_first_open` / `first_session_complete`（含 `secondsSinceFirstOpen`）/ `day1\|3\|7\|30_return`（窗口内首次返回）/ `dormant_bridge_shown\|accepted\|declined`。接线：`main.noteAppOpen`、`HonestyCheckIn.onSessionRecorded`、桥接 `trackEvent`（含 No→declined）。storage `retention-funnel.v1` 纳入 DEV 重置。 | — | `RetentionTelemetry.test.js` · `HonestyBridgeCtaController.test.js` · `docs/RETENTION_FUNNEL.md` | 2026-07-22 |
