...
| **[L237](#L237)** | 应用内提醒设置 + 横幅（场景 **P**） | **有问题**：过去时分无约束；今日已练时设置面板语义不清；须实验室重置后再测横幅 |
