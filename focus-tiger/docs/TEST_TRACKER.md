...
| cloak-sleep / 披毯入睡（进 DORMANT） | UI可见 | 待人工测试 | **主路径**：距上次专注结束 ≥2h 首次进 DORMANT → `cloakSleep` 正放 @6fps≈5.7s → cross-fade → `sleeping`。**回流**：同日再次进 DORMANT（唤醒后再 idle 满 2h 等）→ 直接 `sleeping` 不重复披毯。**调试**：入库素材仍可单条试播。 | — | 改 `localStorage` 的 `focus-session-end` 时间戳模拟 · `#emotion-debug-ui` | 2026-07-21 |
...
| DORMANT 2h 滚动触发 + 回前台 sync | 仅单元测试 | 仅单元测试覆盖 | 单测：`dormantTrigger.test.js` / `dormantIdle.test.js`；smoke 仍用固定 `now`。须人工：真实等待或改 localStorage 时间戳 → 刷新/切回前台 → 进睡 + 首次 cloak 过渡。 | — | `npm test` · `focus-tiger.focus-session-end.v1` | 2026-07-21 |
