...
|---|---|---|---|---|---|---|
| UI Kit 设计实验（tokens + Web Components） | UI可见 | 待人工测试 | **主路径**：`http://127.0.0.1:8765/demo.html?v=20260721d`（须见 **v4 · Companion Mode palette**）。§0 控件灰 vs 文案米；tooltip/dialog 应接近产品 Companion 三选一可读性。**产品壳**：`?product=1` 打开 How shall we sit? 三选一，确认仍是暖米底 + 深标题/说明（token 已接线，观感应几乎不变）。**回流**：关面板再开。 | 2026-07-21：…v3 复测。**同日书面**：Companion 三选一文案框配色没问题，**统一采纳**；v3 除文案框外其它目前看不出问题、**可以试试** → 已定为权威色并 v4 对齐 token + 产品壳 `design-tokens.css`，请用 `?v=20260721d` + 产品壳三选一各看一眼。 | demo `?v=20260721d` · 产品 `/?product=1` | 2026-07-21 |
