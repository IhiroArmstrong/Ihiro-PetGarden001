...
| MilestoneGlow · 琉璃星石变体（meditation-star-reward） | UI可见 | 待人工测试 | **不必等真实连坐 21/100 天。** **实验室（勿 `?product=1`）**：右上情绪入口点 **「里程碑琉璃星石」**（或入库素材 `meditation-star-reward 琉璃星石`）→ 闭目坐禅 + 空中星石。**产品路径快测**：Safari Console（`?product=1`）`__emotionController.playEmotion('milestoneGlow',{milestoneNodeId:'streak-21'})`；或仿 e2e 写入前 20 个练习日到 `focus-tiger.practice-days.v1` 后再达标。**正式语义**：streak-21/100 只播一次星石，非 Celebrating。单元已锁 `pickMilestoneGlowVariant`。**须复测（不抠图版）**：整幅星空/白底烧录、边缘无抠图毛边；硬刷新 tip。 | **2026-08-03**：用户书面同意变体池 + 入库；同日反馈实验室难找 → 补情绪入口「里程碑琉璃星石」。 **同日用户书面（5175）**：抠图效果不行 → **已换**根目录「不抠图的-Yin坐禅-空中魔法奖励…」覆盖同目录 63 帧。 | — | — | 本分支 Vite · 实验室无 `?product=1` · `#emotion-debug-ui` · http://127.0.0.1:5175/ | 2026-08-03 |
