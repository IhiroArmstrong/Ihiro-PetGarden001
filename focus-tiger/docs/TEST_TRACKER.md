...
| starlight-cloak-*（入库保留） | UI可见 | 有问题 | **勿接主线**。磁盘仍有 v3 帧 `starlight-cloak-sleep` / `-wake`；**无**实验室试播钮、**无**产品接线。产品入睡/睡循环/苏醒 = 原 `cloak-sleep`。 | **2026-08-04 早**：v1 不如旧 → Undo。**同日晚**：v3 试播后用户书面——**都暂时不能替换原来的斗篷动画**；确认产品未误接线；已撤试播钮。 | legacy-unclassified | legacy · 暂不进逾期扫描；发布前人工过目 | 素材目录保留 · 产品用 cloak-sleep | 2026-08-04 |
