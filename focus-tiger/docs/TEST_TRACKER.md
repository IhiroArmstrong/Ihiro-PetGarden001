...
| MilestoneGlow · 琉璃星石变体（meditation-star-reward） | UI可见 | 待人工测试 | **不必等真实连坐 21/100 天。** **实验室（勿 `?product=1`）**：右上 **「里程碑琉璃星石」** → 不抠图整幅 + **镜头 100%→158% 线性拉近**。**产品路径快测**：`__emotionController.playEmotion('milestoneGlow',{milestoneNodeId:'streak-21'})`。**正式语义**：streak-21/100 只播一次。硬刷新 tip。 | **2026-08-03**：抠图不行 → 换不抠图源。**同日书面**：要序列逐步放大（首 100% / 末 158%）→ 已加 `playbackZoom`。 | — | — | http://127.0.0.1:5175/ · `#emotion-debug-ui` | 2026-08-03 |
