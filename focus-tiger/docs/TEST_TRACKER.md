...
| 跨动画短叠化统一 1s CapCut | UI可见 | 待人工测试 | **主路径**：微仪式 Idle→smiling、完成/Leave→Idle；`curiousTilt`/`blink`/`riseStretch`/`dormantWake` 进出须约 **1s** 叠化、**不得闪白**。**硬切保持**：Idle 闭目↔睁眼弧、gaze 段间、魔法书回 Idle。**回流**：Rise 后再开微仪式。 | **2026-08-03**：用户书面——短转场易闪白；统一 CapCut；硬切不变。 | — | — | `?product=1&microRitualMs=5000` · 实验室 curiousTilt | 2026-08-03 |
| 轻完成池撤出 curiousTilt | UI可见 | 待人工测试 | 微仪式 / 非首日计时完成：**只**见摆尾 / 点头鞠躬 / 偶发鹦鹉；**不得**再抽到 blink-smile「像没播」。控制台 `__sceneAnimationDispatch.lastPlay.emotionKey` ≠ `curiousTilt`。 | **2026-08-03**：用户拍板——呼吸期已是 smiling，blink 权重并入其它。 | — | — | `?product=1&microRitualMs=2800` · 完成 toast 后看 Yin | 2026-08-03 |
