...
| 门闩一体包 · Companion 点选→真实开表 | UI可见 | 有问题 | **主路径（与代码对齐 · 2026-07-25）**：① Sit→Notice→Breath→**Choose** → 鞠躬后**自动 Focusing**（记忆模式，常为 Here & Now；**不必**再点 Companion）。② 门闩**已就绪**时手动开三选一 → 点 Here & Now / Flow → **立刻 Focusing**，**不得**再出 Notice。③ Offline：门闩未就绪亦可直开（见 L180）。**回流**：Rise 后 gate 关闭 → 须再 Sit 走 Arrival（或 ⚡）；**未** Sit 就点 Here & Now → **会**进 Notice（设计，非静默失败）。Arrival 打开期间 Honesty hide **不得**误开 hint 开表；**storage 仅在 Gate 通过后更新**。375×667。 | **2026-07-22**：静默失败批 3。**2026-07-25 用户书面（P0 ③）**：点 Here & Now → 出「What is present right now?」Notice，认为不对、记得以前修过 → **根因=验收步骤漂移+与 Offline 对照混淆**（见下）；**非**门闩就绪后点选失效。e2e：**A** Choose 自动开表；**A4** 门闩就绪→Here&Now 开表无 Notice；**I2** 未就绪→Here&Now **须**开 Arrival（锁设计）。 | `?product=1` · e2e A/A4/I2/K · `#btn-focus` | 2026-07-25 |
...
| 浏览器 e2e 场景 A/I/K Companion DOM（Playwright） | 纯后端 | 仅单元测试覆盖 | `npm run test:e2e`：**I** hint 开面板；**I2** 未就绪 Here&Now → Arrival（HUD idle）；**A** Choose→自动开表；**A4** 门闩就绪→Here&Now 开表且无 Notice；**A2/A3** 预选→⚡ 开表；**K** Offline 无 Arrival 即开表。 | 2026-07-21：Offline 二次 Sit 已废除。**2026-07-25**：补 A4（L249 门闩就绪点选契约）。 | `e2e/scenario-a.companion.spec.js` | 2026-07-25 |
