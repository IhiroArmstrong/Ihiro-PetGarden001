...
| 「一分钟呼吸」微仪式 · Idle 接入 | UI可见 | 待人工测试 | **主路径**：`?product=1` Idle 见立体入口「A minute of breath / 一分钟呼吸」（青绿 secondary、Sit 上方、Honesty 下方）→ 点开 → 吸↔呼文案 + smiling@4fps + **同拍**呼吸光环/Yin 轻量起伏约 **60s** → 到点 toast「Today counts, too / 今天，也算数」+ **SessionComplete 摆尾**（不 Celebrating、不 Reflection）→ `recordCompletion(1)` + `markToday(1)`。**回流**：中途 Leave 安静退出、不记账不 toast；同日可再点再完成。e2e：`micro-ritual.spec.js`（`?microRitualMs=1500`）。 | 2026-07-22：按拍板口径实现；**同日**：入口改立体钮 + dock gap 16px；吸/呼与光环/Yin 同拍。**动画节奏须人工看** | `?product=1` · `#micro-ritual-idle-entry` · `#micro-ritual` | 2026-07-22 |
