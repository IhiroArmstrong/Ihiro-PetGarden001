...
| **[L249](#L249)** | 门闩一体包 · Companion 点选→开表 | 07-25 主路径 + Rise 回流 **测试 OK** |
...
| Arrival · 轻量气泡 + ⚡ Quick Start（去 Skip 双钮） | UI可见 | 有问题 | **对照** `ARRIVE_MOMENT_DESIGN.md` v2：Notice/Choose 为点图标+观察式短句，**非**重型模态。**本次**：去掉 Arrival 内 Skip / Skip — begin；UI 改为透明气泡/字幕+轻图标格；Sit 路径仍为 Welcome→Notice→Breath→Choose；快速开表改 **⚡ `#quick-start-focus`**（记忆 Companion 模式立刻 Focusing）。**主路径**：Sit → 见轻气泡（非大卡片）；**Arrival 开着时 Sit 隐藏**（防叠图标）→ Notice 点选 → 短句 → Breath → Choose → Focusing；⚡ 仍可见。**点外侧**：Notice / Choose **选择格**打开时，点框外空白须**取消仪式回 Idle**（不开表）；⚡ 仍可立刻开表。**快速路径**：点 ⚡ → 立刻 Rise/Focusing。**回流**：Rise 后再 Sit 仍走完整 Notice/Breath；⚡ 再开表。 | **2026-07-24**…Sit 重叠已改。**2026-07-25 用户书面**：Notice/Choose 选择框应允许点空白消除 → 已补外侧取消；e2e 锁 Notice/Choose 外侧回 Idle。请硬刷新复测。 **2026-07-25 用户书面（5174）**：Sit→Notice / Choose **点空白取消回 Idle** — **测试 OK**。 **2026-07-25 窄屏 375 用户书面（图1）**：Sit→Notice 出现游离 tip「A tap is enough — or skip ahead.」**不对**；点该 tip 后 tip 消失且 **Notice 选择框也被取消**——不对（点 tip ≠ 点空白外侧）。须修：Arrival 中勿出/勿误绑该 tip，且点 tip 不得触发外侧取消。 **2026-07-25 窄屏 375 用户书面（图5）**：Arrival Breath（「Let's arrive together / Inhale…」；用户称 a minute breath）期间仍见 **Sit with Yin**——不对（Arrival 开着须隐藏 Sit）。 **2026-07-25 图1 本地已改（待复测）**：Arrival 进行中不自动出 notice/breath/choose tip；外侧取消豁免 hint/?/用途卡；e2e tip-click 锁住；点空白仍取消。**图5 本地已改（待复测）**：一分钟呼吸进行中隐藏 Sit（`setMicroRitualActive`；窄屏 focusing 布局不再只禁用不藏）；e2e 宽屏+375 锁住。 | `?product=1` · `#arrival-practice` · e2e A + outside-dismiss · `#quick-start-focus` | 2026-07-25 |
...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 有问题 | **主路径**：① 冷启动 hint → **Here & Now / Flow** → Arrival → Choose → Companion 点选 Focusing。② **Offline** → 立刻 Focusing（无 Notice）。③ Sit → Choose → Companion 点选 Focusing。**回流**：Rise 后 hint→Here & Now / Flow → **立刻 Focusing**（门闩保持）；Sit 仍走 Arrival。**点外侧**：面板打开时点空白收起。 | **2026-07-21**：Flow/Reading 鞠躬开表已修。**2026-07-22**：Safari Companion **测试 OK**。**2026-07-25**：外侧关闭；Offline 跳过 Arrival；L249 / Scenario J 回流门闩保持。 **2026-07-25 窄屏 375 用户书面（图2）**：多条路径到 Choose 点头鞠躬后，顶栏仍 **00:00 Calm**、timer 不动，且 **左上无 Focusing `#focus-hud`**——不对；鞠躬后应进入 Focusing 并见 HUD/走动计时。 **2026-07-25 图2 本地已改（待复测）**：根因是鞠躬后 Companion 展开但窄屏未 `ft-narrow-stage-companion`（三选一屏外，像假死 Calm）；契约仍是鞠躬→三选一→点选才 Focusing（非自动开表）。e2e 375 锁 stage 后点选。 | 底部 Sit 旁 dock · **DOM** e2e A/A4/I/J/K · Safari 人工 | 2026-07-25 |
...
| 「本周陪伴」7 格热力图 UI（Idle 常驻） | UI可见 | 有问题 | **主路径**：`?product=1` Idle → 见 7 格。**宽屏 ≥480**：左下 `?` 上方。**窄屏 ≤479 / 375**：ActionBar（? · Calm/time · ♪）+ Yin + **上滑抽屉**——Sit / Quick Start / **Honesty 必须在列** / 呼吸 / How / **Sound → 立刻弹出 Soundscape 选曲面板**（曲目+音量；**禁止**只抬红色 Sound FAB；**禁止**只出一行门闩文案） / **提醒（开设置面板）**；抽屉钮紧凑半高。**Focusing**：左上 `#focus-hud`；Sound FAB 藏、右上 mute 可关。**回流**：Rise → grabber；♪ 静音。**DOM**：e2e `weekly-practice-heatmap.spec.js`（Honesty 常列 + Sound **panel 可见且 FAB 隐藏** + Reminder）。 | **2026-07-24**：375 书面①–⑦。**2026-07-25**：Honesty / 提醒 **测试 OK**；Sound 弹出红色 Sound 钮而非原先选曲框——已改为抽屉 Sound 直接开 Soundscape 面板；**须复测 Sound**。 **2026-07-25 窄屏 375 用户书面（图2）**：多条路径到 Choose 点头鞠躬后，顶栏仍 **00:00 Calm**、timer 不动，且 **左上无 Focusing `#focus-hud`**——不对；鞠躬后应进入 Focusing 并见 HUD/走动计时。 **2026-07-25 图2 本地已改（待复测）**：根因是鞠躬后 Companion 展开但窄屏未 `ft-narrow-stage-companion`（三选一屏外，像假死 Calm）；契约仍是鞠躬→三选一→点选才 Focusing（非自动开表）。e2e 375 锁 stage 后点选。 | `?product=1` · DevTools 375×667 · `#ft-narrow-idle-shell` | 2026-07-25 |
