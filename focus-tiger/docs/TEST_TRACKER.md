...
| 开场即睡 / 冷启动第一幕 Idle | UI可见 | 有问题 | **主路径**：本地已有 ≥2h 前的 `focus-session-end` → 刷新 / 首次打开 `?product=1` → 阿寅须 **Idle 闭目坐禅**（有精神的坐姿呼吸），**不得**立刻播披斗篷→睡着。Honesty 小钮仍可出现（零完成时）。**回流（已拍板）**：切到后台再回前台且仍 ≥2h → **继续披毯进睡**（保留 live DORMANT；≠冷启动）。**对比**：实验室「重置全部本地状态」开局亦 Idle。 | **2026-07-25**：用户书面——进入页面后开场即播披斗篷趴下（当时标「另案」未修）。**2026-07-26**：用户书面——每次第一次试用又是披斗篷睡着；要刚开始看到有精神的 Yin。根因：2h 滚动 DORMANT 在 `onAppReady` 重播 cloak；冷启动现禁进睡。自动化：`dormantIdle` + smoke A1b。**同日用户书面拍板**：回前台且 ≥2h → **继续披毯进睡**（不改为 Idle）。 **2026-07-30 用户书面（图）**：仍见开场/首屏 Asleep 披毯睡（宽屏 HUD Asleep）。须区分：真冷启动刷新 vs 回前台 live sync（≥2h 拍板仍可进睡）。请用实验室清空后硬刷新复验。 **2026-07-30 用户书面复验**：实验室清空全部本地状态 → 硬刷新 `?product=1` → **闭目坐禅 Idle — 测试 OK**。 **2026-08-04 用户书面（5173 · tip `4698eb3`）**：长挂回看披斗篷——确认非硬刷新；已拍板 Expand A **plan A**（关白天无操作披毯）。交叉见 Expand A 行 / §6.11。 | legacy-unclassified | legacy · 暂不进逾期扫描；发布前人工过目 | `?product=1` · 改/保留 `focus-session-end` 后刷新 · `npm run test:smoke` | 2026-07-26 |
...
| dormantWake / Honesty 睡醒序列 | UI可见 | 已通过 | **主路径**：进 DORMANT 后「Honesty唤醒」选时长 → **`cloak-sleep` 34 帧倒放**（**6 fps** ≈5.7s）→ 定格末帧 + **约 10s 呼吸**并行。**回流**：定格后桥接 Yes/No。**对比**：舒展提醒仍 stretchReminder（已无「唤醒(伸懒腰)」调试钮）。 | **2026-07-21～22**：旧 cloak 倒放曾书面通过。**2026-08-04 用户书面**：starlight 苏醒观感不如原来（对比伸懒腰也更差）→ **Undo** 回 cloak 倒放。 | — | — | Honesty / `#emotion-debug-ui` | 2026-08-04 |
...
| parrot-ear-visit-feather / 鹦鹉耳边造访入库 | UI可见 | 待人工测试 | **调试**：无 `?product=1` → `#emotion-debug-ui` →「鹦鹉耳边造访(信使)」或入库素材。**须见**：鹦鹉飞来耳边低语 → 留彩羽飞走；约 **8 fps**；末约 **1s CapCut** 回闭目呼吸。**回流**：播完回 Idle。**勿**与 Welcome 抢播（冷启动欢迎结束前不播信使）。 | **2026-08-03**：入库。**同日用户书面**：调试试播 **测试 OK**；但首次进产品页见鹦鹉误以为 Welcome——已改：欢迎 hold + 结束后延后补播。**工作流根因**：见 `DEV_WORKFLOW_QUALITY` §6.10。 **同日晚用户书面（feature tip 复测）**：有提醒偏好冷启动 → 先书/点头、欢迎后再鹦鹉 — **测试 OK**。 | — | — | `#emotion-debug-ui`（**勿** `?product=1`） | 2026-08-03 |
| 场景 A · 应用内提醒 × 鹦鹉信使 | UI可见 | 待人工测试 | **主路径**：设提醒 → 回前台 / 到期（含同页静候，约每 60s 再评）→ 横幅 + 鹦鹉。**回流**：横幅从隐藏再变为可见（含 suppress 后再显、刷新后再到期）应**再播**信使；同一次可见期间 re-sync 不重播。× 关闭后本页横幅不复现（既有 dismiss）。冷启动：Welcome 动画优先，信使等欢迎结束。**可选邻接**：横幅已出时点 ⚡/Sit 进 Focusing → 横幅因忙碌 **自动藏**（不是点 ×）；Rise 回 Idle 后横幅再出并可再播鹦鹉。 | **2026-08-03**：接线。**同日用户书面**：点 Reminder 首次见鹦鹉可接受；**后续到期不再播**——已改「每次 hidden→visible 可播」+ 60s 静候再评。欢迎抢播见上行；**工作流根因** §6.10。 **同日晚用户书面**：设未来 1～2 分钟等到期（或切后台再回）→ 横幅+鹦鹉 — **测试 OK**。 **同日再书面**：Focusing suppress（⚡→横幅自藏→Rise 回 Idle 再出横幅+鹦鹉）— **测试 OK**（表扬邻接路径说明清楚）。 | — | — | `?product=1` · `e2e/in-app-reminder.spec.js` | 2026-08-03 |
...
| 跨动画短叠化统一 1s CapCut | UI可见 | 已通过 | **主路径**：微仪式 Idle→smiling、完成/Leave→Idle；`curiousTilt`/`blink`/`riseStretch`/`dormantWake` 进出须约 **1s** 叠化、**不得闪白**。**硬切保持**：Idle 闭目↔睁眼弧、gaze 段间、魔法书回 Idle。**回流**：Rise 后再开微仪式。 | **2026-08-03**：用户书面——短转场易闪白；统一 CapCut；硬切不变。 **同日再书面（`origin/develop` tip `62b68bc` = merge PR #102 · behind=0）**：短叠化改 1s 后无闪白、硬切路径仍硬切 — **测试 OK**。关单覆盖分工：e2e/自动化已锁常量与池契约（`EmotionController` / Dispatcher 单测 + `test:smoke`；**不**锁闪白观感）；人工已覆盖微仪式进出叠化无闪白 + 硬切路径仍硬切；仍须人工：无。 | — | — | `?product=1&microRitualMs=5000` · 实验室 curiousTilt | 2026-08-03 |
