...
|---|---|---|---|---|---|---|
| Arrival Practice / 抵达练习（Welcome → Notice → Breath → Choose） | UI可见 | 待人工测试 | 1) Sit → Welcome（默认速微笑）→ Notice → Breath「Let's arrive together」：**放慢眨眼微笑（约 4fps）并保持**，**不要**切到闭目 idle-breathing；合十只在 Choose 确认。2) Choose → Companion。**复测**：Honesty→桥接 Yes→Arrival Breath 是否连贯、不硬切闭目。 | 2026-07-19：Breath 落入 idle 仍不连贯 → **已改**为放慢 `blink-smile` 并停止落入 idle。请复测。 | `http://localhost:5173/` · Honesty 桥接 Yes / Sit · DEV：`__arrivalPractice` | 2026-07-19 |
