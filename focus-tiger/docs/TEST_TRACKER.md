...
| Ambient Soundscape / 默认 Mer-Ka-Ba + 开关 | UI可见 | 待人工测试 | **主路径**：进应用即有显眼 **「关闭音乐」**（默认 Mer-Ka-Ba）；点一下立刻静音变 **「打开音乐」**；再点开回。未 Sit 也可开关。次要「曲目」可选另一首。**回流**：关音乐 → 刷新 → 应保持关闭；再开 → 刷新应保持开。浏览器拦自动播放时点按钮/页面可解锁。 | **2026-07-21**：用户书面——点「Turn music off」音乐关不掉。根因：自动播放解锁 `pointerdown` 与关按钮竞态 + 迟到的 `play()`；已修 `_playbackEpoch` + 去掉 document 解锁。**此前**：默认要有 Mer-ka-ba + 显眼开关。请硬刷新复测关/开。 | 右下角 · `__ambientSoundscapeUI` | 2026-07-21 |
...
| 人工 · 默认音乐一键开关 | UI可见 | 待人工测试 | 1) 重置本地后开 `?product=1`。2) 未 Sit 即应听到 Mer-Ka-Ba（或点一次解锁）。3) 点 **关闭音乐** → 立刻静音。4) 点 **打开音乐** → 立刻再响。5) 回流：关掉后刷新仍静音。 | **2026-07-21**：用户书面要默认音乐+显眼开关。旧「须先 FOCUSING」口径作废。 | `?product=1` · 右下角主按钮 | 2026-07-21 |
