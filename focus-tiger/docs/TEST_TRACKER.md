...
| 「一分钟呼吸」微仪式 · Idle 接入 | UI可见 | 待人工测试 | **主路径**：`?product=1` Idle 见立体入口「A minute of breath / 一分钟呼吸」（青绿 secondary、Sit 上方、Honesty 下方）→ 点开 → 吸↔呼文案 + smiling@4fps + **同拍**呼吸光环/Yin 轻量起伏约 **60s** → 到点 **画面中下部** toast「Today counts, too / 今天，也算数」+ **SessionComplete 摆尾**（不 Celebrating、不 Reflection）→ `recordCompletion(1)` + `markToday(1)`。**进行中**：左上 FocusHUD `#hud-time` / 今日同坐条 / 金环须随墙钟推进（仍不启 FocusSession / Rise）。**回流**：中途 Leave 安静退出、不记账不 toast；同日可再点再完成。**保护面**：Honesty 桥接 Yes/No 可见时入口须隐藏（勿叠住文案）。**Hint**：Idle 点「?」须见 `micro-ritual` tip。e2e：`micro-ritual.spec.js`。 | 2026-07-22：按拍板口径实现；入口立体钮；桥接叠层 hide。**同日再书面**：① 点「?」后「一分钟呼吸」仍无 Hint → **已补登记** `micro-ritual`；② 桥接打开时入口叠层须复测。**2026-07-22 午**：用户书面——①「呼吸一分钟，左上角的 timer 等等仪表都不动；这不对，这个一分钟肯定也算专注内容」②「toast Today counts, too 在下面按钮夹缝中短暂显示，非常不显眼，要不考虑放在画面中间合适位置？」③「其它方面测试 OK」。→ **已改**：HUD 直播 + toast 中置。**同日再复测**：用户书面「基本 OK；只是中置 toast 高度（约 42%）确实挡脸/而且偏高」→ toast 下移约 **62%**。**同日又书面**：补登成功 toast（`Quiet time elsewhere counts, too.`）位置挺好，微仪式完成 toast **应同一位置** → 二者已共用 `MINDFUL_TOAST_PLACEMENT_ACKNOWLEDGE`（同锚点 62%）；请硬刷新对照。 | `?product=1` · `#micro-ritual-idle-entry` · `#micro-ritual` · `#focus-hud` · `#mindful-acknowledge-toast` | 2026-07-22 |
| 「一分钟呼吸」· 吸呼相位与 smiling 节奏 | UI可见 | 待人工测试 | 微仪式进行中：吸/呼约 **2.5s** 交替（同 Arrival）；角色 smiling pingpong **4 fps**；**光环周期 = 5s（吸收敛 / 呼晕开）与文案同拍**；Yin **轻量 scaleY** 吸略扩、呼收回（非 idle-breathing 帧级胸腔，勿硬切闭目）。到点摆尾后再回 Idle。 | 2026-07-22：观感分列；同日加文案↔光环/Yin 同拍。**同日午**：用户书面「其它方面测试 OK」（含本行节奏）。 | 同上 · 可 `?microRitualMs=15000` 缩短人工观察 | 2026-07-22 |
