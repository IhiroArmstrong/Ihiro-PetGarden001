...
| 「本周陪伴」7 格热力图 UI（Idle 常驻） | UI可见 | 待人工测试 | **主路径**：`?product=1` Idle → 见 `#weekly-practice-heatmap` 横排 7 格；亮=`null\|/>0`，暗=浅 `ink-faint`；无文案/感叹号/点击。**回流**：Sit→Focusing 隐藏；Rise 回 Idle 再出现。**窄屏 375**（`@media max-width:479px`）：簇改挂 **HUD 下方**（非左下盖 dock）；HUD 略缩 + mute 留空；dock 胶囊 `max-width:120px`、`gap:16px`；Sound 完整可读且与 How ≥8px。**宽屏 ≥480**：簇仍左下 `?` 上方（原 bottom）。**DOM**：e2e `weekly-practice-heatmap.spec.js`（Idle 7 格 / Focusing 隐藏 / seed 亮暗 / **375 几何不重叠**）。**Hint tip** 仍人工。 | **2026-07-22**：点「?」无 heatmap Hint → 已补登记。**2026-07-24**：用户 DevTools 375——时钟/7 格与 dock 重叠（附图）。**同日修复** `fix/scenario-o-375-chrome-layout`：窄屏抬簇到 HUD 下 + HUD/dock/mute 收窄；**须 375 复测**（簇在 Calm 卡下、不挡老虎脸；dock/?/Sound 间距；宽屏仍左下）。 | `?product=1` · DevTools 375×667 · `#weekly-practice-heatmap-cluster` | 2026-07-24 |
