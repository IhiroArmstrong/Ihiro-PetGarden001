...
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 待人工测试 | 1) 先点掉自动提示。2) 点 **?**。3) 须同时见：本页控件 hints + `help-remedy` + **用途简介卡**（EN/ZH 各一句「能帮你做什么」）。4) 点「Got it / 知道了」关简介卡；点气泡关该条。5) 回流：再点 ? 仍全部出现。 | 2026-07-20：只有元文案。**2026-07-21**：用户要求加 App 用途简介 → 已加。请复测。 | `#onboarding-hint-help` · `#onboarding-app-purpose` | 2026-07-21 |
