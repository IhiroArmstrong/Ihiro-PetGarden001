...
| FocusHUD 金环+呼吸光 / 数字弱化 | UI可见 | 待人工测试 | **主路径**：左上角见**金环 + 中心呼吸光点**（**无**香炉碗/烟、**无**常驻 Focus %）；Sit 开计时后环填充与光点随进度渐亮，时长加重。**悬停/Tab focus** → 才露 Focus %。**回流**：Rise 后环/光回淡、时长回 `00:00`。 | 2026-07-21：原香炉碗被误读为葡萄酒；用户拍板改「只留金环 + 中心小点/呼吸光」。请硬刷新复测：中间无碗、光点缓呼吸、环跟进度走。**自动化**：`focusHudHalo.test.js` + e2e `#hud-state`/`#hud-time`。 | `#focus-hud` · `/?product=1` | 2026-07-21 |
