...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 有问题 | **须先完成 Arrival**（三选一才会解锁）。Arrival 结束后展开。**Here & Now / Flow State：点选后立即开始 Focus+计时**（HUD 走动、按钮变 Rise、Sound 可用）。**Offline Space：只预选，须再点 Sit**。Rise 后再测：未再走 Arrival 前「How shall we sit?」应禁用（禁止选了却不开计时）。 | 2026-07-19：选 Here & Now / Flow 后计时不动（大谬）→曾修；再测又失效。**根因**：Rise 后 `arrivalGateReady=false`，选模式被 main 静默 return，但三选一仍可点。已改为门闩未就绪时禁用点选 + `canBeginFocusOnCompanionModeSelect` 单测锁。请复测：①Arrival 后点 Here & Now 立刻计时；②Rise 后未 Arrival 时 hint 禁用；③再 Sit→Arrival→Flow State 立刻计时。 | 底部 Sit 旁 dock · Arrival 完成后 · DEV：`__companionModePicker` | 2026-07-19 |
...
| Emotion debug UI（右上角调试面板） | UI可见 | 待人工测试 | 逐个点一次性姿态：播完应**定格末帧**，不硬切默认闭目呼吸；点「坐禅闭眼」才回 idle 循环。循环态（睡着/微笑/光环）照常循环。 | 2026-07-19：勿刻板切回默认闭目；不连贯则停末帧→已按此改调试 `holdPose`，请复测。 | `#emotion-debug-ui` | 2026-07-19 |
