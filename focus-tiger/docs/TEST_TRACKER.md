...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 待人工测试 | **主路径**：Choose 确认后三选一展开；**桌面 ≥900px 面板在右侧**（不挡 Yin 鞠躬）。Here & Now / Flow State 点选即计时；Offline 再 Sit。**回流**：Rise 后再 hint → Arrival。 | 2026-07-20：正中大框挡鞠躬→已改右侧栏，请复测。 | 底部 Sit 旁 dock · DEV：`__companionModePicker` | 2026-07-20 |
...
| IdleOrchestrator / 坐禅闭眼 | UI可见 | 待人工测试 | 点「坐禅闭眼」→ 呼吸×5→眨眼；**切换约 1s CapCut 叠代、溶解期定格**，不应闪一下。加速：`__idleOrchestrator.setTiming({breathCyclesBeforeBlink:1})`。 | 2026-07-20：仍闪→闭目↔睁眼改用 1s 叠代（原 180ms）。请复测。 | `#emotion-debug-ui` · `__idleOrchestrator` | 2026-07-20 |
