...
| 人工 · 点 ? 补救（主条 +「还有 N 条」逐条展开） | UI可见 | 待人工测试 | **主路径（375）**：先点掉自动提示 → 点 **?** → 见 **情境主条** tip（如 Sit）+ **「还有 N 条」芯片** + **用途简介卡**（非一次铺开全部）。点芯片 → **只多出 1 条**目录 tip，芯片 N 递减；再点芯片替换下一条。**禁止**底部多 tip 叠成乱指。**回流**：关气泡 / 点外侧收起后再点 ?；Rise 后再点 ?。目录可覆盖：How shall we sit? / 热力图 / 提醒 / 一分钟呼吸 / Sound gated 等（逐条，非同时）。 | **2026-07-20**…**2026-07-26**：芯片迁回。**2026-07-26 晚用户书面（5176 · 375）**：Sit→Arrival→⚡ Focusing / 抽屉呼吸无旧 Sit — **测试 OK**；但点 ? → 主 tip +「还有 N 条」→ 点芯片展开其余后**页面下方很乱**（多 tip 重叠）。已改芯片**逐条**展开（同时最多主条+1）；e2e 锁不洪水+不重叠。须硬刷新复测。 | `#onboarding-hint-help` · `#ft-hint-catalog-chip` · `#onboarding-app-purpose` | 2026-07-26 |
...
| 浏览器 e2e harness · storage wipe + preview webServer | 纯后端 | 仅单元测试覆盖 | **2026-07-26 根因**：`openFreshProductShell` 曾在**每次**导航 wipe `focus-tiger.*`，heatmap seed→`reload()` 后种子被清 → `expected lit for YYYY-MM-DD`（日期来自 `shift(-6)` 相对日，**非**硬编码产品日、**非**产品 lit 逻辑错）。已改为 **每 browser context 只 wipe 一次**（`sessionStorage` gate）。全量套件用 **`vite preview`**（避 `vite dev` 中途挂死）：**CI** production build；**本地** `vite build --mode development`（Vite 下 `DEV` 仍为 false，故实验室钮改看 `MODE==='development'`）。提醒 e2e 须生产构建也暴露 `__inAppReminder` / `__dailyCompletionStore`（同 `__honestyBridge`）。覆盖层：**DOM e2e** harness；**不**锁序列观感。 | — | `e2e/helpers/product-shell.js` · `playwright.config.js` · `weekly-practice-heatmap.spec.js` · `src/main.js` | 2026-07-26 |
...
| 窄屏 · 自动 onboarding 互斥（≤1 条） | UI可见 | 待人工测试 | **主路径（375×667）**：实验室清空引导已读 → `/?product=1`。冷启动同一时刻自动气泡 **≤1**（优先 `help-affordance`，关掉后串行 Sit / How shall we sit? 等）。**回流**：点气泡关掉 → 下一条出现；点 **?** 见情境主条 +「还有 N 条」芯片 + 用途简介卡（点芯片逐条展开，非一次铺开）。**横屏 667×375**：自动仍 ≤1、不退化。**桌面 ≥900**：互斥同样生效；尖角对准 ?。 | 2026-07-21：用户同意 Task 1；截图去掉叠加后观感可。**单元**：`selectExclusiveAutoHintIds`（互斥 id 列表；**非**窄屏 DOM 叠放）。 | `?product=1` · DevTools 375×667 | 2026-07-21 |
