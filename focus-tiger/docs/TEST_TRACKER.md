...
| parrot-ear-visit-feather / 鹦鹉耳边造访入库 | UI可见 | 待人工测试 | **调试**：`#emotion-debug-ui` →「鹦鹉耳边造访(信使)」或入库素材 `parrot-ear-visit-feather`。**须见**：鹦鹉飞来耳边低语 → 留彩羽飞走；约 **8 fps**；末约 **1s CapCut** 回闭目呼吸。**回流**：播完回 Idle，不卡定格。 | **2026-08-03**：根目录 `Yin_Parrot_Ear_Visit_Feather_transparent` 入库为 `parrot-ear-visit-feather`（93 帧 · 1056×864）。 | — | — | `#emotion-debug-ui` · `?product=1` | 2026-08-03 |
| 场景 A · 应用内提醒 × 鹦鹉信使 | UI可见 | 待人工测试 | **主路径**：设提醒时分 → 切后台再回前台（或 DEV `__inAppReminder.setNow` 过时）→ 顶部横幅 `reminder.gentle_waiting` **同时**播鹦鹉信使。**回流**：本页横幅再同步不重播动画；× 关闭后横幅不复现；完整刷新可再播。**忙碌 suppress**：Focusing 中不横幅不播。 | **2026-08-03**：用户书面「禅意信使」场景。e2e 锁 `parrotMessengerPlayed` + emotion key。 | — | — | `?product=1` · `e2e/in-app-reminder.spec.js` | 2026-08-03 |
| 场景 B · 鹦鹉稀有彩蛋（轻完成池） | UI可见 | 待人工测试 | **主路径**：微仪式完成或非首日计时完成 → 偶发（约 7%）播鹦鹉而非摆尾/点头。**须见**：仍**禁止** Celebrating 舞。**另**：streak-7 仪式见 MilestoneGlow 行（50/50）。**羽毛残影**：**不做**（2026-08-03 拍板）。 | **2026-08-03**：进 `LIGHT_COMPLETE_POOL` weight 8；同日晚 streak-7 另接 50/50。 | — | — | 微仪式 / 调试强制 random · `sceneAnimationDispatcher.test.js` | 2026-08-03 |
| parrotMessengerGate | 纯后端 | 仅单元测试覆盖 | 横幅首次 show 才播；已可见 re-sync / 本页已播 / hide → 不播 | — | — | — | `parrotMessengerGate.test.js` | 2026-08-03 |
