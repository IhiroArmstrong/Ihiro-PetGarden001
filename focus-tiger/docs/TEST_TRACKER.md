...
| 右上音符开/关声景（菜单已删 Sound） | UI可见 | 待人工测试 | **主路径（375）**：ActionBar ♪ → Soundscape 选曲面板（非 mute 翻转）；FAB 不可见。**Focusing**：ActionBar 常显，点 ♪ 同样开面板。**主路径（≥480）**：右上 `.ambient-soundscape__mute` → 同面板；右下 Sound FAB **不可见**。**曲目记忆**：选某曲播放 → 再点音符静音 → 再点音符开面板 → **仍高亮上次曲目**（不得误标 Off）；再点该曲可续播。面板显式选 Off 才记住 Off。**回流**：Rise 后停播口径不变。 | **2026-07-29**：承接上行用户书面。**同日**：rebase onto develop after PR#23。 **2026-07-30 用户书面（图4）**：⋯/抽屉 Sound 应删除；音符：无薄荷绿；有声时再点须关音乐。 **2026-07-30 再书面**：清空引导后再验薄荷绿仍无；暂停后再播系统无曲目记忆 → 面板选中态改按 preferred；`mute`≠面板 Off。 | `?product=1` · `#ft-narrow-mute-btn` · `.ambient-soundscape__mute` · `.ambient-soundscape__panel` · `.ambient-soundscape__track.is-selected` | 2026-07-30 |
...
| Onboarding · 首次登录右上音符薄荷绿圆点 | UI可见 | 待人工测试 | **主路径**：实验室清空引导已读 → `?product=1` 冷启动 Idle → 右上音符钮上见**薄荷绿脉冲圆点**（与 ⋯ 菜单同色 `#6db3a0`，挂在钮内右上角，**非**视口边缘浮动圆点）；点音符打开 Soundscape，并弹出 tip（EN「If your environment allows…」/ ZH「若环境允许…」）。**回流**：选曲或开关音乐后圆点 done 消失。**不得**一堆 auto tip 挡操作。 | 2026-07-30：合入 click-trigger；文案按用户急需口径。 **2026-07-30 用户书面（图）**：宽屏 Idle/Asleep 右上音符无薄荷绿脉冲；? / ⚡ / ⋯ 有圆点。选曲面板可开但听不到声音（音符带斜杠）。 **2026-07-30 再书面**：实验室清空后再验 `?product=1` 仍无薄荷绿。→ 改挂钮内 host mint（同菜单样式）；浮动 badge 对 ambient 停用（贴边难辨）。 | `?product=1` · `.ambient-soundscape__mute > .ft-secondary-menu-hint-dot` · `#ft-narrow-mute-btn` | 2026-07-30 |
