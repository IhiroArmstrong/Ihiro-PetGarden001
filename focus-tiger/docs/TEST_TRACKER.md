...
| StateManager 合法转移 warn（不阻断） | 纯后端 | 仅单元测试覆盖 | `StateManager.test.js`：非法转移仍写入但 `console.warn`；合法路径无 warn。**单测 harness** `MoodController.test` 会打出 `IDLE → CELEBRATE` warn（跳步测 CELEBRATE 映射），属预期、未改行为。观察册：`EDGE_CASES.md` | — | `src/core/StateManager.js` | 2026-07-22 |
...
| 静默失败观察册 EDGE_CASES | 纯后端 | 仅单元测试覆盖 | 审计边角入库；批 3 门闩一体包方案待确认。不代替 TEST_TRACKER 验收行。 | — | `docs/EDGE_CASES.md` | 2026-07-22 |
