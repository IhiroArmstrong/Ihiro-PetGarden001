...
| **i18n**（v1 可点切语 · 已拍板） | **下一回合 A+B+C**。目标语：en/es/zh/ja/de/fr（§9.6）；**完整启用先 en+zh**；四语审完再露出。见审计 **§9** | 场景 G |
...
| i18n（默认 en / 可切多语） | UI可见 | 待人工测试 | **拍板（2026-07-30）**：v1 **可点切语** + 记忆偏好。意向语种 en/es/zh/ja/de/fr；**完整可用先 en+zh**；另四语审校 `ready` 后再进选择器与发版声称。**下一回合**：A unit + B e2e（点 UI）+ C 切语入口。人工：375 各启用语排版。 | **2026-07-30**：确认要多语言 + v1 可点切语；六语在无内容审校风险前提下为方向——工程低风险，齐声称四语有审校风险（审计 §9.6）。 | `?product=1` · ⋯/抽屉 Language · `src/locales/*` · 审计 §9 | 2026-07-30 |
