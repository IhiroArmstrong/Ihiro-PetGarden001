...
| starlight-cloak v5 + 经典 50/50 | UI可见 | 待人工测试 | **主路径**：进 DORMANT / Honesty 睡醒 → classic 与 starlight v5 **约各半**（同轮入睡与苏醒变体匹配）。**实验室**：入库素材点 `starlight-cloak-sleep v5 正放/睡循环/苏醒` 与经典 cloak。**Wellness 2A**：≥23 或 <06 冷启动披斗篷 + 夜深 toast；06–10 苏醒仪式 + 晨光 toast；10–23 仍 Idle 开局。**回流**：白天硬刷新不得因旧 2h 戳开场即睡。 | **2026-08-04**：v1/v3 曾拒；同日用户提供 v5 + 要求与旧斗篷并存 50/50 + 2A。 | — | — | worktree Vite · 实验室 `/` · `?product=1` | 2026-08-04 |
| cloak Expand A/B + 长离 2B | UI可见 | 待人工测试 | **Expand A**：本地 ≥23 Idle → 披斗篷进 DORMANT（替代旧 yawn/tea）；或 Idle **≥15min 无操作** → forceDormant。**Expand B**：≥23 中途 Rise 或自然达标 → `cloakSleep` holdPose → Reflection（白天仍 Rise 池 / 庆祝）。**2B**：Focusing 时切走 tab **≥30min** 再回 → `dormantWake`→Idle 呼吸（**仍 Focusing**）。**与 2h→DORMANT**：非 Focusing 且距上次结束≥2h 回前台仍进睡（**不删**）。**回流**：关 Reflection 后披斗篷 hold 回 Idle。 | **2026-08-04**：用户同意接线；2B 与 2h DORMANT 互补整合。 | — | — | worktree Vite · `?product=1` · 调系统时钟 / 藏 tab | 2026-08-04 |
| wakeUp 调试键删除 | 纯后端+调试 | 待人工测试 | **主路径**：实验室 `/` 情绪入口**无**「唤醒(伸懒腰)」；`SPRITE_SEQUENCES.wakeUp` 不存在。**对比**：仍有「两小时舒展」`stretchReminder`；Honesty 睡醒仍 `dormantWake`。 | **2026-08-04**：用户确认未接线、末帧闭眼不佳 → 删除。 | — | — | `#emotion-debug-ui` | 2026-08-04 |
