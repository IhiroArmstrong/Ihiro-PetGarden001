...
| 窄屏 · 主屏三主钮（Sit / Quick Start / Honesty） | UI可见 | 待人工测试 | **主路径（375×667 · `?product=1`）**：Idle 见 `#ft-narrow-home-ctas`——三个**圆球**横排水墨图腾：Sit（蒲团橙 + **圆相 Enso**）/ Quick Start（米金 + **上升一笔**）/ Honesty（米金 + **水墨心/朱印落点**）。点球 → 同桌面语义。**首次 hints**：`sit-button` / `quick-start` / `honesty-optional` 可点气泡消除（互斥串行 ≤1）；点对应球亦记已读。**抽屉**：上滑后 **不得**再出现这三项。**回流**：Rise → 三球再出现；Arrival/Honesty 叠层期壳 suppress。**宽屏 ≥480**：dock 不变。**DOM**：e2e 锁 aria-label + 抽屉无三主钮。 | **2026-07-26 用户书面**：窄屏首页底部太空 → 三主钮上屏。**同日再书面**：须圆球模样；Quick Start 用闪电圆球；另两钮设计图案；加首次可点消 hints。**同日再书面（图腾方案）**：Sit→水墨圆相；Quick→水墨一笔（弃⚡）；Honesty→水墨心/朱印落点（弃✓）。 | DevTools 375 · `#ft-narrow-home-sit` · `#ft-narrow-home-quickstart` · `#ft-narrow-home-honesty` | 2026-07-26 |
