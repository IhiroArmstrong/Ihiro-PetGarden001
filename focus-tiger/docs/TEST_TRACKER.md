...
| 行 | 功能 | 为何在 P0 |
|---|---|---|
| **[L178](#L178)** | Arrival · 轻量气泡 + ⚡ Quick Start | 07-25 晚宽屏 **测试 OK**（②） |
| **[L232](#L232)** | 「本周陪伴」7 格热力图（场景 **O**） | 07-26：**图1–12 相关项测试 OK**（375 · worktree） |
...
| **[L254](#L254)** | 门闩一体包 · Companion 点选→开表 | 07-25 主路径 + Rise 回流 **测试 OK**（③） |
...
| 「本周陪伴」7 格热力图 UI（Idle 常驻） | UI可见 | 有问题 | **主路径**：`?product=1` Idle → 见 7 格。**宽屏 ≥480**：左下 `?` 上方。**窄屏 ≤479 / 375**：ActionBar（? · Calm/time · ♪）+ Yin + **上滑抽屉**——Sit / Quick Start / **Honesty 必须在列** / 呼吸 / How / **Sound → 立刻弹出 Soundscape 选曲面板**（曲目+音量；**禁止**只抬红色 Sound FAB；**禁止**只出一行门闩文案） / **提醒（开设置面板）**；抽屉钮紧凑半高。**Focusing**：左上 `#focus-hud`；Sound FAB 藏、右上 mute 可关。**回流**：Rise → grabber；♪ 静音。**DOM**：e2e `weekly-practice-heatmap.spec.js`（Honesty 常列 + Sound **panel 可见且 FAB 隐藏** + Reminder）。 | **2026-07-24**：375 书面①–⑦。**2026-07-25**：Honesty / 提醒 **测试 OK**；Sound 弹出红色 Sound 钮而非原先选曲框——已改为抽屉 Sound 直接开 Soundscape 面板；**须复测 Sound**。 **2026-07-25 晚用户书面（P0 ⑦ · 场景 O）**：**仍有问题**（375 故事矩阵）；**另线** `fix/scenario-o-375-chrome-layout` 继续修——**不阻塞** `feature/wide-idle-more-menu` push；Task 3 须等本行收口后再开。 **2026-07-25 深夜用户书面（worktree @5174 · 375）**：**图1–图7 测试 OK**（正确树）。**图8 有问题**：Notice 选择框无 ⚡。**图9 有问题**：点 ? 补救 tips 虽不再乱堆成一团，但五条竖叠仍挡老虎、展示欠优——待拍板优化方案后再改。 **2026-07-26 本地已改（待人工测试）**：图8 Arrival 仅露 ⚡；图9 点 ? → 情境单条 +「还有 N 条」展开（学图7）。 **2026-07-26 用户书面（图10–12）**：图8 基本 OK 但 ⚡ 图标偏移；图9 基本 OK 但找不到「还有 N 条」；图11 Reminder 横幅贴顶被裁；问 21:51 横幅消失而 20:51 不消失（DEV `setNow` 停在 ~20:52）。**同日再改（待复测）**：⚡ flex 居中；「还有 N 条」改常驻芯片 `#ft-hint-catalog-chip`；窄屏横幅下移 ActionBar 下。 **2026-07-26 用户书面（375 · Cursor Browser）**：⚡ 居中；`?` 下「还有 N 条」芯片；Reminder 横幅全文可见 — **全部测试 OK**。 | `?product=1` · DevTools 375×667 · `#ft-narrow-idle-shell` | 2026-07-26 |
| 「一分钟呼吸」微仪式 · 方案调研 | 纯文档 | 仅单元测试覆盖 | 方案见 `MICRO_RITUAL_PLAN.md`。**实现已另开下行**。 | 2026-07-22：先调研后实现 | `docs/MICRO_RITUAL_PLAN.md` | 2026-07-22 |
| 「一分钟呼吸」微仪式 · Idle 接入 | UI可见 | 待人工测试 | **主路径**：立体入口 → 吸↔呼文案 + smiling@4fps + **4s 独立光环**（不同拍）约 60s → 中下部 toast + SessionComplete 摆尾 → 记账。进行中 FocusHUD 直播；**呼吸中 Sit 须隐藏**（非仅禁用）。**回流**：Leave 安静退出。桥接时入口隐藏。**DOM**：e2e `micro-ritual.spec.js`（缩短墙钟主路径 + **Sit hidden** + 375 窄屏锁；Leave 不记账；**非** smiling@4fps / 摆尾节奏观感）。 | 2026-07-22：实现+HUD/toast。**同日晚用户书面**：主路径除同拍/质感外基本 OK；同拍须 undo；入口间距基本 OK 但四钮质感须协调。 **2026-07-25 窄屏图5**：呼吸中仍见 Sit → 本地已隐藏（待复测）。 | `?product=1` · `#micro-ritual-idle-entry` · e2e `micro-ritual.spec.js` | 2026-07-25 |
...
| 门闩一体包 · Companion 点选→真实开表 | UI可见 | 已通过 | **主路径**：`?product=1` → Sit→Notice→Breath→**Choose** → 鞠躬后**展开 Companion** → 点 **Here & Now / Flow / Offline** → **立刻 Focusing**（不得再 Notice）。预选 / ⚡ 仍可直接开表。**回流（Scenario J）**：Focus→Rise（Skip Reflection）→ hint → **Here & Now / Flow → 立刻 Focusing**（**不得**再 Notice）。**Sit** 回流仍走完整 Arrival。冷启动未解锁时 Here & Now → Arrival（I2）。375×667。 | **2026-07-22**：静默失败批 3。**2026-07-25**：Choose 后点选仍 Notice → 改鞠躬后展开 Companion。**同日再书面（5174）**：完整一场后 Rise → hint → Here & Now **又出 Notice**（不对，应立刻 Focusing）→ 根因 beginFocus/Rise 清掉门闩；已改为 Arrival/⚡ 解锁后跨会话保持；e2e J。 **2026-07-25 用户书面（5174）**：Sit→Choose Reading→鞠躬后见三选一（仍 Sit）→ Here & Now 立刻 Focusing、无 Notice — **测试 OK**。 **同日再书面（5174）**：Focus→Rise（Skip all）→ How shall we sit? → Here & Now **立刻 Focusing、无 Notice** — **测试 OK**。 **2026-07-25 晚用户书面（宽屏 · P0 ③）**：Companion 点选即开表 — **测试 OK**（与上行 Companion Mode 同批）。 | `?product=1` · e2e A/A4/A4b/J · `#btn-focus` | 2026-07-25 |
...
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 有问题 | 1) 先点掉自动提示。2) 点 **?**。3) Idle 点 ? → **先见 1 条情境 tip**（通常 Sit）+ **`#ft-hint-catalog-chip`「还有 N 条」** + **用途简介卡**；点芯片再展开 How / 热力图 / 呼吸 / Sound 等。4) Honesty 桥接打开时点 ? → 须见 **honesty-bridge** tip + 热力图/提醒/Sound（**不见** micro-ritual）。5) 点气泡关该条；再点 ? 仍全部出现。 | 2026-07-20：只有元文案。**2026-07-21**：加简介卡。**2026-07-22**：A 类测试 OK 后用户再书面缺 tip → **已补登记**四类 + 更新 help-remedy；请硬刷新复测。**2026-07-25**：补 `in-app-reminder`（每日时分），**须复测** Idle「?」是否含时钟 tip。 **2026-07-25 窄屏 375 用户书面（图3）**：点 ActionBar **?** → 一大群补救 hints **乱指**底部不存在的按钮（park 后叠成一团）——须修：窄屏 park 态 tip 须 remap 到 ActionBar / 抽屉 grabber 等可见锚点，禁止指屏外 park 坐标。 **2026-07-25 图3 本地已改（待复测）**：`ft-narrow-park` 下补救 tip remap（含 focus-hud）+ 同锚错开；e2e 375 锁近 ActionBar/grabber。 **2026-07-25 晚用户书面（图3/图6/图7）**：冷启动 Idle 点 ? 仍乱指；**Reminder 面板打开后再点 ?** 仍乱指（图6，新）；Companion 三选一时点 ? 得「Pick one…」体验好（图7）但未普及。**注**：当次 5174 曾指向主仓非 worktree。 **2026-07-25 深夜用户书面（worktree @5174）**：图3/6/7 乱指 **已测试 OK**；**图9 新反馈**：补救 tips 不再乱堆，但五条竖叠仍挡角色——须优化展示（待拍板：情境单条 / 底栏目录 / 抽屉内列表 等）。 **2026-07-26 用户拍板 + 本地已改（待人工测试）**：情境单条（Idle→Sit；Companion→Pick one；其余「还有 N 条」点 help-remedy 展开）；e2e `375 park: ? remedy shows one primary tip + catalog expand`。 **2026-07-26 用户书面（375）**：图12 `?` 下「还有 N 条」芯片 — **测试 OK**。 | `#onboarding-hint-help` · `#onboarding-app-purpose` · `#reminder-preference-toggle` | 2026-07-26 |
...
| 用户场景剧本 SCENARIO_TESTS（A–H + I–P） | UI可见 | 待人工测试 | 权威：`focus-tiger/docs/SCENARIO_TESTS.md`。用 **`?product=1`** 走完整故事串。**新增 O**（7 格热力图）· **P**（应用内提醒 + suppress/defer 说明）。**单元/控制器** `npm run test:smoke`；**DOM 用户链路** `npm run test:e2e`（覆盖层见文首 §A 与 SCENARIO 各场景标题——**全绿 ≠ 故事走完**）。**观感子项已拆成下方独立行，勿只勾本行。** | 2026-07-22：新增正式场景 O/P。**同日**：核对「已自动化」口径，禁止笼统宣称整条故事已锁。 **2026-07-25 晚用户书面（宽屏 · P0）**：场景 **C**（中途 Rise）**测试 OK**（⑤）；场景 **P** **测试 OK**（⑧）；场景 **O** **有问题**（⑦，见 L232；另线 `fix/scenario-o-375-chrome-layout`）。 | `SCENARIO_TESTS.md` · `?product=1` | 2026-07-25 |
