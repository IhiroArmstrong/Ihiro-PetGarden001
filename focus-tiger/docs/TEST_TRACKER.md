...
| 窄屏 ActionBar · Idle 显示本机电脑时钟 | UI可见 | 待人工测试 | **主路径（375）**：Idle ActionBar 中心时间应为**本机墙钟**（如 `12:30`），**不是**会话累计 `00:00`。Focusing 仍可回 `#focus-hud` 会话计时。**回流**：Reminder / Arrival 开关后 Idle 再看仍是墙钟。 | **2026-07-27 用户书面（三 Bugs · 第三）**：顶栏 timer 时有时无、总是 `00:00`，建议改成电脑时间当 clock。**2026-07-29**：在正确基线 `a1dbc9c` 复测仍见 `00:00` + Calm — **未实现**（PR#10 只修 Reminder 溢出，未改 `_syncActionBarFromHud` 镜像 `#hud-time`）。 | `.ft-narrow-action-bar__time` · `NarrowIdleShell._syncActionBarFromHud` | 2026-07-29 |
