...
| **i18n**（v1 可点切语 · 已拍板） | **下一回合 A+B+C**。六语方向；**审完再露**（未 `ready` 不进选择器）。完整可点先 **en+zh**。见审计 **§9.6** | 场景 G |
...
| i18n（默认 en / 可切多语） | UI可见 | 待人工测试 | **拍板**：v1 **可点切语** + 记忆；六语方向；**审完再露**（es/ja/de/fr 未 ready **不进**选择器、不声称）。完整可点先 en+zh。**下一回合** A+B+C。人工：启用语 375 排版。 | **2026-07-30**：可点切语；六语审完再露（拒机翻先上）。 | `?product=1` · Language 入口 · `src/locales/*` · 审计 §9 | 2026-07-30 |
