...
| starlight-cloak-*（入库保留） | UI可见 | 待人工测试 | **勿接主线**。已用 `Yin_Starlight_Cloak_Sleep_forward_v3` **替换** `starlight-cloak-sleep` / `-wake` 各 67 帧（物理倒序苏醒同步重建）。产品仍用 `cloak-sleep`。**须验**：实验室自愿单条试播正放/倒序——抠图边缘、披斗篷→趴下、尾帧睡姿是否干净。过关后再议接线。 | **2026-08-04 早**：v1 抠图/观感不如旧 → Undo。**同日晚**：用户提供 v3，已换帧待复测。 | legacy-unclassified | legacy · 暂不进逾期扫描；发布前人工过目 | `#emotion-debug-ui`（可选） · worktree Vite | 2026-08-04 |
