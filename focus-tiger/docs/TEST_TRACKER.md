...
| 浏览器 e2e harness · storage wipe + preview webServer | 纯后端 | 仅单元测试覆盖 | **2026-07-26 根因**：`openFreshProductShell` 曾在**每次**导航 wipe `focus-tiger.*`，heatmap seed→`reload()` 后种子被清 → `expected lit for YYYY-MM-DD`（日期来自 `shift(-6)` 相对日，**非**硬编码产品日、**非**产品 lit 逻辑错）。已改为 **每 browser context 只 wipe 一次**（`sessionStorage` gate）。全量套件用 **`vite preview`**（避 `vite dev` 中途挂死）：**CI** production build；**本地** `vite build --mode development` 以保留实验室 DEV 钮。提醒 e2e 须生产构建也暴露 `__inAppReminder` / `__dailyCompletionStore`（同 `__honestyBridge` 口径）。覆盖层：**DOM e2e** harness；**不**锁序列观感。 | — | `e2e/helpers/product-shell.js` · `playwright.config.js` · `weekly-practice-heatmap.spec.js` · `src/main.js` | 2026-07-26 |
