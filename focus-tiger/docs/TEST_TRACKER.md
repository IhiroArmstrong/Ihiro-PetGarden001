...
| Hints 视觉护栏试点（④ mint / 几何 / tip 软快照） | 纯后端+e2e | 仅单元测试覆盖 | **无产品行为变更**。选型：禁全页 Yin；主护栏 = mint `::after` RGB + tip↔锚点 boundingBox；辅 = tip 元件 `toHaveScreenshot`（animations disabled）。单元：`hintsVisualGuardrail.test.js`；e2e：`hints-visual-guardrail.spec.js`。Brief：`task-hints-visual-guardrail-pilot.md`。**明确**：机械护栏 ≠ 人工观感关单；尖角「感觉对不对」仍以 TEST_TRACKER 书面为准。 | **2026-08-03** 用户书面：#92 已合；有意义/中风险则安排视觉快照护栏（早抓尖角偏 / mint→奶油）。 | — | — | Brief · `e2e/hints-visual-guardrail.spec.js` · helper | 2026-08-03 |
