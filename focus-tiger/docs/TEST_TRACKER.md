...
| 场景动画 · 设计师整合 + 库存接线政策（文档） | 纯文档 | 仅单元测试覆盖 | **无运行时**。整合设计师场景×动画清单；库存「仅调试」→ Slice B/C；驳回完成池 dance；标 ja 合十代码漂移（A′）。 | **2026-08-01** 用户书面：整合意见；仅清单素材应全部接入业务。 | `SCENE_ANIMATION_WIRING` · `ASSET_INVENTORY` · `task-scene-animation-inventory-wire-slice-b.md` | 2026-08-01 |
| 场景动画 · Slice A（切语合十/鞠躬 + Honesty Idle 点头） | UI可见 | 待人工测试 | **主路径**：`?product=1` → Language → **日本語** → 阿寅**合十**（`palms-together`）→ Idle；再 **English** → 鞠躬（`nod-bow`）；同日再切同一语**不**重复播。Honesty Idle：选时长 → 呼吸结束 → toast **且**短点头 → 桥接。睡态 Honesty：仅 dormantWake，**不**叠点头。微仪式完成仍 `sessionComplete`。**回流**：Focusing 中切语跳过（不补发）。**禁止** Celebrating。375 竖屏各走一遍。 | **2026-08-01 文档核对**：代码 ja→`intentionSet`→实际 **nod-bow**（非合十）——验收「合十」前须先做 Slice A′，或本行暂记「画面为鞠躬、规格未对齐」。 | Brief `task-scene-animation-wiring-v1-slice-a.md` · A′ 见 Slice B Brief · `?product=1` · `localeGreeting.test.js` · `e2e/language-switch.spec.js` | 2026-08-01 |
