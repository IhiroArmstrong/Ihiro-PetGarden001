...
| Ambient Soundscape / 默认 Mer-Ka-Ba + 开关 | UI可见 | 待人工测试 | **主路径**：进应用即有显眼 **「关闭音乐」**（默认 Mer-Ka-Ba）；点一下立刻静音变 **「打开音乐」**；再点开回。未 Sit 也可开关。次要「曲目」可选另一首。**回流**：关音乐 → 刷新 → 应保持关闭；再开 → 刷新应保持开。浏览器拦自动播放时点按钮/页面可解锁。 | **2026-07-21**：用户书面——默认要有 Mer-ka-ba，且显眼按钮随时可关。已改默认开+一键开关（不再门闩于 FOCUSING）。**此前**：2026-07-20 入口测试 OK（旧 gated 口径作废）。 | 右下角 · `__ambientSoundscapeUI` | 2026-07-21 |
