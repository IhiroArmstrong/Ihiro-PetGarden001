...
| 「本周陪伴」7 格热力图 UI（Idle 常驻） | UI可见 | 待人工测试 | **主路径**：`?product=1` Idle → 见 7 格。**宽屏 ≥480**：左下 `?` 上方。**窄屏 ≤479 / 375**：主画布清场——**ActionBar**（? · Calm/time · ♪）+ Yin 放大居中 + **上滑抽屉**汇总 Sit/Honesty/呼吸/How/Sound/提醒；7 格在抽屉内安静展示；底栏无散落按钮。**回流**：Focusing → 抽屉收起、Rise 仍一键；Rise 回 Idle 再出 grabber。**DOM**：e2e `weekly-practice-heatmap.spec.js`（含 375 ActionBar/抽屉）。 | **2026-07-22**：缺 heatmap Hint。**2026-07-24**：DevTools 375 dock 重叠 → 先抬簇 HUD 下。**同日用户书面：375 观感不能通过**，要求 ActionBar + 上滑 BottomSheet + 清底栏（本行）。**须 375 复测**新壳。 | `?product=1` · DevTools 375×667 · `#ft-narrow-idle-shell` | 2026-07-24 |
