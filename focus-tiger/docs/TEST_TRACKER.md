...
| starlight-cloak-*（入库保留） | UI可见 | 待人工测试 | **勿接主线**。已用 `Yin_Starlight_Cloak_Sleep_forward_v3` **替换** `starlight-cloak-sleep` / `-wake` 各 67 帧（物理倒序苏醒同步重建）。产品仍用 `cloak-sleep`。**实验室**：入库素材点 `starlight-cloak-sleep v3(仅试播·勿接)` / `…wake…`。**须验**：抠图边缘、披斗篷→趴下、尾帧睡姿。过关后再议接线。 | **2026-08-04 早**：v1 抠图/观感不如旧 → Undo。**同日晚**：用户提供 v3，已换帧 + 试播钮待复测。 | legacy-unclassified | legacy · 暂不进逾期扫描；发布前人工过目 | `#emotion-debug-ui` · worktree Vite | 2026-08-04 |
...
| dormantWake / Honesty 睡醒序列 | UI可见 | 已通过 | **主路径**：进 DORMANT 后「Honesty唤醒」选时长 → **`cloak-sleep` 34 帧倒放**（**6 fps** ≈5.7s）→ 定格末帧 + **约 10s 呼吸**并行。**回流**：定格后桥接 Yes/No。**对比**：「唤醒(伸懒腰)」仍 stretch-reminder。 | **2026-07-21～22**：旧 cloak 倒放曾书面通过。**2026-08-04 用户书面**：starlight 苏醒观感不如原来（对比伸懒腰也更差）→ **Undo** 回 cloak 倒放。 | — | — | Honesty / `#emotion-debug-ui` | 2026-08-04 |
