...
| 「本周陪伴」7 格热力图 UI（Idle 常驻） | UI可见 | 待人工测试 | **主路径**：`?product=1` Idle → 见 7 格。**宽屏 ≥480**：左下 `?` 上方。**窄屏 ≤479 / 375**：主画布清场——**ActionBar**（? · Calm/time · ♪）+ Yin 放大居中 + **上滑抽屉**汇总 Sit / Quick Start / Honesty / 呼吸 / How / Sound(静音) / 提醒；7 格在抽屉内安静展示。**Focusing / 一分钟呼吸**：左上 `#focus-hud` 计时卡回来；右下 Sound FAB 隐藏，右上 mute 可关。**回流**：Rise → grabber 再出；♪ 可静音（会话结束**不停播**，须点静音）。**DOM**：e2e `weekly-practice-heatmap.spec.js`。 | **2026-07-24**：用户 375 书面附图——① `?` hints 指空；② Focusing Sound FAB 不协调；③ Quick Start 闪电消失；④ Focusing/呼吸 HUD 计时卡消失；⑤ Honesty toast 与桥接重叠；⑥ 抽屉 How/Sound/提醒无响应；⑦ Rise 后无法关音乐。已改 hint 重锚 / handlers / HUD / park / toast 上移 / mute API；**须 375 复测**。 | `?product=1` · DevTools 375×667 · `#ft-narrow-idle-shell` | 2026-07-24 |
