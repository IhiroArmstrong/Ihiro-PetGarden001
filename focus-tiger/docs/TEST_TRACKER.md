...
| | 人工 · 点 ? 补救（主条 +「还有 N 条」逐条展开） | UI可见 | 已通过 | **主路径（375）**：先点掉自动提示 → 点 **?** → 见 **情境主条** tip（如 Sit）+ **「还有 N 条」芯片** + **用途简介卡**（非一次铺开全部）。点芯片 → **只多出 1 条**目录 tip，芯片 N 递减；再点芯片替换下一条。**禁止**底部多 tip 叠成乱指。**回流**：关气泡 / 点外侧收起后再点 ?；Rise 后再点 ?。目录可覆盖：How shall we sit? / 热力图 / 提醒 / 一分钟呼吸 / Sound gated 等（逐条，非同时）。 | **2026-07-20**…**2026-07-26**：芯片迁回。**2026-07-26 晚用户书面（5176 · 375）**：Sit→Arrival→⚡ Focusing / 抽屉呼吸无旧 Sit — **测试 OK**；但点 ? → 主 tip +「还有 N 条」→ 点芯片展开其余后**页面下方很乱**（多 tip 重叠）。已改芯片**逐条**展开（同时最多主条+1）；e2e 锁不洪水+不重叠。**2026-07-26 夜用户书面（5176）**：逐条展开 — **测试 OK**；但 **hints 会被主球挡住**（热力图 tip 在球后）。已改：次要 tip 锚 `#ft-narrow-home-ctas` 上方 + 相交上抬 + tip z-index 34；e2e 锁不与主球带相交。**2026-07-26 夜用户书面（5176）**：tip 完整可读、不钻进三球后 — **测试 OK**。 | `#onboarding-hint-help` · `#ft-hint-catalog-chip` · `#onboarding-app-purpose` | 2026-07-26 |
