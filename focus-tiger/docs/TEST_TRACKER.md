...
| starlight-cloak v5 + 经典 50/50 | UI可见 | 有问题 | **主路径**：进 DORMANT / Honesty 睡醒 → classic 与 starlight v5 **约各半**（同轮入睡与苏醒变体匹配）。**实验室**：入库素材点 `starlight-cloak-sleep v5 正放/睡循环/苏醒` 与经典 cloak。**Wellness 2A**：≥23 或 <06 冷启动披斗篷 + 夜深 toast；06–10 苏醒仪式 + 晨光 toast；10–23 仍 Idle 开局。**回流**：白天硬刷新不得因旧 2h 戳开场即睡。 | **2026-08-04**：v1/v3 曾拒；同日用户提供 v5 + 要求与旧斗篷并存 50/50 + 2A。**同日用户书面（实验室 `/`）**：starlight 正放【测试OK】；苏醒【测试OK】；睡循环【须身体微微鼓起/恢复表睡中呼吸】。**处理**：扩 starlightSleeping 帧幅 067–058 + CSS `ft-sleep-breathing` 微起伏（经典 sleeping 同开）。50/50 产品路径仍待测。 | 轻微（观感） | 本回合已改睡循环呼吸；复测实验室睡循环后可回「待人工测试」 | worktree Vite · 实验室 `/` · `?product=1` | 2026-08-04 |
