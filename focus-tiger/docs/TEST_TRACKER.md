...
| 开场即睡 / 冷启动第一幕 Idle | UI可见 | 待人工测试 | **主路径**：本地已有 ≥2h 前的 `focus-session-end` → 刷新 / 首次打开 `?product=1` → 阿寅须 **Idle 闭目坐禅**（有精神的坐姿呼吸），**不得**立刻播披斗篷→睡着。Honesty 小钮仍可出现（零完成时）。**回流**：切到后台再回前台（或显式 live sync）且仍 ≥2h → 此时才可披毯进 DORMANT（场景 D 仍有效）。**对比**：实验室「重置全部本地状态」开局亦 Idle。 | **2026-07-25**：用户书面——进入页面后开场即播披斗篷趴下（当时标「另案」未修）。**2026-07-26**：用户书面——每次第一次试用又是披斗篷睡着；要刚开始看到有精神的 Yin。根因：2h 滚动 DORMANT 在 `onAppReady` 重播 cloak；冷启动现禁进睡。自动化：`dormantIdle` + smoke A1b。 | `?product=1` · 改/保留 `focus-session-end` 后刷新 · `npm run test:smoke` | 2026-07-26 |
