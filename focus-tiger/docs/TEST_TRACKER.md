...
| Lit 试点 · OnboardingHintsUI（步 4） | UI可见 | 待人工测试 | **主路径**：实验室「清空引导提示已读」→ 见 `help-affordance`（Lit 气泡）。点 **?** → `help-remedy` + 本页全部锚点提示。**回流**：关气泡后再点 ?；Rise 后 FOCUSING 再点 ?（Rise+Sound）。尖角对准见下行。实现：`ft-onboarding-hint-bubble` + `lit`。 | 2026-07-21：试点目标 OnboardingHints；代码已落地。**同日拍板**：复测通过后先停试点、不扩其它模块。请复测。 | `#onboarding-hint-help` · `ft-onboarding-hint-bubble` · Brief | 2026-07-21 |
...
| 人工 · help-affordance 尖角对准 ? | UI可见 | 待人工测试 | 1) 清空引导提示已读。2) `help-affordance` 气泡在 **? 右侧**，**左侧尖角须对准 ? 圆心**（窄屏 clamp 后亦须对准）。3) 回流：缩放窗口后再看尖角。 | 2026-07-20 用户书面：如图，hint 指向没有对准按钮。**2026-07-21**：已加 `--tip-x/--tip-y` 对准锚点中心；请复测。 | `?product=1` 或 `/` · `#onboarding-hint-help` | 2026-07-21 |
| 人工 · 点 ? 补救展示本页全部 hints | UI可见 | 待人工测试 | 1) 先完成一轮操作让 Sit / How shall we sit 等提示**已读消失**。2) 点左下角 **?**。3) 须**同时**出现：? 旁元文案 + **Sit 上方**「点击与阿寅同坐」+ **How shall we sit? 旁**「也可以从这里开始」等各控件锚点提示（FOCUSING 时则为 Rise + Sound 旁）。4) 点单个气泡只关该条，不记已读。5) 回流：关闭全部后再点 ? 仍全部出现。 | 2026-07-20 用户书面：点 ? 只有元文案，没有 hint 说的「都在页面上」的效果。**2026-07-21**：`resolveRemedyHintIds` + `_remedyIds` 已铺开本页全部；请复测。 | `#onboarding-hint-help` · 实验室「清空引导提示已读」或先点掉自动提示 | 2026-07-21 |
