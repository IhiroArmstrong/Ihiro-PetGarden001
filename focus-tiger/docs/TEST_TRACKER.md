...
| Safari Companion **底部横排**仍挡/错位 | 待人工测试 | e2e 未测 WebKit 布局 | **[L126](#L126)** `Companion Mode`（须 **Safari** 复测；只用 Chrome 可降级） |
...
| Companion Mode / 陪伴模式三选一（Here & Now · Offline Space · Flow State） | UI可见 | 待人工测试 | **主路径**：① hint → 任选模式 → Arrival → **Skip — begin 或 Choose/鞠躬后立刻 Focusing + 计时**，**无需**再点 Sit。② Sit → Arrival → 三选一 **点选即计时**（含 Offline）。**回流**：Rise 后再 hint → 任意模式 → Arrival 结束仍自动开表。**Safari 专项**（仅当你还用 Safari）：Choose 后底部 Companion 横排矮条是否挡鞠躬/错位；Chrome 可降级不测。 | **2026-07-21**：Flow/Reading 鞠躬开表已修。**2026-07-21 晚**：Offline 亦禁止二次 Sit。**2026-07-22**：用户书面问 Safari 横排「需要测试啥？」——见回复；Chrome 日常可跳过本专项。 | 底部 Sit 旁 dock · e2e A/K · Safari 人工 | 2026-07-22 |
...
| Tiger Reflection Moment / 结束反思 | UI可见 | 待人工测试 | 正常完成或主动 Rise 结束会话 → 留白约 400ms（完成）/ 300ms（主动）后淡入面板。可选回显本次意图。Q1–Q3：Continue / Skip / Skip all / Esc。非空答案写入 `focus-tiger.reflections.v1`（最近 5 条）；全跳过不落库。 | **2026-07-22**：用户书面问「这个需要测试啥？」——见回复说明；仍待书面验收。 | 会话结束后自动 · 单测：`TigerReflectionMoment.test.js` · DEV：`__reflectionMoment` | 2026-07-22 |
...
| 「?」朱砂 notification-badge | UI可见 | 待人工测试 | **主路径**：新用户左下「?」角有朱砂小红点；点一次「?」后红点消失且不再常驻。**回流**：DEV 重置 hints 后再见红点。勿做成常驻角标噪音。 | 2026-07-21：稀缺强调色。**2026-07-22**：用户书面——朱砂红点「用于系统里面的通知，或者 alert 之类的」（未标测试 OK；现实现仍挂 onboarding「?」未读）。待拍板是否改挂提醒/通知。 | `#onboarding-hint-help` · `/?product=1` | 2026-07-22 |
