...
| **i18n**（v1 可点切语） | ✅ **A+B+C 已落地**（2026-07-30）：unit `i18n.test.js` 入 smoke；e2e `language-switch.spec.js`；⋯/抽屉 Language → 面板；`focus-tiger.locale.v1`；**审完再露**（仅 en/zh） | 场景 G |
...
| Onboarding · 首次登录右上音符薄荷绿圆点 | UI可见 | 待人工测试 | **主路径**：实验室清空引导已读（须在 **非** `?product=1` 的实验室页）→ 硬刷新 `?product=1` → 右上音符钮见**明显薄荷绿脉冲**（约 11px，`has-hint-mint` ::after）。**回流**：仅**选曲**后圆点消失；单次静音**不**清圆点。 | **2026-07-30 用户书面**：清空后再验仍无。→ 根因：`onMuteChromePainted` 在 `let onboardingHints` TDZ 前触发 → **整页 init 崩溃**；改 `onboardingHintHost` 桥接。加大 mint。 | `?product=1` · `.ambient-soundscape__mute.has-hint-mint` | 2026-07-30 |
...
| i18n（默认 en / 可切多语） | UI可见 | 待人工测试 | **已落地 A+B+C**：Idle → ⋯ / 抽屉 **Language** → 面板选 **中文** → Sit 等为中文；再选 **English** 切回；刷新保持。es/ja/de/fr **不出现**。自动化：`npm run test:smoke`（含 `i18n.test.js`）；`test:e2e:changed -- e2e/language-switch.spec.js`。人工：375 中文排版。 | **2026-07-30**：可点切语 + 审完再露；A+B+C 已实现。 | `?product=1` · `#language-preference-panel` · 审计 §9 | 2026-07-30 |
