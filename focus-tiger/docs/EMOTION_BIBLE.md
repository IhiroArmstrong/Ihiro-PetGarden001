...
| `Sleeping` | `sleeping`（2D）/ `SLEEPING`（3D 垫底） | 2D：定格 `cloak-sleep`/`starlight` 入睡末帧 + 躯干呼吸层；旧 `sleeping/` 保留；GLB `tiger-sleeping.glb` |
...
| 0.89 | 2026-08-04 | `Sleeping`：定格入睡末帧 + 躯干 clip 层 scaleY 呼吸（蒲团/镜头不动；取代整图缩放与末帧 pingpong） |
