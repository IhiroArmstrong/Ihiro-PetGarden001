...
| 夜晚使用 | 披着小毯子，动作放缓 | |
| 清晨使用 | 打哈欠、伸懒腰 | |
| 无互动约 10 分钟 | 看向用户方向并挥挥手（轻量提醒），**或**闭眼继续冥想（二选一或随机） | **中间层级**。设计原则：轻量、不打扰；禁止频繁弹窗或紧迫感呼唤——产品目标是**陪伴而非争夺注意力** |
...
|---|---|---|
| 连续专注达到一定时长（如 20 分钟；具体阈值待与 Focus Confidence 机制对齐确认） | 双手合十、微笑、做一次呼吸动作（**非**跳跃 / **非**粒子爆发） | `MINDFUL_FOCUS_MILESTONE`（轮换文案池，3–5 句） |
| 连续工作过长时间（如 2 小时无休息迹象） | 温和提醒动作（如伸懒腰姿态） | `STRETCH_REMINDER`（轮换文案池，3–5 句） |
...
1. 「连续专注 20 分钟」是否与 Focus Confidence / Flow Continuity 信号对齐，还是仅用会话计时墙钟时长？
2. 「2 小时无休息」的计时口径：活跃专注累计、还是页面打开时长？是否与 Interruptions 互斥重置？
3. 同日同一类提醒是否限频（建议限频，避免打扰）。
...
- 文档/代码注释可保留英文（不影响 UI）。
- 所有 UI 展示文案必须走字典机制。
- 预留全局当前语言状态；英文字典可后续批量翻译，现阶段仅需切换骨架（可有空的 `en.json` 占位）。
