...
| `Celebrating` | 庆祝（跳跃欢呼） | 否（一次性播放） | 专注数据**当日首次达标**（如番茄钟/会话达到目标分钟数）；每个自然日仅触发一次，以日期戳判断；同日后续达标仅静默记录 | **100**（最高；播放期间临时夺取基底姿态，播完切 `Smiling`） | **已实现**：GLB `tiger-happy-jump.glb`，`PoseManager` 中 `CELEBRATING`；**部分实现**：`MoodController` 可播 `CHEER` + `TransitionFX`，完整日期戳防刷与姿态切换待接通 |
...
| `WakeUp` | 唤醒起身 | 否（约 2s 过渡） | 用户从 `Sleeping` 态点击老虎，完成最短 1 分钟唤醒仪式/深呼吸引导后播放；完成后回到 `Idle`（非满金，需正常专注会话重新染金） | **90**（高于 `Sleeping`，低于 `Celebrating`） | **待实现**：`Actions.js` 已定义 `WAKE_UP` 枚举与 clip 别名，无运行时触发与资源 |
...
```
Celebrating (100)  >  WakeUp (90)  >  IncenseComplete (80)
...
1. **当日首次达标**：`Celebrating` 夺取展示 → 播完自动切 `Smiling`；同日再次达标不重复庆祝。
2. **长期未专注**：`Sleeping` 覆盖 `Idle`；用户未完成唤醒仪式前保持瞌睡；完成后 `WakeUp` → `Idle`。
3. **一炷香完成 vs 专注达标**：`IncenseComplete` 与 `Celebrating` **相互独立**、强度分级（轻量确认 vs 完整庆祝），不共用跳跃欢呼资源；可同一天先后发生，各自遵守「每日一次」类限制。
4. **每日总结氛围**（雪花 / 花瓣）与实时姿态是**两条独立信号轴**（`DESIGN.md`），可同时叠加，不并入本表姿态状态机。
5. **专注染金**（`focusLevel` 材质插值、金粒子、光环）由 `FocusVisualizer` / 动态效果层驱动，**不是**独立基底姿态；与 `Idle` 等姿态正交叠加。
...
**措辞/表现已按修正版定稿**（不以原始负面/焦虑版本为准）：鼠标绕圈、专注中断、长时间无互动、摸头与欢呼互斥规则；以及本文升级后的「宠物感」互动设计。
...
| 眼睛跟随鼠标（持续追踪） | 瞳孔持续、平滑朝向鼠标方向偏移；鼠标离开浏览器窗口后恢复默认冥想朝向（正前方或微闭，视当前主状态） | **持续性效果，非事件触发**。**实现方式（重要）**：不通过 PNG 序列穷举注视方向；将眼睛/瞳孔拆为独立可移动图层，叠于脸部主体 PNG 之上，程序在受限范围内跟随鼠标坐标。**优先级：高**（宠物生命感核心手段，建议优先实现） |
| 鼠标停留并点击头顶 | 微笑、眯眼 | |
...
| 专注持续 5 分钟 | 做一次深呼吸动作 | |
| 完成一次专注会话 | 开心鼓掌、小跳一下 | 对应已定义的 `Celebrating` 状态触发规则 |
| 连续完成多个专注会话 | 双手合十，开心庆祝 | |
...
| 夜晚使用 | 披着小毯子，动作放缓 | |
| 清晨使用 | 打哈欠、伸懒腰 | |
| 无互动约 10 分钟 | **加权随机**（非五五开）：**70%** 闭眼继续冥想（不主动引起注意）；**30%** 看向用户方向并挥挥手 | **中间层级·已确认**。设计原则：轻量、不打扰；禁止频繁弹窗或紧迫感呼唤。**备注**：挥手相对主动、引人注意；若与安静冥想等概率随机，长期使用会显得频繁呼唤用户，与「不打扰、不干扰专注」原则存在张力。以安静冥想为主、挥手为偶尔小变化，既保留生命感随机性，又不破坏安静陪伴基调。 |
...
| 刺激源 | 老虎反应 | 备注 |
|---|---|---|
| 连续签到 7 天 | 身边出现莲花、金色光点 | |
| 连续签到 30 天 | 袈裟颜色升级、光环升级 | 与 `PROCESS.md` Backlog「奖励系统（金牌 + 3D 塑胶公仔展示）」关联，属同一套里程碑奖励体系的视觉表现，后续应统一设计，不重复规划 |

---

## 第六部分：阶段性正念认可 MindfulAcknowledge（区别于 Celebrating）

这是比 `Celebrating`（达标狂欢：跳跃欢呼 + 金光爆发）**更轻量、更安静的中间层反馈**，用于阶段性专注确认，**不追求视觉强度**。实现时通过 `EmotionController.playEmotion('mindfulAcknowledge' | 'stretchReminder')` 触发动作，文案只引用字典标识符，不写死在业务逻辑里。
...
- 强度分级：`IncenseComplete`（轻量完成）＜ `MindfulAcknowledge`（阶段性安静确认）＜ `Celebrating`（当日达标狂欢）。
...
- 文案统一放在独立语言字典文件：`locales/zh.json`、未来的 `locales/en.json` 等。
- 业务逻辑只引用**文案标识符**（如 `MINDFUL_FOCUS_MILESTONE_1`）。
- 由统一的文案获取函数（见 `src/locales/i18n.js`）根据**当前语言设置**从对应字典取出实际文字。

### 当前阶段

- **优先**完整维护中文字典 `src/locales/zh.json`。
- 文档/代码注释可保留英文（不影响 UI）。
- 所有 UI 展示文案必须走字典机制。
- 预留全局当前语言状态；`en.json` 现为空壳。**英文翻译不在当前阶段启动**（触发条件见 `PROCESS.md` Backlog「英文文案翻译」）：须待 2D 主线正式素材稳定上线且中文文案定稿后，再批量翻译，避免措辞反复修改带来的重复劳动。
