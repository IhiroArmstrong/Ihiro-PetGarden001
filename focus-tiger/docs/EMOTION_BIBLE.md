...
#### 3. 提醒共享限频池（因新增 Re-focus Acknowledge，2026-07-15 重新开放拍板）

- `MindfulAcknowledge`、`stretchReminder` 与 `Re-focus Acknowledge` 共用同一**同日 / 同会话提醒上限池**，避免多种非模态提示各自计数后叠加造成打扰；
- 原「每种类型各自每日最多 3 次」口径不再视为最终定稿；**共享池的同日总上限仍待拍板**；
- `Re-focus Acknowledge` 单次专注会话建议最多 **1–2 次**，具体取 1 还是 2 仍待拍板；
- 超过共享池限额后不再触发视觉/文案，可静默记录事件本身（不影响其它数据统计逻辑）。
...
本节只规范**怎样说**，不决定**何时说、一天说几次**。原有 20 分钟阶段认可与 2 小时伸展提醒的判定基准继续有效；因新增 `Re-focus Acknowledge` 而重新设计的共享同日 / 同会话限频池仍待 `PROCESS.md` 速览中的「提醒文案共享限频」事项拍板。
...
本设计依赖 Focus Confidence V1 运行时信号链路（Page Visibility / `window.blur` / idle 检测）。该链路目前只有 `DESIGN.md` 设计，尚无独立实现模块（见 `PROCESS.md` 速览「明确未完成」）。

因此本次只记录设计规范，**不实现代码、不新增情绪键、不新增语言字典文案**。待 Focus Confidence 信号链路完成后，再单独排期实现与验收。

### 触发判断逻辑（初始建议，阈值待拍板）

1. **分心事件最小定义**：
   - 页面 `visibilityState` 变为 `hidden`，或窗口触发 `blur`；
   - 持续时间达到最低阈值后才记为候选事件，建议从 **15–30 秒**起；
   - 必须排除用户主动点击「结束专注」、会话自然结束、浏览器关闭等明确结束路径；
   - `idle` 可作为 Focus Confidence 的补充信号，但不能单独推断用户的具体情绪或分心原因。
2. **反馈触发门槛**：
   - 只有分心持续时间相对明显时才提示，初始建议为**超过 1 分钟**；
   - 零星几秒的标签切换、系统弹窗或短暂失焦不反馈，避免产品显得监视或过度敏感。
3. **频率控制**：
   - 与 `stretchReminder`、普通 `MindfulAcknowledge` 共用同一同日 / 同会话提醒上限池；
   - 单次专注会话建议最多 **1–2 次**；
   - 共享池同日总上限、单会话最终取 1 还是 2，均列入 `PROCESS.md` 待确认事项。

### 呈现方式

- 复用现有 `MindfulAcknowledge` 的非模态文案条 UI，不新建 UI 组件；
- 只在用户重新获得页面焦点 / 可见性的瞬间呈现，不在分心过程中追加提示；
- 不弹窗、不追问「为什么」、不要求点击确认、不要求用户解释；
- 动作与文案短暂停留后自动消失，并回归基础坐姿呼吸状态；
- 每条文案遵循「**察觉 + 接纳 + 回归**」三段结构，并准备多个中英文变体轮换。

### 文案变体示例（仅作设计样稿，未进入语言字典）
...
### 历史中文文案池（已写入 `src/locales/zh.json`，待观察式改写）

> **0.10 审核结论**：下列文案产出早于「观察式措辞」原则，其中「做得很好」「继续，慢慢来」「站起来走走」等包含评价或指令语气，**不再视为最终产品文案**。本次任务仅更新设计文档，不修改代码或语言字典；实现非模态文案 UI 前须另开文案审校任务，同时更新 `en.json` / `zh.json` 并保持同构。
