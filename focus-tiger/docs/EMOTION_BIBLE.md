...
| `ParrotEarVisit` | 鹦鹉耳边造访（禅意信使） | 否（93 帧 @ **8 fps** ≈11.6s） | **场景 A**：应用内轻提醒横幅（`#in-app-reminder-banner`）本页会话**首次**变为可见时伴随播放；**场景 B**：轻完成 / 微仪式池稀有彩蛋（`LIGHT_COMPLETE_POOL` 低权重）。**不**替换 `MilestoneGlow` streak-7 | **68**（高于 Idle / IntentionSet，低于 SessionComplete / Celebrating / MilestoneGlow） | **已实现（2D）**：`parrot-ear-visit-feather`；正放一次 → ~1s CapCut Idle。羽毛界面残影 /「可收集」面**未做**（禁宠物收集叙事；若做仅视觉残影） |
...
| `ParrotEarVisit` | `parrotEarVisit` → 同名序列 | `public/sprites/.../parrot-ear-visit-feather/frame_001–093.png`（场景 A 提醒信使 · 场景 B 稀有完成彩蛋） |
...
| 0.84 | 2026-08-03 | 入库 `parrotEarVisit`（鹦鹉耳边造访）；场景 A 应用内轻提醒信使；场景 B 轻完成池稀有彩蛋；不替 MilestoneGlow |
