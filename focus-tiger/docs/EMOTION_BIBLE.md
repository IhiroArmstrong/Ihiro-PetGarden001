...
| 刺激源 | 老虎反应 | 备注 |
|---|---|---|
| 连续练习达到里程碑节点 | 解锁只增不减的环境纪念细节 | 已确认方向（初始建议节点）：3 天小香炉、5 天首朵莲花、10 天第二朵莲花（后续逐步至满池）、30 天蒲团刺绣、60 天夜间小灯笼 + 白天小茶盏；详见 `PROCESS.md` Backlog「纪念奖励系统」；只增不减、中断不撤回、不制造断签压力；节点数字实现时可统一微调 |
