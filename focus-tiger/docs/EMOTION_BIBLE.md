...
| `Sleeping` | `sleeping`（2D）/ `SLEEPING`（3D 垫底） | 2D：定格入睡末帧 + 背部-only 呼吸层；旧 `sleeping/` 保留；GLB `tiger-sleeping.glb` |
...
| 0.96 | 2026-08-18 | **收回 Expand B**：Rise / 计时达标进 Reflection 不得 `cloakSleep`；Reflect 开着须醒着同坐。深夜休息仍 Expand A Idle→DORMANT / 2h live / wellness 冷启动 |
