...
- **不要**用 `focustiger.app`
- **宣传站（2026-08-20 · 2026-09-03 现网上线）**：公开营销域也是 **`twinsology.com`**（Slice 0 静态页在 `marketing-site/`；Cloudflare Pages `twinsology-marketing`）。网站 DNS 与发信 DNS **共用该域、分记录**；上 Pages 时 **禁止**改 MX / SPF / DKIM / `NEWSLETTER_FROM`。权威：`task-briefs/task-marketing-site.md`。
