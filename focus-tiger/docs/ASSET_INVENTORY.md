...
| wave-hello | 19 | waveHello / waveHelloWelcome | **欢迎池停接线**；**Collections 引擎已接线**（`collectionsWaveHello`；抽屉不列） | `welcomeBack` / 10min 自主仍另议 |
...
| incenseComplete | DOM 叠层 | 实验室钮「一炷香完成」可预览（仍会消失）；会话结束自动播放未接线（2026-07-25 放弃）。**2026-08-17**：`playBirthAt` 复用于莲花池出生（花留下） |
...
| 文件 | 用途 |
|---|---|
| `sanctuary-preview.png` | Support · Yin's Sanctuary 卡头图（2026-08-15 影棚白底改为暖纸 `#e8dfd2` 烘焙进像素） |
| `membership-meditation-preview.png` | Support · Yin Membership 卡头图（闭目坐禅；入库 2026-08-11；同日烘焙暖纸底） |
| `tea-drinking-preview.png` | Support · Buy Yin a Tea 卡头图（2026-08-15 与左/中卡同样烘焙暖纸 `#e8dfd2`，避免 GitHub/文件预览发黑） |
...
## Mustard Seed · Sumeru memorial seal badge（非序列帧）
