...
| H6 | **验收须带覆盖范围**（2026-09-16）：TRACKER 行 / PR / 「已修好」话术必须写清 **自动化锁了哪些场景、人工测了哪些、明确未测哪些**。禁止用「有问题」「待人工测试」四字代替范围。只补「刚才那个复现」的检查、不列同类失败方式 = 授权下一次马甲。叠层新 UI 对照 `DOC_CODE_CONTRACT` **O-04** 三态键 + 其余列一次填齐，禁止「发现一个洞补一条扫描」。 |
...
**本回合落地**：§6.23 补根因的根因；运行时 `pickConfideLine` 排除上一句可见字面 + generate 失败重抽 fallback；单测锁连续 8 次冻住 exclude 不得连打；连续 generate 失败 ≥2 打 `console.info`（无网络）。2026-09-16：O-04 表面表 + `overlay-contract-ui-check.js` 按列扫；提醒保存补 hint/冷启动共存 e2e。2026-09-18：O-04 `failureFeedback` 升级为 `mutationFeedback.{pending,success,fail}`（#839 Slice 1）。
