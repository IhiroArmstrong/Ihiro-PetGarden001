...
4. **闸门放在 push，不放在本地 commit**  
   **所有 tasks 收尾**（交互修复 / 文档 / 调研 / 素材 / 回归锁）→ **自动本地 commit，不必询问**；`git push` → **仍须用户明确要求**。  
...
| N6 | **任何实质性 task 收尾** → **立刻自动本地 commit**；**禁止**未确认 push |
...
- [ ] **自动本地 commit**（**所有 tasks 都要**；message 含 Fix / Docs / Task / 回归锁等关键词）；未 push  
...
| 动作 | 谁决定 | 目的 |
|---|---|---|
| 本地 `git commit` | Agent 在**任何实质性 task 收尾时自动执行** | 固化基线，防多会话冲掉 |
