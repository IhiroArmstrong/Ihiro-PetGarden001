...
6.1 **验收步骤与代码契约漂移 = 假回归**（2026-07-25 · L249）  
   P0/人工清单若仍写「Arrival 后可选 Companion → 再点 Here & Now」，而代码已是 **Choose 鞠躬后自动开表**，测试者易在 **Rise 后门闩已关** 时直接点 Here & Now，看到 Notice，误报「以前修过又坏了」。  
   e2e I2 还把「未就绪 → Notice」锁成正确 → 自动化全绿也拦不住这份假回归。  
   **须**：改步骤与 `SCENARIO_TESTS` / TEST_TRACKER 同行对齐；补「门闩就绪 → 点选开表」专用 DOM（e2e A4）；口头「点选即开表」必须带门闩/Offline 例外。
