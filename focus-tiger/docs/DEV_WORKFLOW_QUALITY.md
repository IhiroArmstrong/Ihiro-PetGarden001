...
**一句话（整套机制）**：  
回归锁 = 防假修好（回流 + 门闩 + 冒烟 + 自动 commit）+ 防改坏（已好清单 + 继承契约 + 高风险面）+ **汇报可扫读**（末尾决策/知情清单）。
...
| N13 | 交互修复收尾、声称「已修好」**之前**，须先跑通自动化冒烟（`npm run test:smoke` / `npm test`）；不过 → 并入 N5，不得声称修好（见 §6.1 S3） |
...
- [ ] 回流路径已跑通  
- [ ] 静默失败已消除  
- [ ] **自动化冒烟已跑通**（`npm run test:smoke`；不过不得声称修好）  
...
3. **本地 `npm run dev` 读工作区，不依赖 commit 才生效**  
   ...
   另：**「看起来像 bug」也可能是脏 localStorage**（DORMANT / 提醒额度 / hints-seen / intentions 等）。当前只有零散清 key；**一键重置**见 §6.3（待落地）。测回流 / 次日场景前，先排除脏状态再开修。
...
> **优先级**：§6.1 最先（减轻手工回归负担）；§6.2 并入 6.1 的 S5；§6.3、§6.4 可后排。  
> 如何变成真约束：见 **§6.5 晋级路径**。

### 6.1 自动化冒烟测试（最高优先级）

**问题**：回归发现方式长期是「改完 → 用户手工点 → 再报告」，成本压在用户身上，且永远在改动**之后**才发现问题。`SCENARIO_TESTS.md` 已有可执行剧本，但此前只指导人工。

**要求**：

| # | 规范 | 状态 |
|---|---|---|
| S1 | 从场景 A–D 挑选确定性步骤，转成自动化冒烟（主链路即可） | **已落地**（控制器级，见下） |
| S2 | 概率触发步骤（idle 变体等）不纳入自动化，继续人工 + 强制触发入口 | 持续 |
| S3 | 回归锁收尾（N6）前须先跑冒烟；不过 → 并入 N5，不得声称修好 | **已升格 N13** + regression-lock |
| S4 | 自动化新增/修改后，同步 `SCENARIO_TESTS.md` 注明「已自动化，见 tests/…」 | 本次已同步 |
| S5 | 用户报告且确认修复的 bug，默认追加自动化回归；无法自动化才降级 TEST_TRACKER 人工锁（**扩展 N3**） | **已升格 N12** |

**技术选型（2026-07-20）**：项目仅有 `node --test`，**无** Playwright / Cypress。按「不擅自引入新依赖」：

- **当前**：`src/core/scenario-smoke.test.js` — 控制器/Store 串联冒烟（8 条，场景 A–D 主链路 + 回流 J）。跑：`npm run test:smoke`（或 `npm test`）。  
- **未做 / 仍须人工**：浏览器 DOM、序列观感（Idle 不闪、Celebrating 动画）、真实切标签页 60s、Ambient 面板等。  
- **可选后续**：若要真浏览器 e2e，再单独立项引入 Playwright（须书面拍板加依赖）。

### 6.2 Bug 修复必留回归项

并入 **§6.1 S5 / N12**，不单独重复任务。门闩失败用例仍是子集，不是全量。

### 6.3 一键清空本地状态（开发环境调试工具）· 后排

| # | 规范 |
|---|---|
| L1 | DEV debug 面板「重置全部本地状态」，一键清空项目 localStorage key；仅开发环境，不得进生产构建 |
| L2 | 清空后刷新，回到等同场景 A 开局（全新用户） |
| L3 | §4 注意事项 3：测不到 / 像 bug 时，**先用此按钮排除脏状态**（落地后勾掉） |

### 6.4 共享资源对照表 · 后排

| # | 规范 |
|---|---|
| R1 | 维护对照表：`EmotionController` / `IdleOrchestrator` / 各 localStorage key 被哪些功能共用；touch 前先查表 |
| R2 | 与 §2.3 高风险面**互补**：§2.3 = 已知踩坑点；本表 = 当前共享资源谁在用 |
| R3 | 新增 emotion key / localStorage key / idle 变体时顺手补一行 |

可放 `ARCHITECTURE.md` 附录或 `docs/SHARED_RESOURCES.md`；勿与 §2.3 合并成一张表。

### 6.5 其它既有项 + 晋级路径

**其它**：

- [ ] 动高风险面前记 `baseline: <short-hash>`（开工回复 / Task Brief）  
- [ ] 高风险面：按模块补「最小复测脚本 / 控制台加速手法」  
- [ ] Task Brief 模板：固定「已好清单 / 保护面 / baseline」字段示例  
- [ ] 非交互类改动的轻量门禁是否与交互修复区分  
- [ ] 多 Agent / 多会话并行时的基线约定  
- [ ] 反面教材短表（Companion 回流、Idle 闪、Sit 抢点、脏 localStorage）  
