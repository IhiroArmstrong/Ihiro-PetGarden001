...
| N29 | 触及 overlayBusy / HUD 呼吸驱动 / 遮罩 dim：Task Brief 须写出点名消费者的核对结论句（禁止纯打勾）。写法见 `COLLAB.md` 第七节；清单见 `SHARED_RESOURCES.md` §4.1–4.2 与 `Z_INDEX.md` Idle 常驻 chrome |
...
- [x] Task Brief：共用机制核对须写结论句（非纯打勾）→ **COLLAB.md 第七节**（2026-09-09）
...
| 2026-09-09 | 升格 N29：共用机制核对须写 Brief 结论句；清单扩 `SHARED_RESOURCES` §4.1–4.2 / `Z_INDEX` Idle chrome；写法 SSOT `COLLAB.md` 第七节 |
...
| 2026-09-16 | §6.23 升级为「验收须带覆盖范围」通用条目；提醒保存 / Witness 接线 / Confide 复读三例并入 |
| 2026-09-16 | §6.23 补 H7 +「穷尽失败模式 → 建表 → 按表扫」通用方法论；实例见 `DOC_CODE_CONTRACT` §四 |
...
### 6.20 Focusing Recover 幽灵提示「隐蔽」· 错开只锁坐标（2026-08-23）
...
### 6.23 验收须带覆盖范围（假关 / 马甲回归 / 叠层漏接）（2026-09-04 · 2026-09-16 升格）

**现象**：用户 2026-08-31 报 Confide 同面板约第 5–6 句复读首句 generate（`Yes.` / `I am curious…`）。Agent 合入 echo sanitize / sequence dispose（#492 一带），把 `ISSUE_LEDGER` 标成 **已解决**。TRACKER「第 5–6 句不得复读」行一直是 **待人工测试**，从未关单。2026-09-04 QA Electron（`origin/develop` tip `65a50add`）再测：约第 **11** 句起连打 `Sit a while. Tea is still warm.`（corpus `fallback-03`）。用户问「老问题为什么一直没有解决」。

**不是**神秘回潮把 echo 修删掉。查证：

| 层 | 事实 |
|---|---|
| A · 账本假关 | `ISSUE_LEDGER`「已解决」= Agent 自判代码路径已堵 echo。**没有**用户在 tip 上走完 ≥6 句闲聊。regression-lock §7：**人工复测未做不得写已修复**；ledger 用了更松的词。 |
| B · 契约锁窄 | 单测锁「不要把上一句 generate **全文**再贴出来」。TRACKER 步骤写明 **允许** generate 失败走 corpus 茶句。失败（或 KV 序列撑不住）后每次都抽到同一 `fallback-03` → 用户看到的仍是「复读」，只是马甲从 echo 换成茶。 |
| C · 能聊已关 | L2 行 2026-08-25 已标「已通过」，关单句是「问答基本可以」。分列 ① 写过「不得从第 3 句起整段改茶句」，但关单覆盖分工**没有**把「第 10+ 句仍生成」列进人工已覆盖。茶套话回归后关单被事实打穿。 |
| D · 门槛后移 | 上次约第 5 句、这次约第 11 句，符合「上下文/序列越长越容易 generate 失败 → 同一 fallback」而不是「第 N 句魔法数」。修 echo 不修失败后的可见多样性 = 阈值往后推，用户还是撞上。 |

**根因的根因（2026-09-04 分析师）**：验收标准一直锚定在「这一次复现长什么样」（echo 首句 / 第 5 句 / 茶句 id），而不是「复读」这个**属性**。所以马甲能换：`Yes.` → 茶句 → 将来别的句子；阈值从约第 5 句挪到约第 11 句。代码层对应：`_sessionExclude` 是 Set，三句 fallback 用尽后 **size 冻在 3**，`pickConfideLine` 空池回绕仍用同一个 salt → 连打 `fallback-03`。

**因果一句话**：ledger 已解决 ≠ TRACKER 关单 ≠ 用户可感知不再复读；自动化只锁 echo 形态，等于授权下一件马甲。**堵法**：锁「连续可见闲聊答句不得字面相同」，与诱因无关。

**工作流补丁（须遵守）**：

| # | 要求 |
|---|---|
| H1 | `ISSUE_LEDGER` 标「已解决」前：对应 `TEST_TRACKER` 行须已有用户书面关单，或状态仍为「待人工测试 / 有问题」且 ledger 只写「代码已合、待复测」——**禁止**用已解决代替关单 |
| H2 | 用户报「复读 / 套话」时，回归锚须锁**可见答句字面**（连续不得同一句），禁止只锁 sanitize(echo of turn 1) |
| H3 | TRACKER 写「失败可见语料 fallback」时须加：**连续失败不得重复同一 fallback 句**；否则等于授权茶套话 |
| H4 | 已关单的「能聊」若用户再报套话 → **同回合**把该行改回「有问题」并开 open-blocker；禁止只改邻接「复读」行而留下「已通过」 |
| H5 | 关单问一句：这次关的是**具体复现**还是 TRACKER **通用规范**？规范未单独验收 → 不得顺带关规范行（`TEST_TRACKER`「标已通过」第 6 款） |
| H6 | **验收须带覆盖范围**（2026-09-16）：TRACKER 行 / PR / 「已修好」话术必须写清 **自动化锁了哪些场景、人工测了哪些、明确未测哪些**。禁止用「有问题」「待人工测试」四字代替范围。只补「刚才那个复现」的检查、不列同类失败方式 = 授权下一次马甲。叠层新 UI 对照 `DOC_CODE_CONTRACT` **O-04** 七列一次填齐，禁止「发现一个洞补一条扫描」。 |
| H7 | **类别契约须穷尽列维再按表扫**（2026-09-16）：判定为**一类**系统性缺陷（同一根因可在不同表层/马甲再现）时，开工前走下方「穷尽失败模式 → 建表 → 按表扫」三步；实例清单见 `DOC_CODE_CONTRACT.md` §四。禁止等第二次事故才补列、禁止只扫出事那一列。 |

**通用方法论：穷尽失败模式 → 建表 → 按表扫**（2026-09-16 · 复制进 Brief / PR）

**何时用**：已判定缺陷是**一类**漏洞，不是一次性笔误。O-04 叠层七列、V-01 跨视口可见性、Confide「复读」属性锁，都是**同一方法**的实例化——不是三套平行流程。

| 步 | 动作 | 禁止 |
|---|---|---|
| **1 · 穷尽失败模式** | 问「**这一类**用户还能怎样失败？」按**属性**列维（接线漏、失败静默、被遮挡抢点、连续字面复读…），不靠「这次复现长什么样」 | 只锁当次 echo / 当次 z-index / 当次 timeout |
| **2 · 建表** | 维度**一次列齐** → SSOT 表（优先 `src/core/*ContractRegistry.js`）；每行一实例，每列一失败维；做不到的列须显式 `gap` / `grandfather` | 发现一个洞补一列；新 occupancy 默 gap |
| **3 · 按表扫** | `scripts/*-doc-check.js` 注册进 `docs:check`，**整表**扫；行为锁 **(b)** 按行或按维声明；TRACKER 写 H6 覆盖范围三分法 | 只补出事那一列；契约单测绿冒充遮挡共存 e2e |

**已落地实例**（细则 `DOC_CODE_CONTRACT.md` §四）：

| 领域 | 表（SSOT） | 按表扫 |
|---|---|---|
| 叠层可点 UI | O-04 七列 · `OVERLAY_UI_SURFACE` | `overlay-contract-ui-check.js` |
| 跨视口可见 | V-01 · `visibilityContractRegistry` | `visibility-contract-doc-check` + e2e 整表 |
| Confide 复读 | TRACKER 规范行（属性：连续可见字面不得相同） | `confideReplyUniqueness.test.js` |

**与 H1–H6 的关系**：H6 规定 TRACKER **怎么写**覆盖范围；H7 + 本段规定**怎么建**类别盾——列维须穷尽、扫描须按整表。新开类别契约 → 先走 `DOC_CODE_CONTRACT.md` §四通用清单，再套该领域实例（例 O-04）。

**三例（同一条，不是三条平行规范）**：

1. **Confide 复读**（本条原事故）：ledger 已解决 ≠ TRACKER 关单；单测只锁 echo 形态 → 茶句马甲。  
2. **提醒 → 保存**：点气泡无反馈 / 面板 z 低于 hint 34。合入后若 TRACKER 仍写「有问题」而不写「e2e 锁了保存、未锁冷启动与 hint 共存」，下一轮仍会假绿。  
3. **留痕迹选句**：registry 有 id 但开 picker 未 `request*Slot()` → 摸头锚抢点；提交失败静默 return。契约单测绿 ≠ 遮挡共存已测。

**本回合落地**：§6.23 补根因的根因；运行时 `pickConfideLine` 排除上一句可见字面 + generate 失败重抽 fallback；单测锁连续 8 次冻住 exclude 不得连打；连续 generate 失败 ≥2 打 `console.info`（无网络）。2026-09-16：O-04 表面表 + `overlay-contract-ui-check.js` 按七列扫；提醒保存补 hint/冷启动共存 e2e。

### 6.24 手动复测清库配方口口相传 · CI 绿手测红（2026-09-13）

**现象**：吹花冷启动人工复测失败（占用像花、动画不播），自动化全绿。后来发现当次 Console 漏清 `scene-anim-daily`，症状与真代码 bug 几乎同形。权威步骤其实写在 `SCENARIO_TESTS` 场景 V / `SHARED_RESOURCES`，但验收行把错误配方又誊抄了一遍。

**因果一句话**：没有唯一运行时配方 → 文档互相复述 → 手误被钉成官方口径。堵法不是「下次记得看文档」，是 **`debugScenarioReset.js` + DEV `__ftDebug.resetScenario` + 半清库 console 警告 + 排查 Step A0**。

**工作流补丁（须遵守）**：

| # | 要求 |
|---|---|
| R1 | 人/Agent 复测只跑 `window.__ftDebug.resetScenario('<id>')`（**仅 Vite DEV**；生产不挂）。e2e 从同一模块 import 配方。 |
| R2 | TRACKER / PR 验收行只许**引用**场景 id，禁止默写 `localStorage.removeItem`。 |
| R3 | CI/单测绿、手测红 → 先走 `ui-bug-triage` Step A0，再怀疑刚合的代码。 |
| R4 | 残余风险：场景「该清哪几个 key」仍是人工映射。新加日配额/门闩 key 时，同 commit 改模块 `*_RESET_*_KEYS` **和** `debugScenarioReset.js` 配方。 |

### 6.25 练习备份扩 key 未锁「导出→导入→可见」闭环（2026-09-13 · PR #713 人工验收）
...
**本回合落地**：`idlePracticeBadges` honor restored sanctuary marks；backup **v4** 纳入 `focus-tiger.focus-coins.v1` + import refresh；`fix-backup-import-badge-restore` tracker 行 + 本 §6.25。
