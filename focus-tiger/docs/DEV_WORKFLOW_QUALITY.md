...
| # | 规范 |
|---|---|
| N14 | 每次任务汇报（含文档-only / 调研 / 修复）的回复**末尾**必须有一个**独立小节**「**待你决定 / 待你知道**」清单；**禁止**只在正文叙述里带过。无事项时写「无（本次无需你拍板或额外知情）」。会话明显结束时「待你知道」须写明锁态（已删 / releasable）；拆盘不默认同条提议（口令见 `git-worktree-hygiene`）。 |
...
| F5 | 关单级（已合 `develop`）只认主仓 + 既有 5173：先 ff-pull 再硬刷新（`TEST_TRACKER`「主干一次性关单验收」）。**研发自检**（未合入旁支）须写明 **分支 + 非 5173 端口 + worktree**——测到无修 tip ≠ 本修无效（同型 08-02 薄荷绿清空）。**禁止**让 feature Vite 占用 5173 |
...
**补丁（F5 · 2026-08-15 修订）**：关单级打开 `http://127.0.0.1:5173/?product=1` 之前，本机主仓须已 ff-pull 到当时 `origin/develop` tip（硬刷新 ≠ git pull）。未合入的修仍须写清旁支 worktree + **非 5173** 端口；**禁止**默认「打开 5173 即本修」，也 **禁止**让 feature Vite 占 5173。细则 `TEST_TRACKER`「主干一次性关单验收」。
...
| G7 | 研发自检写清 **旁支 worktree 端口**（本修 ≠ 主仓 5173）。关单级只认主仓 5173，见 `TEST_TRACKER`「主干一次性关单验收」 |
