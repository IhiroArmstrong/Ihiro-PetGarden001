...
| 动作 | 谁决定 | 目的 |
|---|---|---|
| 本地 `git commit` | Agent 在**任何实质性 task 收尾时自动执行** | 固化基线，防多会话冲掉；**已完成且验证通过的改动不得跨任务周期悬置** |
| `git push` | **仅用户明确要求** | 远程闸门；避免半成品上远程 |

补充纪律：

- **commit 单位**按「逻辑完整改动」划分，不按时间片划分。  
- **纯文档任务同样适用**：只要 `docs:check` 或相应校验已通过，就应当立即 commit。  
...
禁止：post-commit 自动 push、保存即 commit、未经确认推大体量抽帧 / `node_modules`、用 `update docs` / `misc fix` / `wip` 之类 message 交差。
