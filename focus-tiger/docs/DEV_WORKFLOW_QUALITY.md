...
**因果一句话**：**ledger 已解决 ≠ TRACKER 关单 ≠ 用户可感知不再复读**；自动化只锁 echo 形态，把产品允许的茶句 fallback 变成新的连打套话。
...
| H2 | 用户报「复读 / 套话」时，回归锚须锁**可见答句字面**（连续 N 次不得同一 corpus id / 同一英文句），禁止只锁 sanitize(echo of turn 1) |
...
**本回合落地**：记入 TRACKER / ISSUE_LEDGER；**不改** Confide 运行时（一次一任务：先闭环记录与根因）。
