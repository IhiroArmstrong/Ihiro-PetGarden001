...
| **2** | A′ | Moment Whisper（每键一生一次）+「?」桥接 | `task-five-moments-whisper-a.md` | **实现中**（`feature/five-moments-whisper`） |
| **3** | D′ | Journey Log（本地 · Tea Log 模式；非 HealthKit） | `task-journey-log-d.md` | **实现中**（`feature/journey-log`） |
