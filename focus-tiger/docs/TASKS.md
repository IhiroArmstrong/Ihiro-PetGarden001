...
```
> **2026-07-30 口径（2026-08-16 修订）**：本项**不是**「本地电脑版 APP」的终局。电脑版 Mac DMG 壳已拍板 **Electron**（见 `PROCESS.md` Backlog「**本地桌面 APP 打包**」+ Brief `task-desktop-shell-electron.md`）。PWA 继续只服务浏览器安装。**v1.0.0 纯本地 / v1.1 云端**仍有效：核心路径不依赖联网；service worker 只能增强，不得变成「无网不可用」。  
