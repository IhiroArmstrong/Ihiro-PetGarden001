...
```
> **2026-07-30 口径**：本项是 Phase 0 候选交付之一，**不是**「本地电脑版 APP」的最终打包选型。桌面双击可跑的路径（Electron / Tauri / PWA·薄壳）与云端依赖下的离线降级，见 `PROCESS.md` 开放决策与 Backlog「**本地桌面 APP 打包选型（v1 阻塞）**」——须先选型，再决定本任务是否仍做、做到哪一层。
