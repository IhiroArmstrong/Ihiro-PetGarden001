...
## 7. 客户端打包（Electron / DMG · 与云 overlay 正交）

> **2026-09-10 拍板**：本层主战场仍是云上表/闭包 overlay（§2–§5）；Electron 打包后的客户端源码保护是 **另一条线**（Epic **#5** `mac-dmg-release`），**不归** §5 口令队列。

### 7.1 地面真相

Electron + DMG 打包之后，源码暴露情况会变，但**没有变成「安全」**——用户依然能看到源码，只是门槛从「右键查看网页源代码」变成「需要多走几步」：

| 事实 | 说明 |
|---|---|
| `app.asar` | 前端 HTML/CSS/JS 塞进 `.app` 包内的归档文件；Mac 右键应用→「显示包内容」可找到 |
| asar ≠ 加密 | 开源工具 `npx asar extract app.asar output/` 可原样解压，与构建时几乎一致（若做了压缩混淆，会变成难读单行，但逻辑仍可扒） |
| 实际门槛 | 从「网页 F12」→「下载 DMG、装上、解压 asar」；对懂一点技术、真想抄的人（如竞品团队），<10 分钟 |

### 7.2 可选加深（仍非「防死」）

| 手段 | 效果 | 成本 |
|---|---|---|
| 代码混淆（terser / obfuscator.io） | 扒出来的代码难读，增加理解逻辑成本 | 低；挡不住有耐心的人 |
| **V8 字节码编译**（如 bytenode） | 不再是直接可读源码，需额外反编译功夫 | **Electron 生态相对成熟；性价比合理** |
| C++/Rust 原生模块 | 防护力显著更强 | 工程复杂度与维护成本显著上升；对 score/徽章/纪念印公式 **大概率不值得** |

### 7.3 拍板结论

- **建议不变**：score/徽章/纪念印公式本身不是「抄走就能复刻产品护城河」的核心机密；真正值钱、值得投入保护精力的是阿寅人设、美术、内容库（版权能覆盖、也更难被简单复制）。
- **若只想加一道「防随手抄」门槛**：**V8 字节码编译是性价比比较合理的选项**。
- **禁止**为「防剽窃」把核心逻辑做到原生模块量级（除非保护对象本身值得且已单独立项）。
- **任务线**：Epic **#5** 子切片 · `TASKS.md`「苹果 DMG 发布准备」· 与 Epic **#21** 耦合（概念对齐，排期归 #5）。
