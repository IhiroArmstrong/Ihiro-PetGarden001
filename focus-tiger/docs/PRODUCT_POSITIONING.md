...
技术边界（已认可，实现另 Brief）：`node-llama-cpp` 只在 Electron 主进程；模型首次下载不进 DMG；Focusing 时卸载；**窄屏壳不加载、不露出生成入口**。**低配（总内存 ≤8.5 GiB，Mac 与 Windows 同样）默认不出入口。** L1 开工前 **不上产品入口**。L2 内部多轮对话须攒跑偏案例调 prompt，**禁止** L0/L1 一过就给真实用户。**付费（2026-08-20）**：**Focus Tiger Pro US$12.99/月** 含 Base（B 轨）+ 本地智能体；Stripe Price 已记、Checkout 未接。本地模型仍只 Electron；L1 后付款可走 Web。
