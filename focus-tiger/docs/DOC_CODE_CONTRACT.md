...
| **(e) TEST_TRACKER 碎片（试点）** | 新增验收行走 `docs/tracker-entries/`；不要求功能 PR 同步机器块；拼装触发见 `TEST_TRACKER.md`「拼装触发」（批量测前 / 下班前 / ≥5 WARN） | `scripts/assemble-tracker.js`（`tracker:check` 已并入 `docs:check`；≥5 WARN 仍绿） |
...
```bash
cd focus-tiger
npm run docs:sync       # 刷新全部 (a) 机器块（pre-commit 自动跑）
...
cd focus-tiger && npm run docs:sync   # 刷新机器块（含 kb-live-gap-audit.md）；钩子自动 stage 变更
...
| **覆盖** | session-lock 技术闸 + `docs:sync`（机器块自动刷新并 stage）+ 结构对齐 `docs:check` + 行为契约 `test:smoke` 主体（不含 `test:e2e`） |
...
新 occupancy 行 **禁止** `mode: 'gap'`。存量 gap 必须 `grandfather: true`。z-index 常量化全表扫描仍是 **Z-dim**，本条不替代。
