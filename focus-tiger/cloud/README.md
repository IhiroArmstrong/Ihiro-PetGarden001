...
独立 API（TypeScript + Wrangler）。含历史 stub 路由 + **Founder Supporter Pack**（一次性 Stripe Checkout + KV）。
...
| `POST` | `/api/create-checkout-session` | Stripe Checkout（one-time）→ `{ url }` |
| `POST` | `/api/stripe-webhook` | Stripe 验签 → 写 `SUPPORTER_KV` |
| `POST` | `/api/verify-supporter` | `{ email }` → `{ supporter, purchasedAt? }` |
...
| `/api/verify-supporter` | **10/min/IP**（单独桶） |
| `/api/stripe-webhook` | **豁免全局**；仍 **300/min/IP**（防 HMAC 刷量） |

### curl：邮箱核验（无记录时）

```bash
curl -s -X POST http://127.0.0.1:8787/api/verify-supporter \
...
```
cloud/
  wrangler.jsonc      # Worker 名 focus-tiger-cloud + SUPPORTER_KV
  src/
    index.ts
    types.ts
    lib/http.ts | validate.ts | cors.ts | stripe.ts | supporterKv.ts
    middleware/rateLimit.ts
    routes/dailyMessage.ts | emotionWeight.ts
    routes/createCheckoutSession.ts | stripeWebhook.ts | verifySupporter.ts
```
