...
| `POST` | `/api/focus-circle` | Focus Circle：`action` = `create` \| `join` \| `leave` \| `status` \| `presence_peek`（含 `hereTodayOthers`） \| `presence_heartbeat` \| `presence_leave` \| `was_here_mark` \| `witness_*`；`TIP_KV` `circle:v1:*` + `circle:v1:sit:{id}` + `circle:v1:here:{id}` + `circle:v1:witness:{id}`（最多 8 人；presence TTL 120s；witness 滚动 24h） |
