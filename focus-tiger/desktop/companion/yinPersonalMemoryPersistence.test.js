...
test('forgetYinPersonalMemoryEntry removes memory from json', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'ft-yin-memory-'));
  await setYinPersonalMemoryConsent(dir, true);
  const withMemory = await rememberYinPersonalMemoryFromConfide(dir, {
    userText: 'I prefer quiet, short reflections.',
    route: 'fallback',
    replySource: 'generate',
    turnOrdinal: 0,
  ...
  const after = await forgetYinPersonalMemoryEntry(dir, id);
  assert.equal(after.memories.length, 0);
  const roundTrip = await readYinPersonalMemoryState(dir);
  assert.equal(roundTrip.memories.length, 0);
});
