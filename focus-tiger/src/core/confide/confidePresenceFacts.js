...
const TREND_QUESTION_RES = [
  ...
  /\bhave\s+i\s+been\s+feeling\s+different\s+lately\b/i,
  /\bfeeling\s+different\s+lately\b/i,
  /\blast\s+couple\s+of\s+weeks\b.*\b(mood|emotions?)\b/i,
  ...
  /\bmood\s+trend\b/i,
  /\bmood\s+been\s+trending\b/i,
  /\btrending\s+up\s+or\s+down\b/i,
  /\bbeen\s+present\s+much\b.{0,40}\bcan you check\b/i,
  /\bcan you check\b.{0,40}\bbeen present\b/i,
  /情绪.*(改善|变好|变化)/,
  /这两周.*情绪/,
  /最近.*(两周)?.*(情绪|心情).*怎么样/,
  /心情.*(这两周|最近)/,
