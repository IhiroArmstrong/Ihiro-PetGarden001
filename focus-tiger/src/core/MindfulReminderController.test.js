...
import { TAB_RETURN_WHISPER_COOLDOWN_MS } from './tabReturnWhisperGate.js';
import { TAB_RETURN_WHISPER_MAX_MS } from '../input/AttentionSignals.js';
...
function whisperSetup({ nowMs = 2_000_000, offers = true } = {}) {
  const whispers = [];
  const emotions = [];
  const shown = [];
  let t = nowMs;
...
test('tab-return whisper: <A silent, [A,B] offer, >B classic Re-focus not whisper', () => {
  const { controller, whispers, shown, emotions } = whisperSetup();
  controller.startSession();

  const silent = controller.handleAttentionReturn({
    durationMs: 19_999,
  ...
  const mid = controller.handleAttentionReturn({
    durationMs: 20_000,
    displayEligible: false,
    whisperEligible: true
  });
  assert.equal(mid.kind, 'whisper');
  assert.equal(whispers.length, 1);
  assert.equal(controller.getSessionStats().refocusHandledThisSession, 0);
  assert.deepEqual(shown, []);

  const { controller: longCtrl, whispers: longWhispers, shown: longShown } =
    whisperSetup();
  longCtrl.startSession();
  const above = longCtrl.handleAttentionReturn({
    durationMs: TAB_RETURN_WHISPER_MAX_MS + 1,
    displayEligible: true,
    whisperEligible: false
  });
  assert.equal(above.kind, 'refocus');
  assert.equal(longWhispers.length, 0);
  assert.deepEqual(longShown, ['REFOCUS_ACKNOWLEDGE']);
  assert.equal(emotions.length, 0);
});

test('tab-return whisper: cooldown second return stays silent and does not Re-focus in-band', () => {
  const { controller, whispers, setNow, now } = whisperSetup();
  controller.startSession();
  const first = controller.handleAttentionReturn({
    durationMs: 45_000,
    displayEligible: false,
    whisperEligible: true
  });
  assert.equal(first.kind, 'whisper');
  assert.equal(whispers.length, 1);

  const blocked = controller.handleAttentionReturn({
    durationMs: 45_000,
    displayEligible: false,
    whisperEligible: true
  });
  assert.equal(blocked.kind, 'logged');
  assert.equal(blocked.reason, 'cooldown');
  assert.equal(whispers.length, 1);
  assert.ok(controller.getTabReturnWhisperCooldownRemainingMs() > 0);

  setNow(now() + TAB_RETURN_WHISPER_COOLDOWN_MS);
  const again = controller.handleAttentionReturn({
    durationMs: 45_000,
    displayEligible: false,
    whisperEligible: true
  });
  assert.equal(again.kind, 'whisper');
