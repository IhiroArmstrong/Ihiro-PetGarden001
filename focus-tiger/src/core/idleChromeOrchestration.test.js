...
describe('listSecondaryChromeEntries', () => {
  ...
  it('narrow drawer omits honesty and breath; includes companion/reminder/language/zen-cinema/supporter', () => {
    const entries = listSecondaryChromeEntries('narrow-drawer', allOn);
    assert.deepEqual(
      entries.map((e) => e.proxy),
      ['companion', 'reminder', 'language', 'zen-cinema', 'supporter']
    );
  });

  it('wide more omits honesty and breath; includes companion/reminder/language/zen-cinema/supporter', () => {
  ...
  it('language + zen-cinema + supporter remain when secondary gates off (Sound is not a menu row)', () => {
